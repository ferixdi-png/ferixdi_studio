...
${soloMode ? `  "dialogue_A_ru": "15-30 слов монолог, макс 2 символа |. Персонаж говорит прямо в камеру",
  ...
  "photo_scene_en": "${remake_mode ? 'КОПИЯ СЦЕНЫ ИЗ ОРИГИНАЛА. Medium shot (по пояс). ТА ЖЕ локация, ракурс, композиция, позы, освещение. Для КАЖДОГО персонажа — ДОСЛОВНО вставь character_en + wardrobe_anchor + signature_element. Устройство НЕВИДИМО, руки заняты реквизитом. Персонажи в СВОЕЙ одежде. 200-300 слов EN + negative prompt' : 'ОЧЕНЬ ДЛИННЫЙ И ПОДРОБНЫЙ промпт 250-350 слов EN. Medium shot photo capturing the EXACT HOOK MOMENT (frame 0). НЕБАНАЛЬНЫЙ ракурс (не стандартный two-shot). Устройство НЕВИДИМО. ОБЯЗАТЕЛЬНО подробно: схема кадра и POV, микромимика каждого персонажа (положение бровей, рта, глаз), реквизит в руке A, конкретные детали фона (минимум 3 предмета), свет (направление, температура, тени на лице), текстура кожи и одежды, поза и жесты. + negative prompt'}",
  "video_emotion_arc": {
    "hook_en": "0.0-0.7s: ${remake_mode ? 'СКОПИРУЙ тип хука из оригинала + signature micro_gesture A. Стоп-скролл' : 'ПОДРОБНО 60-80 слов EN: стоп-скролл визуальный хук. Персонаж A УЖЕ В ДВИЖЕНИИ. Опиши: что делает A (жест, взгляд в линзу, реквизит), что делает B (side-eye, микромимика), камера (micro push-in), звук (вдох/щелчок/шорох). No text on screen'}",
${soloMode ? `    "monologue_en": "0.7-7.0s: описание монолога на английском — один персонаж, прямо в камеру, killer word ≈ 6.8s",` : `    "act_A_en": "0.7-3.5s: ${remake_mode ? 'СКОПИРУЙ темп и паузы из оригинала + micro_gesture A, B РОТ СТРОГО ЗАКРЫТ' : 'ПОДРОБНО 80-100 слов EN: A говорит провокацию. Опиши: артикуляция A (жесты руками, наклон корпуса, мимика на ключевых словах), B МОЛЧИТ рот СТРОГО закрыт (только: моргание каждые 4-6с, side-eye, nostril flare, eyebrow shift, finger tap). Камера: micro-movement. Дыхание, вдох перед фразой. No text on screen'}",
    "act_B_en": "3.5-7.0s: ${remake_mode ? 'СКОПИРУЙ ритм доставки killer word из оригинала + A РОТ СТРОГО ЗАКРЫТ, пафосная поза' : 'ПОДРОБНО 80-100 слов EN: B отвечает панчлайном. 0.25с ПАУЗА перед ответом (tension beat — B expression меняется: loaded micro-smirk). B: голос снижается перед killer word → 0.2с пауза → взрыв артикуляции → jaw snap → глаза сужаются. A МОЛЧИТ рот СТРОГО закрыт: mid-gesture freeze → глаза расширяются. KW≈6.8s. Camera: micro push-in на killer word. No text on screen'}",`}
...
════════════════════════════════════════════════════════════════`;
    ...
    if (video_file) {
      parts.push({
        text: `\n\n[🎬 ПРИКРЕПЛЁННОЕ ОРИГИНАЛЬНОЕ ВИДЕО — СМОТРИ ПОКАДРОВО И СЛУШАЙ КАЖДОЕ СЛОВО!

ТВОЯ ЗАДАЧА: создать ТОЧНУЮ КОПИЮ этого видео с нуля. Один-в-один.

1) ДИАЛОГ — ДОСЛОВНО: расшифруй КАЖДОЕ слово из видео. Не пересказывай, не улучшай, не сокращай. Верни dialogue_A_ru и dialogue_B_ru = ТОЧНАЯ РАСШИФРОВКА оригинала. 100% слов сохранить.
2) ДЕЙСТВИЯ — ПОКАДРОВО: что делает каждый человек в каждую секунду? Жесты рук, повороты головы, наклоны корпуса, шаги — ВСЁ воспроизвести.
3) МИМИКА — МИКРОВЫРАЖЕНИЯ: какие эмоции на лице? Когда поднимаются брови? Когда сужаются глаза? Когда открывается рот? Скопируй покадрово.
4) КАМЕРА — ТОЧНЫЙ РАКУРС: откуда снимают? Фронтально? Сбоку? Сверху? Средний план? Крупный? Скопируй точно.
5) ЛОКАЦИЯ — ВСЕ ДЕТАЛИ: что за место? Какие стены, пол, свет, предметы на фоне? Опиши детально.
6) СВЕТ — НАПРАВЛЕНИЕ И ЦВЕТ: откуда свет? Тёплый/холодный? Жёсткий/мягкий? Тени где?
7) ЗВУК — ФОНОВЫЕ ШУМЫ: какие звуки кроме речи? Музыка? Шорохи? Эхо?
8) ТЕМП И РИТМ: скорость речи, паузы между репликами, момент панчлайна — всё скопируй.
9) ЭНЕРГИЯ: нарастание напряжения, момент взрыва, тип финала — точная копия эмоциональной кривой.

ЕСЛИ ПОЛЬЗОВАТЕЛЬ НЕ ВЫБРАЛ СВОИХ ПЕРСОНАЖЕЙ — описывай людей ИЗ ОРИГИНАЛА как есть.
ЕСЛИ ПОЛЬЗОВАТЕЛЬ ВЫБРАЛ ПЕРСОНАЖЕЙ — замени людей на НАШИХ, но ВСЁ ОСТАЛЬНОЕ (действия, локация, ракурс, свет, диалог) = ТОЧНАЯ КОПИЯ.]`
      });
      parts.push({
