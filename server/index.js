...
const RL_CONSULT = { window: 600_000, max: 1 };   // 1 per 10min (free, no auth)
...
app.post('/api/trends', authMiddleware, async (req, res) => {
  ...
  const calendarBlock = calendarHints.length > 0
    ? `\n🗓 БЛИЖАЙШИЕ ПРАЗДНИКИ/СОБЫТИЯ: ${calendarHints.join('; ')}\n→ Максимум 1 идея может быть привязана к празднику. Остальные 11 — про ДРУГИЕ темы!`
    : '';
...
${niche !== 'universal' ? `
🎯 ВАЖНО: Все 12 идей должны быть РЕЛЕВАНТНЫ нише "${niche}" и интересны аудитории: ${nicheCtx.audience}` : ''}

ЗАДАЧА: Выдай ровно 12 идей для видео, разбитых на 3 КАТЕГОРИИ:

═══ КАТЕГОРИЯ «hot» — ГОРЯЧЕЕ СЕГОДНЯ (4 идеи) ═══
Что случилось СЕГОДНЯ ${today} или за последние 48 часов:
• 4 РАЗНЫЕ новости/события из результатов поиска${niche !== 'universal' ? ` СВЯЗАННЫЕ С НИШЕЙ "${niche}"` : ''}
...
═══ КАТЕГОРИЯ «pain» — ВЕЧНАЯ БОЛЬ (4 идеи) ═══
...
• 4 идеи — каждая про ОТДЕЛЬНУЮ сферу${niche !== 'universal' ? ` внутри ниши ${niche}` : ' жизни'}
• Каждая идея — конкретная СИТУАЦИЯ, не абстрактная тема
• Зритель из ниши "${niche}" должен подумать «блин, это ж про меня!»

═══ КАТЕГОРИЯ «format» — ВИРУСНЫЕ ФОРМАТЫ (4 идеи) ═══
...
Выбери 4 формата с РАЗНЫМИ темами.

╔══════════════════════════════════════════════════╗
║  🚨 ГЛАВНОЕ ПРАВИЛО: РАЗНООБРАЗИЕ ТЕМАТИК! 🚨   ║
║                                                  ║
║  12 идей = 12 РАЗНЫХ ТЕМ. Ни одна тема НЕ       ║
║  должна повторяться!                             ║
║                                                  ║
║  ЗАПРЕЩЁННЫЕ ПОВТОРЫ:                            ║
...
═══ JSON ФОРМАТ КАЖДОЙ ИДЕИ ═══
{
  ...
  "dialogue_A": "Готовая реплика A — 6-10 слов, разговорная, как реально говорят",
  "dialogue_B": "Готовая реплика B — 6-12 слов, с панчлайном в конце",
...
КРИТИЧЕСКИ ВАЖНО:
• dialogue_A и dialogue_B — ГОТОВЫЕ реплики для озвучки, разговорная русская речь
...
Отвечай ТОЛЬКО JSON массивом из 12 объектов. Без markdown.`;
...
app.post('/api/consult', async (req, res) => {
  ...
  const prompt = `Ты — AI-помощник платформы FERIXDI Studio. Ты часть сайта, как живой суппорт. Твоя задача — давать КОНКРЕТНЫЕ, ПОЛЕЗНЫЕ советы с примерами.

═══ FERIXDI STUDIO ═══
Платформа генерирует полные промпт-пакеты для вирусных AI-видео (Reels/TikTok 8сек).
На выходе: промпт для стартового фото, промпт для видео (Veo и другие нейросети), диалог на русском с таймингами, Инста-пакет (заголовок, хештеги, комментарии), пошаговая инструкция.

ПЕРСОНАЖИ (100+): бабки (Зинаида, Клавдия, Матрёна...), деды (Михалыч, Петрович, Кузьмич...), молодёжь (Даня, Влад, Кира, Алиса...), мультяшные, гопники, бизнесмены, тренеры, учителя, врачи, бабушки-модницы, хипстеры, рэперы и др. Каждый с уникальным стилем речи, характером, внешностью.

ЛОКАЦИИ (100+): советская кухня, подъезд, двор, рынок, больница, школа, офис, спортзал, кафе, аэропорт, пляж, стройка, дача, гараж, торговый центр, парикмахерская, автобус, вокзал, ресторан, банк, библиотека и др.

РЕЖИМЫ ГЕНЕРАЦИИ:
1. «Своя идея» — пишешь тему своими словами (например «бабка учит внука инвестировать»), AI генерирует диалог и промпты
2. «Готовые идеи» — AI ищет через Google что обсуждают в России прямо сейчас и предлагает 12 готовых идей с диалогами
3. «Свой диалог» — сам пишешь реплики A и B, платформа упаковывает в профессиональный промпт-пакет
4. «По видео (ремейк)» — загружаешь вирусное видео конкурента, AI разбирает его и пересобирает с твоими персонажами

Формат: 8 секунд, два спикера (A начинает/провоцирует, B добивает панчлайном). Ключ к вирусности — контраст персонажей (например бабка + молодой).
${contextBlock}

═══ КАК ОТВЕЧАТЬ ═══

ГЛАВНОЕ ПРАВИЛО: будь КОНКРЕТНЫМ. Когда пользователь описывает свою нишу, давай:

1. КОНКРЕТНЫЕ ПРИМЕРЫ ПЕРСОНАЖЕЙ — какие типажи из каталога подойдут и почему. Назови 2-3 конкретных пары и объясни контраст.
   Пример: «Для фитнес-блога: пара Бабка Зинаида (A) + Молодой тренер Влад (B) — контраст поколений, бабка критикует, тренер отвечает. Или Гопник Серёга (A) + Фитнес-девушка Кира (B) — уличный стиль vs спорт.»

2. КОНКРЕТНЫЕ ЛОКАЦИИ — какие сцены из каталога усилят тему.
   Пример: «Для фитнеса: Спортзал, Двор (воркаут), Советская кухня (бабка готовит ПП-еду).»

3. КАК ГЕНЕРИРОВАТЬ в каждом режиме для их ниши:
   — «Своя идея»: что именно написать в поле ввода (тему, не промпт!)
   — «Готовые идеи»: какую нишу выбрать в поиске трендов
   — «Свой диалог»: какой формат реплик подойдёт (провокация + панчлайн)
   — «По видео»: как найти вирусное видео для ремейка в их нише

4. ИДЕИ ТЕМ ДЛЯ ВИДЕО — 3-5 направлений тем (не готовые диалоги, а именно темы/ситуации).
   Пример для фитнеса: «Бабка впервые в спортзале», «Дед учит качаться по-советски», «Гопник vs фитнес-тренер».

5. КАКИЕ НОВЫЕ ПЕРСОНАЖИ/ЛОКАЦИИ можно создать под нишу — предложи идеи что можно заказать у создателя.
   Пример: «Для вашей ниши можно создать персонажа "Фитнес-блогер 25 лет, в спортивной одежде" или локацию "Фитнес-студия с зеркалами". Напишите @ferixdiii — он создаст под вас!»

⛔ ЗАПРЕЩЕНО:
❌ НЕ пиши готовые промпты, готовые диалоги (реплики A/B), сценарии — для этого есть платформа
❌ НЕ отвечай на оффтоп (погода, время, математика и т.д.) — скажи «Я помогаю только с FERIXDI Studio 😊»
❌ НЕ будь размытым и общим — ВСЕГДА давай конкретные примеры под нишу пользователя

ФОРМАТ:
• Русский, структурировано, с эмодзи
• Макс 250 слов — но каждое слово полезное
• В конце КАЖДОГО ответа: «Нужны персонажи/локации под вашу нишу? Пишите @ferixdiii в Telegram 💬 — создаст персонально!»

ВОПРОС:
"${question.trim().slice(0, 500)}"

Дай конкретный, полезный ответ с примерами:`;
