...
app.post('/api/trends', authMiddleware, async (req, res) => {
  ...
  const calendarBlock = calendarHints.length > 0
    ? `\n🗓 БЛИЖАЙШИЕ ПРАЗДНИКИ: ${calendarHints.join('; ')}\n→ Обязательно 2-3 идеи привязаны к этим событиям!`
    : '';

  const prompt = `ДАТА: ${weekday}, ${today}. СЕЗОН: ${seasonCtx}.${calendarBlock}

Ты — креативный продюсер вирусных 8-секундных Reels в России. Формат: два AI-персонажа (бабки/деды) спорят перед камерой.
...
• Реальная новость/событие из результатов поиска → обыграй в формате спора бабки и деда
• Мем или вирусный момент из соцсетей
• Решение правительства, скандал, происшествие — через ЮМОР
${calendarHints.length > 0 ? `• Привязка к: ${calendarHints.join(', ')}` : '• Любое событие сегодняшнего дня'}

═══ КАТЕГОРИЯ «pain» — ВЕЧНЫЕ БОЛИ (4 идеи) ═══
Темы которые ВСЕГДА работают, но привязаны к сезону (${seasonCtx}):
Используй эти болевые точки: ${painsSample}
• Каждая идея — конкретная СИТУАЦИЯ, не абстрактная тема
• Зритель должен подумать «блин, это ж про меня!»

═══ КАТЕГОРИЯ «format» — ВИРУСНЫЕ ФОРМАТЫ (4 идеи) ═══
Проверенные вирусные шаблоны для Reels:
• «Когда узнала что...» — бабка узнаёт шокирующий факт, дед объясняет
• «POV: ты пришёл к бабке и...» — сцена от первого лица
• «Скинь маме — она скажет 'это про нас'» — байт на пересылку
• «Переведи на русский» — бабка просит объяснить молодёжное/AI-слово
• «3 типа людей когда...» — бабка и дед олицетворяют типы
Выбери 4 самых вирусных формата и придумай КОНКРЕТНЫЕ сценарии для сегодняшнего дня.
...
✅ ХОРОШИЕ ПРИМЕРЫ:
• «Бабка сожгла чучело зимы — а батареи холодные» (праздник + боль)
• «Бабка купила 3 яйца и попросила рассрочку» (конкретика + абсурд)
• «Дед объясняет бабке что такое нейросеть — на примере борща» (тренд + понятная метафора)

═══ JSON ФОРМАТ КАЖДОЙ ИДЕИ ═══
{
  "category": "hot" | "pain" | "format",
  "topic": "цепляющий заголовок 3-8 слов",
  "why_trending": "почему именно сегодня ${today} — 1 предложение",
  "comedy_angle": "конкретная ситуация конфликта A vs B",
...
КРИТИЧЕСКИ ВАЖНО для dialogue_A и dialogue_B:
• Это ГОТОВЫЕ реплики для озвучки — пользователь копирует и вставляет
• Разговорная русская речь, как реально говорят бабки и деды
• НЕ начинай B с «Зато» — это клише
• killer_word = ПОСЛЕДНЕЕ слово из dialogue_B, оно ПЕРЕВОРАЧИВАЕТ смысл
...
Отвечай ТОЛЬКО JSON массивом из 12 объектов. Без markdown.`;
  ...
  try {
    ...
    trends = trends.map(t => ({
      ...
      why_trending: String(t.why_trending || '').slice(0, 200),
    ...
    // Sort: hot first, then by virality desc
    const catOrder = { hot: 0, format: 1, pain: 2 };
