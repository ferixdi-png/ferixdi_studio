...
const RL_GEMINI  = { window: 300_000, max: 1 };   // 1 Gemini request per user per 5min (cost control)
...
ТЕХНИКИ КОМЕДИИ (используй хотя бы ДВEЕ):
...
app.post('/api/generate', authMiddleware, async (req, res) => {
  ...
  if (!checkRateLimit(`gemini:${userId}`, RL_GEMINI.window, RL_GEMINI.max)) {
    ...
    const waitSec = entry ? Math.ceil((entry.windowStart + RL_GEMINI.window - Date.now()) / 1000) : 300;
    return res.status(429).json({ error: `Лимит: 1 запрос в 5 минут. Подожди ещё ~${waitSec} сек.` });
  }
