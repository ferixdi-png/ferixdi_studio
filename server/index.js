...
═══ ЕСЛИ ТИП A (ДИАЛОГ/СКИТ) — КОПИРУЙ ДИАЛОГ ═══
1. РАСШИФРУЙ ДИАЛОГ — каждое слово ДОСЛОВНО
2. dialogue_A_ru = ДОСЛОВНАЯ копия речи первого (можно изменить 1-2 слова макс)
3. dialogue_B_ru = ДОСЛОВНАЯ копия речи второго (можно изменить 1-2 слова макс)
4. killer_word = последнее ударное слово из речи B
5. НЕ ПЕРЕПИСЫВАЙ! Бери слова ИЗ ВИДЕО!
6. Темп, паузы, эмоции — КОПИРУЙ из оригинала
...
8. ОДЕЖДА ОРИГИНАЛА: что надето на людях в видео? (спортивная форма, повседневная, рабочая)

⚠️ photo_scene_en ОБЯЗАН описывать ТУ ЖЕ сцену, композицию и действие что в оригинале — только с НАШИМИ персонажами.
...
⚠️ Если в оригинале один человек делает действие (спорт/танец) — описывай ТО ЖЕ действие с нашим персонажем.` : `ШАГ 1: ОПРЕДЕЛИ ТИП — диалог/скит (есть речь) или визуальное/действие (нет речи).
...
[0.00–0.60] ХУК — ${hookAction?.action_ru || 'визуальный хук'} (звук: ${hookAction?.audio || 'ambient'}). Без слов. Зрителя надо зацепить за 0.6 секунды.
[0.60–7.30] МОНОЛОГ — ${charA.name_ru} говорит прямо в камеру. 15-30 слов, темп: ${charA.speech_pace || 'normal'}. Окно: 6.7с. Один персонаж, одна камера, прямой взгляд. KILLER WORD ≈ 7.1s.
[7.30–8.00] RELEASE — реакция/пауза/усмешка. НОЛЬ слов. Только мимика.` : `ТАЙМИНГ (строго 8 секунд ±0.2s):
[0.00–0.60] ХУК — ${hookAction?.action_ru || 'визуальный хук'} (звук: ${hookAction?.audio || 'ambient'}). Без слов. Зрителя надо зацепить за 0.6 секунды.
...
[7.30–8.00] RELEASE — ${releaseAction?.action_ru || 'реакция'}. НОЛЬ слов. Только смех и физическая реакция.`}
...
ТЕХНИКИ КОМЕДИИ (используй хотя бы одну):
...
🚨🚨🚨 КРИТИЧНО: photo_scene_en ОБЯЗАН описывать СЦЕНУ ИЗ ОРИГИНАЛЬНОГО ВИДЕО с нашими персонажами.
• СКОПИРУЙ: место действия, композицию кадра, ракурс камеры, позы, действия, освещение из оригинала
• ЗАМЕНИ: людей из оригинала на НАШИХ персонажей (используй их внешность из ПАСПОРТА ИДЕНТИЧНОСТИ)
• Если в оригинале камера снимает со стороны (не selfie) — описывай камеру СО СТОРОНЫ
• Если в оригинале спортивная площадка/зал/улица — описывай ТУ ЖЕ локацию
• Если в оригинале человек делает действие (подаёт мяч, танцует, бежит) — наш персонаж делает ТО ЖЕ действие
• НЕ НАВЯЗЫВАЙ "Smartphone selfie" если в оригинале ДРУГОЙ ракурс!
...
${soloMode ? `    "monologue_en": "0.6-7.3s: описание монолога на английском — один персонаж, прямо в камеру, killer word ≈ 7.1s",` : `    "act_A_en": "0.6-3.8s: описание на английском",
  ...
  "remake_veo_prompt_en": "[ЗАПОЛНИ ПО ШАБЛОНУ НИЖЕ — НЕ ПЕРЕПИСЫВАЙ СТРУКТУРУ, ТОЛЬКО ПОДСТАВЬ ДАННЫЕ ИЗ ОРИГИНАЛА]\n\n[БЛОК 1 — СЦЕНА И ЛОКАЦИЯ: опиши точное место из оригинала — тип помещения/улицы, материалы стен/пола/фона, освещение (тип источника, направление, цветовая температура, качество теней), атмосфера, предметы заднего плана. 70-90 слов. ЕСЛИ в оригинале НЕ selfie — НЕ пиши selfie POV здесь.]\n\n[БЛОК 2 — РАКУРС КАМЕРЫ: скопируй точный угол из оригинала — selfie POV / сбоку / спереди / снизу / сверху; дистанция крупный план / средний / общий; движение камеры (статика / push-in / pan). Handheld micro-jitter 0.8-2px at 2-5Hz, breathing oscillation. 35-45 слов.]\n\n[БЛОК 3 — ПЕРСОНАЖ A: ВСТАВЬ ДОСЛОВНО БЕЗ ИЗМЕНЕНИЙ: ${charA.prompt_tokens?.character_en || charA.appearance_ru || charA.name_ru}. Wardrobe: ${charA.identity_anchors?.wardrobe_anchor || '—'}. Signature: ${charA.identity_anchors?.signature_element || '—'}. Затем опиши его позу и положение в кадре ТОЧНО как в оригинале.]\n\n[БЛОК 4 — ПЕРСОНАЖ B: ВСТАВЬ ДОСЛОВНО БЕЗ ИЗМЕНЕНИЙ: ${charB.prompt_tokens?.character_en || charB.appearance_ru || charB.name_ru}. Wardrobe: ${charB.identity_anchors?.wardrobe_anchor || '—'}. Signature: ${charB.identity_anchors?.signature_element || '—'}. Затем опиши его позу и положение в кадре ТОЧНО как в оригинале.]\n\n[БЛОК 5 — ДЕЙСТВИЕ И ДВИЖЕНИЯ: опиши что делает каждый персонаж последовательно — жесты, движения тела, мимика, взгляды — ТОЧНО как в оригинале, адаптировано под наших персонажей. Включи: blink rate каждые 3-5s, micro head-turns 1-3°, fabric rustle on movement, breathing visible. 70-90 слов.]\n\n[БЛОК 6 — ТЕХНИЧЕСКИЕ ПАРАМЕТРЫ И NEGATIVE PROMPT: 9:16 vertical portrait. 8 seconds. Hyperrealistic smartphone footage indistinguishable from real video. Phone sensor noise ISO 800-1600. Slight JPEG compression artifacts. Imperfect auto white-balance. Visible pores, fine lines, age-appropriate skin texture. Authentic fabric weave and surface materials. NO text, NO subtitles, NO captions, NO watermarks, NO UI, NO REC badge, NO smooth airbrushed skin, NO studio lighting, NO CGI look.]"` : ''}${product_info?.description_en || ctx.hasProductImage ? `,
...
app.post('/api/consult', async (req, res) => {
  ...
  if (!checkRateLimit(`consult:${ip}`, RL_CONSULT.window, RL_CONSULT.max)) {
    return res.status(429).json({ error: 'Лимит: 5 вопросов за 10 минут. Подождите немного. Для подробной консультации пишите @ferixdiii в Telegram.' });
  }
...
• Формат МОНОЛОГ (1 персонаж, соло): персонаж говорит прямо в камеру, как селфи-видео
...
• Себестоимость одного ролика ~3,6 ₽
...
• 5 режимов генерации: своя идея, готовые идеи из трендов, свой диалог, ремейк видео (AI анализирует и пересоздаёт), МЕМ-РЕМЕЙК (загружаешь мем/видео → получаешь промпт нулевого кадра + промпт анимации для Kling 2.6)
...
✅ Пятый режим генерации! Загружаешь любой мем или видео → AI анализирует и создаёт два промпта: 1) промпт нулевого кадра (Frame 0) для Google Imagen/Flow, 2) промпт анимации для Kling 2.6. Идеально для пересоздания вирусных мемов со своими AI-персонажами
...
• Для генерации видео — ТОЛЬКО Google Flow: https://labs.google/fx/tools/flow (Veo). Себестоимость ~3,6 ₽ за ролик
• Первый месяц генерации — в подарок при покупке конвейера. Детали по доступу к Google Flow — в обучении внутри Studio (14 уроков)
• НИКОГДА не рекомендуй сторонние сервисы, API, подписки, тарифы. Если спрашивают про API/тарифы — отвечай: «Никаких API не нужно! Studio бесплатно с промо. Для видео — Google Flow (~3,6₽/ролик), первый месяц в подарок. Вся механика в обучении (14 уроков)»
...
ВОПРОС: "${question.trim().slice(0, 2000)}"`
    ...
    : `Ты — AI-консультант FERIXDI Studio по бесплатному трафику через AI-видео. У пользователя НЕТ промо-кода. Твоя задача — ДАТЬ РЕАЛЬНУЮ ПОЛЬЗУ (общие концепции AI-видео трафика под его нишу), но конкретную экспертизу (пары персонажей, готовые планы, стратегии) оставлять за полным доступом (10 000₽). Пользователь должен уйти с пониманием КАК работает AI-видео для его ниши и желанием получить полный доступ.
...
• 5 режимов: своя идея, идеи из трендов, свой диалог, ремейк видео, мем-ремейк
...
• Объяснить почему AI-видео эффективнее таргета: себестоимость 3,6₽/ролик, органический охват, без рекламного бюджета
• Рассказать про принцип серийности: одни и те же персонажи в каждом ролике → узнаваемость → подписки
• Показать масштаб возможностей: 200+ персонажей + конструктор, 144+ локаций + конструктор, 5 режимов (вкл. мем-ремейк), 80+ пресетов, фото товара, референс стиля, English Adaptation, редактор диалога, раскадровка, антиповтор
...
📝 ШАБЛОН:
• Спрашивает про нишу → объясни КАК работает AI-видео в его нише (общий принцип), покажи масштаб, упомяни что конкретные пары + план — с полным доступом. 👉 10 000₽ @ferixdi.ai
• Спрашивает конкретную фишку → дай общую концепцию, потом «Конкретные связки и стратегии — в обучении (14 уроков) + AI-маркетолог с полным доступом». 👉 10 000₽
• Говорит «дорого» → посчитай: таргет 50-100К/мес, конвейер 10 000₽ один раз + первый месяц генерации в подарок, ролик 3,6₽, окупаемость с первой интеграции

🔧 КРИТИЧНО — ТЕХНИЧЕСКАЯ ЧАСТЬ (ЗАПОМНИ НАВСЕГДА!):
• Пользователю НЕ НУЖНЫ никакие API! Ни OpenAI, ни Midjourney, ни ElevenLabs — НИЧЕГО!
• Studio работает БЕСПЛАТНО после ввода промо-кода. Для видео — ТОЛЬКО Google Flow (https://labs.google/fx/tools/flow), ~3,6₽/ролик. Первый месяц генерации — в подарок при покупке
• Детали по доступу к Flow — в обучении внутри Studio (14 уроков)
• Если спрашивают про API/тарифы — «Никаких API не нужно! Всё что нужно — Studio + Google Flow. Детали в обучении после покупки»
