...
{${remake_mode ? `
  "original_video_analysis": {
    "num_people": "ЧИСЛО людей в кадре",
    "num_speakers": "ЧИСЛО говорящих",
    "people_description": ["50-80 слов на КАЖДОГО: возраст, пол, телосложение, одежда (цвет+материал+фасон), причёска, лицо, аксессуары, обувь. Если 3 — 3 элемента."],
    "body_positions": ["Позиция КАЖДОГО: сидит/стоит/наклонился + где в кадре (слева/справа/центр)"],
    "props_in_hands": ["Что ДЕРЖИТ каждый: газета/стакан/ничего. ТЕКСТ на предметах если виден."],
    "text_on_objects": "Видимый текст/логотипы в кадре. null если нет.",
    "location_exact": "ДЕТАЛЬНОЕ место 40-60 слов: каждый предмет, материал, цвет. НЕ 'barn' а 'бревенчатый сруб с голубыми наличниками, лавка, штакетник'",
    ...
    "camera_angle": "eye_level/low_angle/high_angle/dutch/over_shoulder",
    "camera_movement": "static/pan_left/pan_right/handheld_shake/zoom_in/follow/none",
    "lighting_exact": "Свет 20-30 слов: направление, тип, температура, тени",
    "dialogue_transcription": ["ДОСЛОВНО каждая реплика с указанием КТО: 'Персонаж 1: текст', 'Персонаж 2: текст'. Мат=мат."],
    "per_speaker_timing": ["'0.0-2.5s: Персонаж 3 говорит', '2.5-7.0s: Персонаж 2 отвечает', '7.0-8.0s: все смеются'"],
    "action_timeline": ["Посекундно: '0-2s: действие', '2-5s: действие'. Каждый жест, поворот."],
    "mood": "тип энергии: деревенский юмор / спор / абсурд / wholesome",
    "what_makes_viral": "1-2 предложения — что делает видео вирусным"
  },` : ''}
...
${soloMode ? `  "dialogue_A_ru": "15-30 слов монолог, макс 2 символа |. Персонаж говорит прямо в камеру",
  ...
  "photo_scene_en": "${remake_mode ? (isVideoPlaceholder ? 'ТОЧНАЯ КОПИЯ первого кадра на EN. 200-300 слов. Composition type: Two-Shot/Three-Shot/Solo/Group из num_people. Используй original_video_analysis: location_exact (каждый предмет, материал, цвет), people_description (КАЖДЫЙ человек отдельно + body_positions + props_in_hands + text_on_objects), camera_type+framing+angle, lighting_exact. Заверши: Character, clothing, environment strictly identical to source video. 8k. --ar 9:16. + negative prompt (NO text overlays, NO watermarks)' : 'ТОЧНАЯ КОПИЯ первого кадра на EN. 200-300 слов. Используй original_video_analysis для локации, камеры, света, предметов. ПЕРСОНАЖЕЙ замени: character_en + wardrobe_anchor + signature_element из Identity Lock. Позиции (сидит/стоит) и props — ИЗ ОРИГИНАЛА. 8k. --ar 9:16. + negative prompt') : 'ОЧЕНЬ ДЛИННЫЙ И ПОДРОБНЫЙ промпт 250-350 слов EN. Medium shot photo capturing the EXACT HOOK MOMENT (frame 0). НЕБАНАЛЬНЫЙ ракурс (не стандартный two-shot). Устройство НЕВИДИМО. ОБЯЗАТЕЛЬНО подробно: схема кадра и POV, микромимика каждого персонажа (положение бровей, рта, глаз), реквизит в руке A, конкретные детали фона (минимум 3 предмета), освещение (цветовая температура, направление, тени), текстуры кожи и одежды, атмосфера. + negative prompt'}",
  "video_emotion_arc": {
    "hook_en": "${remake_mode ? 'ИЗ ОРИГИНАЛА: первые 0.5-1с — кто что делает, какой хук. 40-60 слов EN' : '0.0-0.7s: ПОДРОБНО 60-80 слов EN: стоп-скролл визуальный хук. Персонаж A УЖЕ В ДВИЖЕНИИ. Опиши: что делает A (жест, взгляд в линзу, реквизит), что делает B (side-eye, микромимика), камера (micro push-in), звук (вдох/щелчок/шорох). No text on screen'}",
${soloMode ? `    "monologue_en": "0.7-7.0s: описание монолога на английском — один персонаж, прямо в камеру, killer word ≈ 6.8s",` : `    "act_A_en": "${remake_mode ? 'ИЗ per_speaker_timing: кто говорит первым, audio sync текст, жесты, мимика. Кто МОЛЧИТ и что делает. 60-80 слов EN' : '0.7-3.5s: ПОДРОБНО 80-100 слов EN: A говорит провокацию. Опиши: артикуляция A (жесты руками, наклон корпуса, мимика на ключевых словах), B МОЛЧИТ рот СТРОГО закрыт (только: моргание каждые 4-6с, side-eye, nostril flare, eyebrow shift, finger tap). Камера: micro-movement. Дыхание, вдох перед фразой. No text on screen'}",
    "act_B_en": "${remake_mode ? 'ИЗ per_speaker_timing: кто говорит вторым, audio sync текст с killer word, жесты. Кто МОЛЧИТ. Если 3+ говорящих — каждого с таймкодами. 60-80 слов EN' : '3.5-7.0s: ПОДРОБНО 80-100 слов EN: B отвечает панчлайном. 0.25с ПАУЗА перед ответом (tension beat — B expression меняется: loaded micro-smirk). B: голос снижается перед killer word → 0.2с пауза → взрыв артикуляции → jaw snap → глаза сужаются. A МОЛЧИТ рот СТРОГО закрыт: mid-gesture freeze → глаза расширяются. KW≈6.8s. Camera: micro push-in на killer word. No text on screen'}",`}
    "release_en": "${!enableLaughter ? '7.0-8.0s: БЕЗ СМЕХА. Тишина. Замершие лица. Rewatch-bait 0.3s' : remake_mode ? 'ИЗ ОРИГИНАЛА: финал — физика смеха КАЖДОГО / тишина. Camera shake/static. 40-60 слов EN' : scene_hint ? 'ФИНАЛ СОГЛАСНО ИНСТРУКЦИИ ПОЛЬЗОВАТЕЛЯ (см. выше). Rewatch-bait 0.3s' : 'ПОДРОБНО 60-80 слов EN: АБСОЛЮТНЫЙ НОЛЬ СЛОВ. Только натуральный заразительный ржач. Опиши физику: трясущиеся плечи, запрокинутая голова, слёзы, хриплый звук. Камера трясётся от смеха. 7.7-8.0с: rewatch-bait micro-expression. LOOP LOCK: финальная поза совместима с кадром 0. No text on screen'}"
  },
  "video_atmosphere_en": "${remake_mode ? 'КОПИЯ атмосферы оригинала на EN: звуки локации, реверб, room tone, шорохи. Свет из lighting_exact. RAW: ISO шум, пересветы, micro-jitter. 80-120 слов EN' : '120-180 слов на английском. ПОДРОБНО: конкретные звуки локации (минимум 3), реверб, room tone, микрофоника телефона, дыхание, шорохи одежды, клики слюны. Свет: направление, температура, тени. RAW-эстетика: ISO шум, пересветы, JPEG-компрессия. Камера: micro-jitter, breathing oscillation, OIS/EIS. Частицы: пыль/пар в свете'}",
  "end_frame_en": "${remake_mode ? 'ФИНАЛЬНЫЙ КАДР оригинала на EN, 100-150 слов. КАЖДЫЙ человек: поза, выражение лица. Если смех — физика: плечи, голова, рот, слёзы. Фон = location_exact. Свет = lighting_exact. Camera shake если с рук. NO TEXT. 8k. --ar 9:16' : 'ПОДРОБНЫЙ промпт финального кадра (7.5-8.0с) на английском, 150-200 слов. Точное описание: физика смеха (трясущиеся плечи, запрокинутая голова, слёзы на глазах, открытый рот), позы обоих персонажей, камера (shake от смеха, slight defocus), свет на лицах в момент смеха, микродетали (морщины от улыбки, блестящие глаза, пот). Rewatch-bait: неоднозначное micro-expression в последние 0.3с. LOOP LOCK: финальная поза совместима с кадром 0 для авто-лупа. Negative: no text, no overlays'}",
  ...
  "remake_veo_prompt_en": "[1:1 COPY — ALL DATA FROM original_video_analysis]\n\n[BLOCK 1 — SCENE (60-80 words EN): location_exact + props_in_hands + text_on_objects (if newspaper says 'JOY CASINO' — INCLUDE IT). lighting_exact. 2-3 ambient sounds.]\n\n[BLOCK 2 — CAMERA (30-40 words EN): num_people -> composition (Solo/Two-Shot/Three-Shot). camera_type+framing+angle+movement. Handheld micro-jitter if from hands. RAW phone: ISO 800-1600, JPEG artifacts, blown highlights.]\n\n[BLOCK 3 — ALL CHARACTERS (80-150 words EN): ${isVideoPlaceholder ? 'From people_description: EACH person separately — age, face, hair, clothing (exact color+material), accessories. body_positions (sitting/standing/leaning). props_in_hands.' : 'Replace appearance with Identity Lock (character_en + wardrobe_anchor). Keep body_positions and props_in_hands from original.'} Hyper-realism: pores, wrinkles, age spots. 8k.]\n\n[BLOCK 4 — PER-SECOND ACTION (150-250 words EN — MAIN BLOCK): From per_speaker_timing + action_timeline. Format:\n**[X.X-Y.Ys]:** WHO speaks (audio sync: 'exact Russian text'), gestures, expressions. WHO is STRICTLY SILENT (sealed lips) and what they do.\nRULES: 1) ONE speaker at a time, others SILENT. 2) 100% lip-sync. 3) Timecodes = real duration. 4) Every gesture from action_timeline.]\n\n[BLOCK 5 — ENDING (40-60 words EN): Final moment from action_timeline. If laugh — physics: shaking shoulders, head back, tears. If silence — frozen faces. Camera shake if handheld. Rewatch-bait 0.3s.]\n\n[BLOCK 6 — TECH (30-40 words EN): 9:16. Duration = real from per_speaker_timing. RAW smartphone, ISO 800-1600, JPEG artifacts. NEGATIVE: NO text/subtitles/watermarks/UI/smooth skin/studio lighting/CGI.]"
...
═══ ПРИОРИТЕТ №3: ПОКАДРОВЫЙ АНАЛИЗ ═══
1) СЦЕНА: конкретное место (не "barn" а "сруб с голубыми наличниками, лавка, штакетник").
2) ПЕРСОНАЖИ: КАЖДОГО отдельно — возраст, одежда (цвет+материал), причёска, аксессуары, поза, позиция в кадре.
3) ПРЕДМЕТЫ: что держит каждый? Текст на предметах (если "JOY CASINO" — пиши!).
4) КАМЕРА: ракурс, крупность, статика/движение.
5) ДЕЙСТВИЯ: что делает каждый посекундно.
6) ИЗОЛЯЦИЯ РЕЧИ: КТО говорит с X по Y секунду. Остальные МОЛЧАТ.
7) МИМИКА: выражения лиц каждого по моментам.
8) СВЕТ: направление, тип, температура, тени.
9) ЗВУК: фоновые шумы.
10) ТЕМП: длительность реплик, паузы, момент killer word.
11) ЧИСЛО ЛЮДЕЙ: сколько? Кто говорит? Кто молчит?
12) ФИНАЛ: смех/тишина? Поза каждого в последнем кадре.
