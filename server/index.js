...
${input_mode === 'suggested' ? `РЕЖИМ «СЮРПРИЗ» — СОЗДАЙ ВИРУСНЫЙ ШЕДЕВР 🎬

ТЫ НЕ ПРОСТО ВЫБИРАЕШЬ ТЕМУ. Ты создаёшь МИНИ-ФИЛЬМ за 8 секунд — дерзкий, стильный, визуально дорогой, с диалогом который хочется переслать другу с фразой «ахах, это жёстко».

═══ АЛГОРИТМ СОЗДАНИЯ (выполняй ПОСЛЕДОВАТЕЛЬНО) ═══

ШАГ 1 — ПРИДУМАЙ ВАУ-КАДР:
Начни НЕ с темы, а с ВИЗУАЛЬНОГО ОБРАЗА. Какой стартовый кадр заставит человека остановить скролл?
Примеры сильных кадров:
• Персонаж A держит в руке что-то абсурдное (квитанцию 3 метра длиной, пустой кошелёк с молью, GPS-трекер на кошке)
• Контраст в кадре (роскошный фон + убитая сумка, модная одежда + авоська с картошкой)
• Персонаж застигнут за чем-то (прячет чек от жены, гуглит «как вернуть молодость», разговаривает с холодильником)
• Неожиданный реквизит в руках (золотая лопата, кредитный договор как свиток, калькулятор размером с книгу)
ЗАПРЕЩЕНО: скучные нейтральные кадры где просто два человека стоят и разговаривают. Кадр ОБЯЗАН иметь визуальный крючок!

ШАГ 2 — ВЫБЕРИ МЕХАНИКУ ШУТКИ (используй РАЗНЫЕ, не повторяй!):
ОБЯЗАТЕЛЬНО используй одну из этих механик:
🔄 РАЗВОРОТ-180: A уверен в одном → B показывает что реальность РОВНО НАОБОРОТ ("Экономлю на такси!" → "Ты за неделю на кофе потратил больше чем я на бензин за месяц")
📈 ЭСКАЛАЦИЯ В АБСУРД: начинается с бытового → раздувается до космического масштаба ("Молоко подорожало" → "Скоро корову по ипотеке брать будем")
🪞 ЗЕРКАЛО: B показывает A его же поведение с другой стороны ("Ты весь день в телефоне!" → "Я на маркетплейсе покупки ищу. А ты три часа чужую жизнь в stories смотришь")
💣 САМОПОДРЫВ: B случайно выдаёт про себя что-то ХУЖЕ чем обвинение A ("Ты опять деньги потратила?!" → "Не потратила | вложила... в лотерейные билеты")
🎯 БЫТОВОЙ СНАЙПЕР: ультра-конкретная деталь которую узнает 80% зрителей ("Три раза переспросил цену и всё равно купил", "Открыл холодильник пятый раз надеясь что еда появится")
🔥 ГОРЬКАЯ ПРАВДА: B говорит то, о чём все думают но никто не произносит вслух
⏰ НОСТАЛЬГИЧЕСКИЙ КОНТРАСТ: сравнение «раньше vs сейчас» с неожиданным выводом
🎪 АБСУРДНАЯ ЛОГИКА: довести обычный аргумент до безумного вывода

ШАГ 3 — НАЙДИ ТЕМУ-ТРИГГЕР (болезненную для РФ/СНГ):
Тема должна быть НАСТОЛЬКО узнаваемой, что зритель через 0.5 секунды думает: «О, это точно про меня / мою маму / моего мужа»
ГОРЯЧИЕ ЗОНЫ 2026:
• ДЕНЬГИ И ЦЕНЫ: молоко за 800₽, скидка которая не скидка, «акция» = старая цена, кредитная карта как наркотик, доставка дороже заказа
• МАРКЕТПЛЕЙСЫ: Ozon прислал не то, 5 звёзд у мусора, возврат как квест, «похоже на фото» — нет, «бесплатная доставка» от 5000₽
• СЕМЬЯ И БЫТ: муж/жена и термостат, пакет с пакетами, «я только гляну» в магазине = 3 пакета, холодильник пустой но открываешь 10 раз
• ЖКХ И КВАРТИРА: квитанция-триллер, ремонт «на недельку» = полгода, сосед с перфоратором, управляйка-призрак
• ПОКОЛЕНИЯ: бабушка и нейросеть, дед и QR-код, «ок бумер» от внука, звонок vs голосовое
• ЗДОРОВЬЕ: очередь в поликлинику, врач гуглит при тебе, «попейте витаминчики», анализы на 20 страниц
• ТРАНСПОРТ: самокатчик-камикадзе, такси = пол-зарплаты, навигатор в тупик, парковка = квест
• НЕЙРОСЕТИ И ТЕХНО: AI советует бред, робот-пылесос съел носок, умная колонка подслушивает, 6 пальцев от нейросети
ЗАПРЕЩЕНО: политика, абстракции, темы старше года, узкоспециальное, скучное.

ШАГ 4 — НАПИШИ ШУТКУ КОТОРУЮ ХОЧЕТСЯ ПЕРЕСЛАТЬ:
Финальный тест: если диалог прочитать вслух другу — он ЗАСМЕЁТСЯ? Если нет — ПЕРЕДЕЛАЙ.
• Юмор ПРОСТОЙ, НАГЛЫЙ, ЖИВОЙ — понятный с первого раза
• Не умничай — лучше грубая правда чем элегантная пустота
• Фразы как ЦИТАТЫ — хочется запомнить и пересказать
• Killer word БЬЁТ как молот — без него фраза разваливается
• Финальный смех ЗАРАЗИТЕЛЬНЫЙ — зритель смеётся вместе с персонажами

ШАГ 5 — ВИЗУАЛЬНАЯ ПОДАЧА (дорого и стильно):
• Реквизит в руке A — ОБЯЗАТЕЛЬНО, это визуальный якорь и завязка сюжета
• Локация — не просто фон, а ЧАСТЬ ИСТОРИИ (если про деньги — фон с ценниками; если про ЖКХ — видна батарея/трубы)
• Освещение создаёт НАСТРОЕНИЕ (не просто «дневной свет»)
• Одежда и аксессуары персонажей контрастируют и создают химию
• Финальный кадр с совместным смехом — ТЁПЛЫЙ, ЖИВОЙ, заразительный

АНТИПОВТОР: Если в thread_memory есть предыдущие генерации — используй ДРУГУЮ механику шутки, ДРУГУЮ тему, ДРУГОЙ визуальный приём. Никогда не повторяй один и тот же паттерн.` : `СВОБОДНАЯ ГЕНЕРАЦИЯ — СОЗДАЙ ВИРУСНЫЙ ШЕДЕВР:

ТЫ НЕ ПРОСТО ПРИДУМЫВАЕШЬ ШУТКУ. Ты создаёшь мини-фильм за 8 секунд.

АЛГОРИТМ:
1. ПРИДУМАЙ ВАУ-КАДР — визуальный образ который остановит скролл (абсурдный реквизит, контраст, неожиданная ситуация)
2. ВЫБЕРИ МЕХАНИКУ ШУТКИ — разворот-180, эскалация в абсурд, зеркало, самоподрыв, бытовой снайпер, горькая правда
3. НАЙДИ ТЕМУ — болезненную, узнаваемую, бытовую ситуацию из жизни РФ/СНГ (цены, маркетплейсы, семья, ЖКХ, поколения, здоровье, транспорт, нейросети)
4. НАПИШИ ДИАЛОГ — простой, наглый, живой, с killer word который бьёт как молот
5. СДЕЛАЙ ВИЗУАЛ ДОРОГИМ — реквизит в руке, локация как часть истории, контраст, кинематографичный свет

Реакция зрителя: «ахах, это жёстко» + немедленная пересылка другу.
ЗАПРЕЩЕНО: политика, абстракции, скучные нейтральные кадры, предсказуемые шутки, заумные слова.`}
