...
${nicie === 'tech' ? `• «ChatGPT написал код — он работает, но никто не знает как» (AI-абсурд)
...
${nicie === 'finance' ? `• «Положил миллион на вклад под 18% — купил мороженое» (инвестиции)
...
${nicie === 'education' ? `• «Сдал ЕГЭ на 100 баллов — поступил, но специальность не нравится» (экзамены)
...
${nicie === 'relationships' ? `• «Он написал "Привет" — я уже придумала имена нашим детям» (dating apps)
...
${nicie === 'travel' ? `• «Билеты по акции за 5000 — с багажом и едой вышло 25000» (авиабилеты)
...
${nicie === 'food' ? `• «Готовила по рецепту — получилось совсем не как на картинке» (рецепты)
...
${nicie === 'parenting' ? `• «Купил развивающие игрушки на 20000 — ребёнок играет коробкой» (игрушки)
...
${nicie === 'realestate' ? `• «Ипотека под 6% — через год ставка 16%, платёж вырос в 2 раза» (ипотека)
