...
□ killer_word создаёт эффект? (если убрать — монолог теряет удар)` : `□ dialogue_A_ru содержит 6-15 слов? (посчитай!)
...
${soloMode ? `    "monologue_en": "0.6-7.3s: описание монолога на английском — один персонаж, прямо в камеру, killer word ≈ 7.1s",` : `    "act_A_en": "0.6-3.8s: описание на английском",
  ...
  "remake_veo_prompt_en": "ПОЛНЫЙ ПРОМПТ ДЛЯ VEO, КОПИРУЮЩИЙ ОРИГИНАЛЬНОЕ ВИДЕО. Опиши: ту же сцену (локация, фон, атмосфера), то же действие (движения, жесты, позы), тот же ракурс камеры, то же освещение — но с НАШИМИ персонажами. Это НЕ диалоговый промпт — это ТОЧНОЕ описание того что происходит визуально в оригинальном видео, адаптированное под наших персонажей. 200-400 слов на английском. Если в оригинале спорт/танец/действие — описывай ИМЕННО это действие."` : ''}${product_info?.description_en || ctx.hasProductImage ? `,
...
• Пользователь выбирает режим → пишет идею/тему → выбирает персонажей → Studio генерирует полный пакет (промпт для Veo, диалог, хештеги, описание)
• Потом пользователь копирует промпт → вставляет в Google Flow (Veo) → получает готовое видео → публикует в Instagram
• Весь процесс: 2 мин в Studio + 5 мин генерация + 3 мин публикация = 10 минут на ролик
• Себестоимость одного ролика ~3,6 ₽
• 200+ готовых персонажей + КОНСТРУКТОР СВОИХ персонажей (можно создать уникального с нуля — задаёшь внешность, характер, стиль речи, и он получает Identity Lock)
• 144+ готовых локаций + КОНСТРУКТОР СВОИХ локаций (можно описать свою — магазин, офис, кафе — и AI встроит её в промпт)
...
• 4 режима генерации: своя идея, готовые идеи из трендов, свой диалог, ремейк по видео (загрузка MP4 — AI анализирует и пересоздаёт)
...
• 200+ AI-персонажей с Identity Lock (75 параметров) + КОНСТРУКТОР своих персонажей
• 144+ локаций + КОНСТРУКТОР своих локаций (можно описать свой магазин, офис, кафе)
• 120 вирусных пресетов «Сюрприз» — уникальный промпт каждый клик, никогда не повторяется
• Загрузка фото товара в кадр + референс стиля/фона (загрузи фото своего помещения)
...
• Показать масштаб возможностей: 200+ персонажей + конструктор своих, 144+ локаций + конструктор своих, 4 режима, 120 пресетов, загрузка фото товара + референс стиля
