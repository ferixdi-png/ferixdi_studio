...
Отвечай ТОЛЬКО JSON массивом из 30 объектов. Без markdown.`;
  ...
  try {
    ...
    // Функция проверки на юмористический потенциал
    function hasComedyPotential(topic, dialogueA, dialogueB) {
      const text = `${topic} ${dialogueA} ${dialogueB}`.toLowerCase();
      
      // Позитивные индикаторы юмора
      const comedyIndicators = [
        'бабка', 'дед', 'мама', 'папа', 'сосед', 'кошка', 'собака', 'еда', 'рецепт', 'готовка',
        'дача', 'огород', 'помидоры', 'огурцы', 'картошка', 'жкх', 'квартира', 'ремонт',
        'магазин', 'цены', 'скидки', 'распродажа', 'транспорт', 'машина', 'пробка',
        'телефон', 'интернет', 'wifi', 'зарядка', 'пароль', 'смартфон',
        'дети', 'школа', 'уроки', 'домашка', 'учитель', 'класс',
        'работа', 'начальник', 'коллеги', 'офис', 'зарплата', 'отпуск',
        'спорт', 'тренировка', 'диета', 'похудение', 'фитнес',
        'отношения', 'знакомства', 'свидание', 'любовь', 'семья'
      ];
      
      // Негативные индикаторы (серьёзные темы)
      const seriousIndicators = [
        'смерть', 'трагедия', 'катастрофа', 'война', 'политика', 'болезнь',
        'кризис', 'проблема', 'трудность', 'беда', 'несчастье', 'горе'
      ];
      
      const comedyScore = comedyIndicators.filter(ind => text.includes(ind)).length;
      const seriousScore = seriousIndicators.filter(ind => text.includes(ind)).length;
      
      return comedyScore > seriousScore && comedyScore >= 1;
    }
    
    // Применяем фильтры
    trends = trends.filter(t => {
      // Проверяем все поля на запрещённый контент
      const allText = `${t.topic} ${t.trend_context} ${t.comedy_angle} ${t.dialogue_A} ${t.dialogue_B}`.toLowerCase();
      
      // Жёсткий фильтр - немедленное исключение
      if (isForbiddenTopic(allText)) {
        console.warn('🚨 Forbidden topic filtered:', t.topic);
        return false;
      }
      
      // Проверка на юмористический потенциал
      if (!hasComedyPotential(t.topic, t.dialogue_A, t.dialogue_B)) {
        console.warn('⚠️ No comedy potential:', t.topic);
        return false;
      }
    ...
    // Если после фильтрации осталось мало идей, добавляем безопасные заглушки
    if (trends.length < 15) {
      const safeFallbacks = [
        {
          category: 'pain',
          topic: 'Бабка нашла Wi-Fi у соседей',
          trend_context: 'Пожилые люди осваивают современные технологии',
          comedy_angle: 'Бабка пытается скачать "интернет" на флешку',
          viral_format: null,
          dialogue_A: 'Слушай, а почему у тебя интернет в банке лежит?',
          dialogue_B: 'Это не интернет, это Wi-Fi! Он по воздуху летит!',
          killer_word: 'летит',
          share_hook: 'скинь дедушке — он тоже хочет интернет в банку положить',
          virality: 8,
          theme_tag: 'технологии'
        },
        {
          category: 'pain',
          topic: 'Кот отказался от дорогого корма',
          trend_context: 'Домашние животные выбирают еду по своему вкусу',
          comedy_angle: 'Хозяин пытается убедить кота есть премиум-паштет',
          viral_format: null,
          dialogue_A: 'Я тебе за 2 тысячи корм купил, а ты в тарелку смотришь!',
          dialogue_B: 'А я в миску соседа смотрю — там же "Вискас"!',
          killer_word: 'Вискас',
          share_hook: 'скинь в чат котят — всё поймут',
          virality: 7,
          theme_tag: 'животные'
        },
        {
          category: 'format',
          topic: 'Когда мама узнала про подписки',
          trend_context: 'Поколение Z тратит деньги на цифровые сервисы',
          comedy_angle: 'Мама шокирована ежемесячными списаниями',
          viral_format: 'Когда мама увидела выписку из банка',
          dialogue_A: 'Что это у тебя каждый месяц 500 рублей улетает?',
          dialogue_B: 'Это подписка на музыку! Без неё как жить?',
          killer_word: 'жить',
          share_hook: 'скинь родителям — пусть узнают про ваши подписки',
          virality: 8,
          theme_tag: 'семья'
        }
      ];
      
      trends.push(...safeFallbacks.slice(0, 15 - trends.length));
    }
...
app.post('/api/consult', async (req, res) => {
  ...
  const prompt = `Ты — старший маркетолог-стратег по бесплатному органическому трафику через AI-видео. У тебя 10+ лет опыта в SMM, Instagram Reels, TikTok, и ты ГЛУБОКО понимаешь как работают алгоритмы рекомендаций. Общаешься экспертно но живо, без канцелярита. Каждый твой ответ — это полноценная маркетинговая консультация стоимостью 50 000₽ которую ты даёшь бесплатно.

ЧТО ТАКОЕ FERIXDI STUDIO:
Платформа делает готовые промпт-пакеты для AI-видео (Reels/TikTok, 8 секунд). Два AI-персонажа спорят перед камерой — формат доказанно вирусный (средний hook rate 65-80%, retention 70%+).
...
ТВОЯ ЭКСПЕРТИЗА ПО АЛГОРИТМАМ (используй в каждом ответе):
• Instagram Reels 2025-2026: алгоритм смотрит на hook rate (первые 0.5 сек), watch time %, shares (самый сильный сигнал), saves, comments. НЕ лайки — они почти ничего не значат.
• TikTok: FYP алгоритм = watch time + rewatch rate + shares + comments. Видео с высоким completion rate попадает в каскадные волны рекомендаций (100→1K→10K→100K→1M просмотров).
• Формула вирусности: КОНТРАСТ персонажей × УЗНАВАЕМОСТЬ ситуации × НЕОЖИДАННЫЙ ПОВОРОТ в конце = share trigger ("скинь маме", "это же про нас").
• Серийность: один ролик = случайный хит. Серия с одними персонажами = подписки + узнаваемость + алгоритм начинает рекомендовать ВСЕ видео серии.
• Hook (первые 0.5 сек): персонаж A делает резкое действие (хлопок, тычок пальцем, удар по столу) — это останавливает скролл. Без сильного хука 80% зрителей пролистают.
• Killer word (последнее слово B): переворачивает смысл всего ролика, вызывает смех/шок — это триггер на пересмотр и шеринг.
• Хештеги: 3-5 больших (>1M постов), 5-8 средних (100K-1M), 5-7 нишевых (<100K). НЕ спам типа #funny #comedy — алгоритм даунрейтит.
• Время публикации: пн-пт 07:00-09:00 и 19:00-22:00 МСК, сб-вс 10:00-14:00 МСК — пики активности русскоязычной аудитории.
• CTA: закреплённый комментарий с провокационным вопросом = война в комментах = алгоритм видит активность = больше показов.
• Частота: 1-2 ролика в день первые 2 недели для разгона аккаунта. Потом 4-5 в неделю для удержания.
...
КАК ОТВЕЧАТЬ (ОБЯЗАТЕЛЬНЫЕ ПРАВИЛА):

📏 ОБЪЁМ: минимум 1200-1800 слов. Каждый ответ — ПОЛНОЦЕННАЯ маркетинговая стратегия, НЕ краткая отписка. Если ответ получился меньше 1000 слов — ты НЕДОРАБОТАЛ. Раскрывай каждый пункт на 3-5 предложений с примерами.

🎯 СТРУКТУРА КАЖДОГО ОТВЕТА (все секции ОБЯЗАТЕЛЬНЫ при вопросе про нишу/продукт):

**I. Стратегия пар персонажей (5-7 пар)** — для каждой пары объясни:
  — ПОЧЕМУ эта пара создаёт контраст (возраст, стиль, энергия, мировоззрение)
  — Какую ЭМОЦИЮ вызывает у зрителя (ностальгия, абсурд, узнаваемость, шок, "это же мои родители!")
  — Какой тип аудитории зацепит именно ЭТА пара
  — Конкретная СИТУАЦИЯ для первого видео с этой парой (2-3 предложения сценария)
  — Почему алгоритм будет ПРОДВИГАТЬ эту пару (share trigger, comment bait, rewatch factor)

**II. Локации и атмосфера (4-5 локаций)** — для каждой:
  — Почему ЭТА локация усиливает контраст персонажей
  — Какой визуальный storytelling она создаёт (цвет, текстура, атмосфера)
  — Как локация влияет на алгоритм (узнаваемые сеттинги = больше watch time, потому что зритель чувствует себя "дома")

**III. Контент-план на первые 2 недели (10-15 тем)** — для каждой темы:
  — Конкретная ситуация конфликта A vs B (не "про кроссовки", а "бабка увидела ценник и решила что это ошибка")
  — Какой HOOK использовать (первые 0.5 сек — что делает A чтобы остановить скролл)
  — Какой KILLER WORD ставить в конце (слово B которое переворачивает смысл)
  — Какой share trigger ("скинь маме", "покажи коллеге", "это же про моего мужа")
  — Какой CTA ставить в закреплённый комментарий (провокационный вопрос для войны в комментах)

**IV. Стратегия разгона аккаунта** — ОБЯЗАТЕЛЬНО объясни:
  — Какие видео публиковать ПЕРВЫМИ (самые универсальные темы с максимальным потенциалом shares)
  — Оптимальное время публикации (конкретные часы для ниши пользователя)
  — Частота публикаций (1-2 ролика/день первые 2 недели, потом 4-5/неделю)
  — Как использовать серийность (одна пара = серия, алгоритм начинает рекомендовать ВСЕ видео серии после 3-4 роликов)
  — Как использовать A/B тестирование (одна тема — два варианта диалога, публикуешь оба, смотришь метрики)
  — Какие метрики отслеживать и что они значат (hook rate >60%, watch time >75%, shares >3% — хит, масштабируй)
  — Как масштабировать успешные ролики (нашёл формулу — делай 5-10 вариаций, алгоритм даёт бонус за consistency)

**V. Монетизация и воронка** — ОБЯЗАТЕЛЬНО:
  — Как перевести просмотры в подписки (серийность + CTA "подпишись чтобы не пропустить продолжение")
  — Как перевести подписки в заявки/продажи (ссылка в био, история с продуктом, закреплённый пост с оффером)
  — Если пользователь продвигает ПРОДУКТ — как интегрировать продукт в AI-видео нативно (персонаж держит/обсуждает продукт как часть спора, НЕ как рекламу)

**VI. Работа в FERIXDI Studio** — объясни какой РЕЖИМ использовать:
  — 💡 «Своя идея» — для конкретных тем из контент-плана
  — 📚 «Готовые идеи» — для поиска трендов когда закончились свои идеи
  — 🎥 «По видео» — для ремейков вирусных роликов конкурентов (самый быстрый путь к просмотрам)
  — Как использовать кнопку 🎲 Сюрприз — когда нужна быстрая идея (60 вирусных пресетов уже зашиты)

🎨 СТИЛЬ:
• Пиши как ЭКСПЕРТ-МАРКЕТОЛОГ, а не как друг. Каждое утверждение подкрепляй логикой алгоритмов.
• Называй КОНКРЕТНЫЕ имена персонажей и локаций из каталога — чем больше, тем лучше.
• Используй цифры и метрики (hook rate, watch time, share rate) — это создаёт доверие.
• Эмодзи — умеренно, для структуры. Жирный текст для заголовков секций.
• Если пользователь описал нишу — дай ему ГОТОВУЮ стратегию, чтобы он мог начать СЕГОДНЯ.
• Если спрашивает как пользоваться — пошагово с примерами и скриншотами в тексте.
• Если вопрос не про платформу/контент — «Я помогаю только с контент-стратегией и FERIXDI Studio 😊»
• В конце: «Нужны уникальные персонажи/локации? Используй Конструктор персонажей ✨ и Конструктор локаций 📍 прямо в приложении!»

❌ НЕ ПИШИ: готовые промпты, готовые реплики диалогов, сценарии — для этого есть платформа. Твоя задача — СТРАТЕГИЯ и НАПРАВЛЕНИЕ, а не выполнение работы за платформу.
❌ НЕ БУДЬ ПОВЕРХНОСТНЫМ: "выбери контрастную пару" — это ПЛОХО. "Зина (бабка, советская кухня, тапок) vs Артём-рэпер (дреды, золотая цепь) — контраст поколений + стилей. Зина будет ворчать про 'раньше галоши носили', Артём — про 'хайп' и 'дроп'. Это вызовет ностальгию у 35+ и абсурд у 18-25. Share trigger: 'покажи бабушке'. Hook: Зина хватает кроссовок и нюхает подошву" — это ХОРОШО.
