...
function buildAIPrompt(ctx) {
  ...
  // Video mode with placeholder: Gemini must analyze original video for characters
  const isVideoPlaceholder = input_mode === 'video' && (charA?.id === 'video_original' || !charA?.prompt_tokens?.character_en);
...
Пользователь хочет УНИКАЛИЗИРОВАТЬ видео — результат должен быть ТОЧНОЙ ПОКАДРОВОЙ КОПИЕЙ оригинала. Та же речь, те же действия, та же камера — но уникальное видео.
${!ctx.charA?.id || ctx.charA?.id === 'video_original' ? `
⚠️ РЕЖИМ ПРЯМОЙ КОПИИ (CHARACTER LOCK ОТКЛЮЧЕН):
Пользователь НЕ выбрал персонажа из базы. Ты должен ПОЛНОСТЬЮ ОПИСАТЬ людей из оригинального видео:
1. ОПИШИ КАЖДОГО человека из видео МАКСИМАЛЬНО ДЕТАЛЬНО: возраст, пол, внешность, цвет волос, цвет глаз, тип кожи, телосложение, черты лица
2. ОПИШИ одежду ТОЧНО как в видео: цвет, материал, фасон, аксессуары, украшения
3. ОПИШИ локацию ТОЧНО как в видео: место, освещение, предметы, атмосфера
4. В photo_scene_en ИСПОЛЬЗУЙ СВОЙ анализ оригинала — НЕ шаблонные данные персонажа. Если в видео молодая девушка — описывай МОЛОДУЮ ДЕВУШКУ. Если мужчина — МУЖЧИНУ.
5. ЗАПРЕЩЕНО: менять молодую девушку на бабушку, мужчину на женщину, молодого на старого
6. ЗАПРЕЩЕНО: использовать слова "elderly", "бабушка", "пожилой" если в видео молодой человек
7. ЗАПРЕЩЕНО: копировать character_en или wardrobe_anchor из секции Identity Lock (они пустые)
8. ИГНОРИРУЙ секцию CHARACTER IDENTITY LOCK ниже — она не содержит данных в этом режиме
9. Результат = ТОЧНАЯ КОПИЯ оригинала: та же энергия, те же люди, та же сцена, те же действия` : ''}
${video_meta ? `
ОРИГИНАЛ ВИДЕО:
...
Результат должен быть ТОЧНОЙ ПОКАДРОВОЙ КОПИЕЙ оригинала. Каждое слово, каждое действие, каждый ракурс — ОДИН В ОДИН.
...
⚠️ photo_scene_en ОБЯЗАН описывать ТУ ЖЕ сцену, композицию и действие что в оригинале${isVideoPlaceholder ? ' — описывай людей/одежду/локацию ТОЧНО как в оригинале' : ' — с персонажами из Identity Lock в ИХ фирменной одежде'}.
⚠️ remake_veo_prompt_en ОБЯЗАН описывать ТО ЖЕ действие/движение что в оригинале${isVideoPlaceholder ? '' : ' — с нашими персонажами'}.
...
⚠️ Если в оригинале один человек делает действие (спорт/танец) — описывай ТО ЖЕ действие${isVideoPlaceholder ? '' : ' с нашим персонажем'}.
${isVideoPlaceholder ? '⚠️ ОДЕЖДА: описывай одежду людей из оригинала ТОЧНО как есть — НЕ ЗАМЕНЯЙ.' : '⚠️ ОДЕЖДА: персонажи ВСЕГДА в своей фирменной одежде из Identity Lock — ЗАПРЕЩЕНО копировать одежду людей из оригинала.'}
⚠️ ВИРУСНЫЙ ПОТЕНЦИАЛ: определи что именно сделало оригинал вирусным (неожиданный поворот / абсурдная ситуация / узнаваемая жиза / идеальный тайминг) и СОХРАНИ это в ремейке.` : `ШАГ 1: ОПРЕДЕЛИ ТИП — диалог/скит (есть речь) или визуальное/действие (нет речи).
...
ДЛЯ ОБОИХ ТИПОВ: проанализируй обложку визуально (сцена, композиция, ракурс, действия, движения, свет) и воспроизведи в photo_scene_en и remake_veo_prompt_en${isVideoPlaceholder ? ' — описывай людей ТОЧНО как в оригинале' : ' с нашими персонажами'}.`}
...
• ${isVideoPlaceholder ? 'Как сохранить оригинальную харизму людей из видео?' : 'Как НАШИ персонажи впишутся в этот формат? (их специфический стиль речи / внешность / характер)'}
• Что нужно ТОЧНО сохранить из оригинала чтобы не потерять вирусность?
Используй ответы для максимально точного remake — ТОЧНАЯ КОПИЯ оригинала.`;
...
${input_mode === 'suggested' ? `РЕЖИМ «СЮРПРИЗ» — PRODUCTION-READY ВИРУСНЫЙ РОЛИК ДЛЯ VEO 3.1 🎬

Формат: 8-секундное вирусное диалоговое видео под РФ/СНГ для Veo 3.1.
Твоя задача — не просто придумать шутку, а создать ПОЛНОСТЬЮ ПРОРАБОТАННЫЙ, ДЛИННЫЙ, ДЕТАЛИЗИРОВАННЫЙ, ТЕХНИЧЕСКИ ТОЧНЫЙ, УНИКАЛЬНЫЙ сценарий, который можно ПОЧТИ БЕЗ ДОРАБОТОК брать в генерацию видео.

════ АБСОЛЮТНЫЕ ПРИОРИТЕТЫ ════
№1 — ТРЕНД ИМЕННО ТЕКУЩЕГО ДНЯ. Ролик = свежий кусок сегодняшней ленты.
№2 — ШУТКА ИМЕННО ТЕКУЩЕГО ДНЯ. Не «вне времени», а та что звучит свежо СЕЙЧАС.
№3 — РЕАЛИСТИЧНОСТЬ сцены, поведения, мимики, света, камеры и эмоций.
№4 — НЕБАНАЛЬНЫЙ, ВИЗУАЛЬНО РЕДКИЙ КАДР И СХЕМА СЪЁМКИ — чтобы ролик не выглядел как типовой AI-шаблон.
№5 — ОЧЕНЬ ПОДРОБНЫЕ, ДЛИННЫЕ, НАСЫЩЕННЫЕ ДЕТАЛЯМИ промпты по каждому полю — photo_scene_en, video_emotion_arc, video_atmosphere_en, end_frame_en.

НЕЛЬЗЯ ХАЛТУРИТЬ. Нельзя писать коротко. Нельзя давать поверхностные формулировки вроде «красивый кадр», «кинематографично», «мужчина смотрит», «девушка смеётся». Всё должно быть расписано предметно, визуально, технически.

ТЫ ОБЯЗАН строить ролик ОТ ШУТКИ, не от картинки, не от люкса, не от пафоса.

════ ШАГ 0 — ОПРЕДЕЛИ ТРЕНД СЕГОДНЯШНЕГО ДНЯ ════
Перед генерацией ОБЯЗАТЕЛЬНО определи:
• Какой мемный вайб актуален ИМЕННО СЕГОДНЯ?
• Какой тип шутки и конфликта люди реально пересылают СЕГОДНЯ?
• Какая интонация и механика панчлайна звучит по-современному?
• Какой ритм фраз похож на живую сегодняшнюю ленту, а не на старый анекдот?
Используй свои знания о трендах 2025-2026 в РФ/СНГ мемах, Reels, TikTok, Threads.
Если шутку можно было выпустить полгода назад и сегодня — В НЕЙ МАЛО ТРЕНДА. ПЕРЕДЕЛАЙ.

════ ШАГ 1 — НАПИШИ ДИАЛОГ ════

ЖЕЛЕЗНЫЕ ПРАВИЛА:
1. ТОЛЬКО ДИАЛОГ. Не монолог. Не narration. Только живая короткая сценка.
2. ВСЯ РЕЧЬ — СТРОГО 0.0–7.0с. Никакая реплика НЕ залезает на 8-ю секунду.
3. ПОСЛЕДНЯЯ СЕКУНДА (7.0–8.0с) — ТОЛЬКО натуральный заразительный смех. Громкий ржач, ощущение что реально порвало. НОЛЬ слов.
4. В САМОМ НАЧАЛЕ — РЕЗКИЙ МАТ-ХУК. Первая реплика (0.0–1.5с) — мощный дерзкий словесный удар с матом/жёсткой лексикой (по swear_level). Мгновенно тормозит ленту и создаёт конфликт.
5. ШУТКА ПОНЯТНА С ПЕРВОГО ПРОСЛУШИВАНИЯ. Без заумных слов, без объяснений.
6. KILLER WORD — короткий, убойный, цитируемый, мемный, пересылаемый.

КАРКАС: резкий мат-хук → короткий обмен → убойная добивка → 1 секунда заразительного смеха

════ ШАГ 2 — ПРИДУМАЙ НЕБАНАЛЬНУЮ СХЕМУ КАДРА ════

⚠️ АНТИШАБЛОН: НЕ ИСПОЛЬЗУЙ банальные AI-схемы:
• НЕ обычный фронтальный two-shot (два человека в центре кадра)
• НЕ типовой люкс-фон (белый интерьер, ресторан, зеркала)
• НЕ скучный симметричный крупняк
• НЕ стандартную постановку «стоят и говорят»
• НЕ кадр «мужчина + женщина + симметрия + пафос»

ВМЕСТО ЭТОГО ищи ЖИВЫЕ, РЕАЛИСТИЧНЫЕ, НЕБАНАЛЬНЫЕ ракурсы:
...
• Dashboard POV / passenger POV / backseat POV (для авто-сцен)
• Shoulder POV — камера из-за плеча одного персонажа
• Off-balance framing — персонажи не по центру, а чуть сдвинуты
• Partial obstruction — часть кадра перекрыта объектом (дверной косяк, столб, спинка стула)
• Тесная точка съёмки — кухня 6м², лифт, тамбур, прихожая
• Фон который РАБОТАЕТ НА ПРИКОЛ (ценники за спиной, развал на кухне, разбитый экран)

Небанальный кадр нужен не ради выёбона, а РАДИ УСИЛЕНИЯ ШУТКИ и живости.

════ ШАГ 3 — РЕАЛИЗМ (обязательный фильтр) ════

Видео должно выглядеть как РЕАЛЬНО СНЯТЫЙ, СЛУЧАЙНО ПОЙМАННЫЙ момент на смартфон:
• Физическая правдоподобность движений и поз
• Естественная мимика (не стеклянные лица)
...
• Правдоподобный средовой свет (не студийный)
• Ощущение присутствия — запись друга на телефон

ЗАПРЕЩЕНО В ПРОМПТАХ:
• Стеклянные/пластиковые лица
• Рекламная стерильность и глянцевость
• Слишком идеальные позы
• Мёртвые паузы без микродвижений
• Фальшивые модельные улыбки
• Одинаковые безжизненные лица

════ ШАГ 4 — ПИШИ МАКСИМАЛЬНО ПОДРОБНО ════

ТЫ ОБЯЗАН писать ОЧЕНЬ ДЛИННЫЕ и ДЕТАЛИЗИРОВАННЫЕ промпты:
• photo_scene_en — МИНИМУМ 250 слов, лучше 300+. Подробно: кожа, свет, ткань, руки, глаза, пропс, фон, воздух
• video_emotion_arc (каждый сегмент) — ПОДРОБНО: что делает говорящий, что делает молчащий, куда глаза, что камера, микромимика, дыхание
• video_atmosphere_en — МИНИМУМ 120 слов: конкретные звуки, реверб, room tone, микрофоника, ткань, дыхание
• end_frame_en — МИНИМУМ 150 слов: точное описание финального кадра со смехом, физика тела, камера, свет

Лучше слишком подробно, чем недокручено. Лучше длинный промпт с управляемым результатом, чем короткая абстракция.
НЕ ПИШИ общие слова — пиши КОНКРЕТИКУ: не «тёплый свет» а «single 3200K tungsten overhead 45° camera-left, warm falloff on cheekbones, deep amber shadow under jaw».

════ ГОРЯЧИЕ ЗОНЫ ТРЕНДА 2026 (РФ/СНГ) ════
• ДЕНЬГИ И ЦЕНЫ: «акция» = старая цена, доставка дороже заказа, кредитка как наркотик
• МАРКЕТПЛЕЙСЫ: Ozon/WB прислал не то, 5 звёзд у мусора, «бесплатная доставка» от 5000₽
• СЕМЬЯ И БЫТ: муж/жена и термостат, «я только гляну» = 3 пакета
• ЖКХ: квитанция-триллер, сосед с перфоратором, ремонт «на недельку» = полгода
• ПОКОЛЕНИЯ: бабушка и нейросеть, дед и QR-код
• ЗДОРОВЬЕ: очередь в поликлинику, врач гуглит при тебе
• ТРАНСПОРТ: такси = пол-зарплаты, самокатчик-камикадзе
• НЕЙРОСЕТИ: AI советует бред, робот-пылесос съел носок
ЗАПРЕЩЕНО: политика, абстракции, темы старше года, скучное, старые анекдоты.

════ ЧТО ЗАПРЕЩЕНО ════
• Старые анекдотные схемы и шутки «вне времени»
• Банальные бытовые диалоги без свежего разворота
• Длинные реплики не помещающиеся в 7 секунд
• Искусственные AI-фразы, литературная речь, заумные слова
• Скучные заходы без резкого старта, слабая концовка
• Формальный/фальшивый смех, красивая сцена без сильной шутки
• КОРОТКИЕ ПРОМПТЫ — каждое поле ОБЯЗАНО быть подробным и длинным
• БАНАЛЬНЫЕ СХЕМЫ КАДРА — фронтальный two-shot, симметрия, люкс-фон

════ ОБЯЗАТЕЛЬНАЯ САМОПРОВЕРКА ════
□ Тренд ТЕКУЩЕГО ДНЯ, не старая шутка?
□ Свежая актуальная механика, не шаблон?
□ Первая реплика с мат-хуком?
□ Вся речь в 7 секунд?
□ Последняя секунда только смех?
□ Панчлайн сильный, короткий, цитируемый?
□ Кадр НЕБАНАЛЬНЫЙ — не стандартный AI two-shot?
□ Промпты ДЛИННЫЕ и ПОДРОБНЫЕ (photo≥250 слов, atmosphere≥120)?
□ Сцена выглядит ЖИВОЙ — как снято на телефон, не AI-шаблон?
□ Диалог хочется переслать?
Если хотя бы один пункт слабый — ДОРАБАТЫВАЙ.

АНТИПОВТОР: Если в thread_memory есть предыдущие — используй ДРУГУЮ механику, тему, вайб, тип конфликта, ДРУГУЮ схему кадра. Никогда не повторяй паттерн.` : `СВОБОДНАЯ ГЕНЕРАЦИЯ — PRODUCTION-READY РОЛИК ДЛЯ VEO 3.1:

Формат: 8-секундное вирусное диалоговое видео под РФ/СНГ.
АБСОЛЮТНЫЙ ПРИОРИТЕТ: тренд и шутка ИМЕННО ТЕКУЩЕГО ДНЯ + МАКСИМАЛЬНО ПОДРОБНЫЕ промпты.

АЛГОРИТМ:
1. Определи мемный вайб, тип шутки и конфликта актуальный СЕГОДНЯ
2. Напиши диалог: резкий мат-хук → короткий обмен → убойная добивка. Речь строго 0–7с, смех 7–8с
3. Придумай НЕБАНАЛЬНУЮ схему кадра (не стандартный two-shot, а живой ракурс — через предмет, через отражение, shoulder POV, off-balance, partial obstruction)
4. Пиши ВСЕ промпты МАКСИМАЛЬНО ДЛИННО и ПОДРОБНО: photo≥250 слов, atmosphere≥120, каждый arc-сегмент с полной детализацией
5. Реализм: момент как будто случайно пойман на телефон, микрохаос, живые реакции

ЗАПРЕЩЕНО: старые анекдоты, короткие промпты, банальные AI-кадры, абстракции, слова в последней секунде.`}
...
${isVideoPlaceholder ? `
════════════════════════════════════════════════════════════════
VIDEO COPY MODE — CHARACTER PASSPORT ОТКЛЮЧЕН
Пользователь НЕ выбрал персонажа из базы. Ты ОБЯЗАН проанализировать оригинальное видео и описать КАЖДОГО человека:
1. ВНЕШНОСТЬ: возраст, пол, цвет волос, цвет глаз, тип кожи, форма лица, телосложение, рост
2. ОДЕЖДА: цвет, материал, фасон, аксессуары, украшения — ТОЧНО как в видео
3. ЛОКАЦИЯ: место, освещение, предметы, атмосфера — ТОЧНО как в видео
4. ДЕЙСТВИЯ: что делают люди, как двигаются, куда смотрят
5. ЗАПРЕЩЕНО: менять молодую девушку на бабушку, мужчину на женщину, молодого на старого, худого на полного
6. ЗАПРЕЩЕНО: использовать слова "elderly", "пожилой", "бабушка" если в видео молодой человек
7. ВСЮ одежду и аксессуары КОПИРУЙ из оригинала — не придумывай свою
8. Результат = КОПИЯ оригинала но ЛУЧШЕ: та же энергия, те же люди, улучшенное качество
════════════════════════════════════════════════════════════════
` : `
...
• Имя: ${charA.name_ru}
• Возраст: ${charA.biology_override?.age || (isVideoPlaceholder ? 'as in original video' : 'elderly')}
...
• Имя: ${charB.name_ru}
• Возраст: ${charB.biology_override?.age || (isVideoPlaceholder ? 'as in original video' : 'elderly')}
...
${isVideoPlaceholder ? `
VIDEO COPY MODE — Правила Identity Lock ОТКЛЮЧЕНЫ. Копируй внешность, одежду, локацию ТОЧНО из оригинального видео.
Речь копируй дословно. Не придумывай новую речь если она есть в оригинале.
` : `
`}
...
[7.00–8.00] RELEASE — ${releaseAction?.action_ru || 'реакция'}. АБСОЛЮТНЫЙ НОЛЬ СЛОВ. Никаких реплик, никаких добивок голосом. ТОЛЬКО громкий натуральный заразительный ржач — ощущение что персонажей РЕАЛЬНО порвало. Тряска камеры. Rewatch-bait: микро-выражение в последние 0.3с.`}

════════════════════════════════════════════════════════════════
${remake_mode ? `⚠️⚠️⚠️ РЕЖИМ РЕМЕЙКА — ТОЧНАЯ КОПИЯ ОРИГИНАЛЬНОГО ВИДЕО:

🔴 ЭТО УНИКАЛИЗАТОР ВИДЕО. ЗАДАЧА: СОЗДАТЬ ИДЕНТИЧНУЮ КОПИЮ ВИДЕО С НУЛЯ.
🔴 ДИАЛОГ = 100% ДОСЛОВНАЯ РАСШИФРОВКА ОРИГИНАЛА. НИ ОДНОГО СЛОВА НЕ МЕНЯТЬ.
🔴 ДЕЙСТВИЯ = ПОКАДРОВАЯ КОПИЯ. КАЖДЫЙ ЖЕСТ, КАЖДЫЙ ПОВОРОТ, КАЖДЫЙ ВЗГЛЯД.
🔴 ЛОКАЦИЯ = ТА ЖЕ САМАЯ (если пользователь не указал другую).
🔴 КАМЕРА = ТОТ ЖЕ РАКУРС, ТА ЖЕ КРУПНОСТЬ, ТО ЖЕ ДВИЖЕНИЕ.
🔴 РИТМ = ТОТ ЖЕ ТЕМП РЕЧИ, ТЕ ЖЕ ПАУЗЫ, ТА ЖЕ ЭНЕРГИЯ.

ПРАВИЛА РЕМЕЙКА:
1. РАСШИФРУЙ каждое слово из оригинала — ДОСЛОВНО, буква в букву, без пересказа
2. СОХРАНИ 100% слов из оригинала — КАЖДОЕ слово неприкосновенно
3. ЗАПРЕЩЕНО менять слова, переставлять, «улучшать», сокращать, дополнять
4. ЗАПРЕЩЕНО придумывать новый диалог — ты РАСШИФРОВЩИК, не автор
5. Темп, паузы, эмоциональная кривая — КОПИРУЙ из оригинала ПОКАДРОВО
6. Категорию юмора определи по СОДЕРЖАНИЮ оригинала, не придумывай новую
7. СТРУКТУРУ ДИАЛОГА КОПИРУЙ КАК ЕСТЬ: количество реплик, порядок, кто говорит — ВСЁ как в оригинале
8. ДЕЙСТВИЯ КОПИРУЙ ПОКАДРОВО: если человек поднимает руку на 2й секунде — наш персонаж тоже. Если наклоняется — тоже. Если смеётся — тоже. ОДИН В ОДИН.
9. ЛОКАЦИЮ КОПИРУЙ ТОЧНО: если снимали на кухне — описывай кухню. Если на улице — улицу. Каждый предмет фона.
10. КАМЕРУ КОПИРУЙ: если selfie — пиши selfie. Если снимали со стороны — пиши со стороны. Тот же угол.

ЕДИНСТВЕННОЕ ЧТО МЕНЯЕТСЯ (если пользователь выбрал персонажей):
- Внешность людей → заменяется на character_en наших персонажей
- Одежда → заменяется на wardrobe_anchor наших персонажей
ВСЁ ОСТАЛЬНОЕ = ТОЧНАЯ КОПИЯ ОРИГИНАЛА.

ЕСЛИ ПОЛЬЗОВАТЕЛЬ НЕ ВЫБРАЛ ПЕРСОНАЖЕЙ — описывай людей из оригинала как есть (возраст, внешность, одежда — всё из видео).

ТЕСТ: если dialogue_A_ru или dialogue_B_ru отличаются от оригинала ХОТЯ БЫ НА 1 СЛОВО — это БРАК.
ТЕСТ: если photo_scene_en описывает ДРУГУЮ сцену чем в оригинале — это БРАК.
ТЕСТ: если remake_veo_prompt_en описывает ДРУГИЕ действия чем в оригинале — это БРАК.` : (input_mode === 'script' && script_ru) ? `⚠️⚠️⚠️ РЕЖИМ СВОЕГО ДИАЛОГА — ТЕКСТ ПОЛЬЗОВАТЕЛЯ НЕПРИКОСНОВЕНЕН:
...
${remake_mode ? `ПРАВИЛА ФОТО-ПРОМПТА (photo_scene_en) — РЕЖИМ РЕМЕЙКА = ТОЧНАЯ КОПИЯ КАДРА 0!
🚨🚨🚨 КРИТИЧНО: photo_scene_en = СТАРТОВЫЙ КАДР (frame 0) видео-ремейка. Пользователь генерирует ФОТО, затем ВИДЕО из этого фото.
• СКОПИРУЙ ОДИН-В-ОДИН: место действия, композицию кадра, ракурс камеры, позы, действия, освещение — ТОЧНО из оригинала
• ЛОКАЦИЯ: опиши ТУ ЖЕ САМУЮ локацию что в оригинале — материалы стен, пол, предметы фона, свет. Если кухня — опиши ту же кухню. Если улица — ту же улицу. КАЖДЫЙ предмет фона.
• РАКУРС: если в оригинале камера снимает со стороны — описывай СО СТОРОНЫ. Если selfie — selfie. Если сверху — сверху. ТОЧНАЯ КОПИЯ.
• ПОЗЫ: если человек стоит — наш стоит. Если сидит — сидит. Если руки на бёдрах — руки на бёдрах. КАЖДЫЙ жест скопировать.
• ДЕЙСТВИЯ: если в оригинале человек делает действие (подаёт мяч, танцует, бежит, готовит) — наш персонаж делает ТО ЖЕ САМОЕ действие.
• ЕСЛИ ПОЛЬЗОВАТЕЛЬ ВЫБРАЛ ПЕРСОНАЖЕЙ: замени людей на НАШИХ с ПОЛНЫМ identity lock (character_en + wardrobe_anchor + signature_element). Персонажи в СВОЕЙ фирменной одежде.
• ЕСЛИ ПОЛЬЗОВАТЕЛЬ НЕ ВЫБРАЛ ПЕРСОНАЖЕЙ: опиши людей ИЗ ОРИГИНАЛА как есть — их возраст, внешность, одежду, причёску. Всё из видео.
• ЭНЕРГИЯ: передай ТОЧНО то же настроение и эмоциональный заряд что в первом кадре оригинала
• Пиши на АНГЛИЙСКОМ, начинай с описания ТОЧНОЙ КОПИИ сцены из оригинала` : `ПРАВИЛА ФОТО-ПРОМПТА (photo_scene_en) — ЭТО КАДР 0 ВИДЕО!
...
• 250-350 слов, единый плотный абзац. МЕНЬШЕ 250 СЛОВ = БРАК — добавь: детали кожи персонажей, освещение на лицах, положение рук, выражение глаз, складки ткани, пропс, микромимику, текстуру волос, посадку одежды, жесты, психологическую энергию.
...
• НЕБАНАЛЬНАЯ СХЕМА КАДРА: НЕ стандартный фронтальный two-shot! Ищи живые ракурсы — через предмет на переднем плане, через отражение, shoulder POV, off-balance framing (персонажи не по центру), partial obstruction (часть кадра перекрыта объектом), тесная точка съёмки (кухня, лифт, тамбур). Кривоватый живой угол как если друг внезапно включил камеру. Фон РАБОТАЕТ НА ПРИКОЛ.
• КИНЕМАТОГРАФИЧНЫЙ ФОН: опиши локацию МАКСИМАЛЬНО КОНКРЕТНО и визуально богато — не «kitchen» а «worn Soviet kitchen with humming Saratov fridge, yellowed peeling wallpaper, bare bulb overhead casting harsh shadows, chipped enamel pot on stove, steam rising from kettle». Фон создаёт визуальный мир и контекст истории. Драматические детали окружения усиливают зрелищность. МИНИМУМ 3 конкретных предмета/детали фона.
• ДЕТАЛИ ПЕРСОНАЖЕЙ: подробно опиши для КАЖДОГО: микромимику (ширина рта, положение бровей, натяжение мышц), взгляд (направление, интенсивность, блеск), позу (наклон корпуса, положение плеч, жестикуляция), одежду (как сидит, складки, текстура ткани), кожу (текстура по возрасту, зоны цвета, поры). НЕ ПИШИ «woman looks angry» — ПИШИ «jaw clenched, left eyebrow raised 3mm higher than right, nostril flare, fingers white-knuckling the receipt».
...
${remake_mode ? `• РЕЖИМ РЕМЕЙКА — ПОКАДРОВАЯ КОПИЯ ОРИГИНАЛА:
• СМОТРИ ВИДЕО ПОКАДРОВО И ОПИСЫВАЙ ТОЧНО ЧТО ВИДИШЬ:
• hook: ЧТО ИМЕННО происходит в первые 0.7с оригинала? Какое действие? Какой жест? Какой взгляд? ОПИШИ ТОЧНО ЭТО ЖЕ ДЕЙСТВИЕ для нашего персонажа.
• act_A: ЧТО ДЕЛАЕТ говорящий А в оригинале? Как жестикулирует? Куда смотрит? Что делает руками? ЧТО ДЕЛАЕТ молчащий в это время? СКОПИРУЙ ПОКАДРОВО каждое движение.
• act_B: ЧТО ДЕЛАЕТ говорящий B в оригинале? Как произносит панчлайн? Какая пауза перед ним? ЧТО ДЕЛАЕТ A когда слушает? СКОПИРУЙ КАЖДЫЙ ЖЕСТ.
• release: ЧТО происходит в финале оригинала? Смех? Шок? Немая сцена? Какой именно? ОПИШИ ТОЧНО ТО ЖЕ САМОЕ.
• ТЕМП: скорость речи, длина пауз, момент кульминации — ВСЁ из оригинала ОДИН В ОДИН.
• ЭНЕРГИЯ: та же эмоциональная кривая — тот же нарастание, тот же пик, тот же взрыв.
• КАЖДЫЙ СЕГМЕНТ: опиши что РЕАЛЬНО происходит в оригинале, не выдумывай и не обобщай.` : `• hook (pillar 11+6): ВИЗУАЛЬНЫЙ хук — персонаж УЖЕ В ДВИЖЕНИИ с кадра 0 (не начинает действие, а в разгаре). STAR PRESENCE: camera magnetism — взгляд в линзу намертво с первого кадра. Энергия ≥80% немедленно. Camera VEO 3.1: micro push-in 0.8px/frame первые 10 кадров. НЕ текстовый хук — только живая эмоция!
...
• Пиши на АНГЛИЙСКОМ, 120-180 слов. ПОДРОБНО и КОНКРЕТНО — не абстрактно!
...
{${remake_mode ? `
  "original_video_analysis": {
    "num_people": "ЧИСЛО — сколько людей ВСЕГО в кадре (1, 2, 3, 4...)",
    "num_speakers": "ЧИСЛО — сколько людей ГОВОРЯТ (не все люди в кадре говорят)",
    "people_description": ["ПОДРОБНО опиши КАЖДОГО человека из видео ОТДЕЛЬНЫМ элементом массива: возраст (точно!), пол, рост, телосложение, одежда (ТОЧНО: цвет+материал+фасон каждого предмета), причёска (цвет+длина+стиль), лицо (форма, морщины, борода), аксессуары (очки, украшения, платок), обувь. 50-80 слов на человека. Если 3 человека — 3 элемента в массиве."],
    "body_positions": ["Позиция КАЖДОГО человека: сидит на лавке / стоит перед ними / лежит / идёт / наклонился. Где именно в кадре: слева/справа/в центре/на переднем плане."],
    "props_in_hands": ["Что держит КАЖДЫЙ человек: газета с надписью X / стакан / телефон / ничего. ТОЧНО описывай ТЕКСТ на предметах если он виден."],
    "text_on_objects": "Весь видимый текст в кадре: надписи на предметах, вывески, бренды, логотипы. Если текст виден — он ОБЯЗАН быть в промпте. null если текста нет.",
    "location_exact": "УЛЬТРА-ДЕТАЛЬНОЕ описание места — НЕ категория ('barn'), а КОНКРЕТИКА: 'деревенский двор, деревянная лавка перед бревенчатой избой с голубыми резными наличниками, штакетник справа, берёзы на заднем плане'. Каждый предмет фона, каждый материал, каждый цвет. 60-100 слов",
    "camera_type": "selfie / third_person_static / third_person_handheld / drone / tripod",
    "camera_framing": "extreme_close_up / close_up / medium_close / medium / medium_wide / wide / full",
    "camera_angle": "eye_level / low_angle / high_angle / dutch / shoulder_pov / over_shoulder",
    "camera_movement": "static / pan_left / pan_right / tilt_up / tilt_down / handheld_shake / zoom_in / zoom_out / follow / none",
    "lighting_exact": "ТОЧНОЕ описание света: направление (слева/справа/сверху/сзади), тип (яркое солнце/пасмурный/лампа/свечи), температура (тёплый золотой/холодный голубой/нейтральный), качество теней (резкие/мягкие/нет), блики. 30-50 слов",
    "dialogue_transcription": ["ДОСЛОВНАЯ расшифровка КАЖДОЙ реплики ОТДЕЛЬНЫМ элементом: 'Персонаж 1: Она вчера была моя, а сегодня Мишкина!', 'Персонаж 2: Хрен у Мишки как сосна на картине Шишкина!'. КАЖДОЕ слово. Мат = мат. Диалект = диалект. Указывай КТО говорит."],
    "per_speaker_timing": ["ТОЧНОЕ время речи каждого: '0.0-2.5s: Персонаж 3 (стоящий мужчина) говорит', '2.5-7.0s: Персонаж 2 (женщина на лавке) отвечает', '7.0-8.0s: все смеются'. Секунды = РЕАЛЬНЫЕ из видео."],
    "action_timeline": ["ПОСЕКУНДНАЯ раскадровка: '0.0-0.5s: мужчина в пальто подходит к лавке, наклоняется к сидящим', '0.5-2.5s: говорит жестикулируя правой рукой, пара на лавке смотрит на него улыбаясь', '2.5-3.0s: пауза, женщина набирает воздух', '3.0-7.0s: женщина громко отвечает, мужчина с газетой слушает ухмыляясь'. Каждое действие, каждый жест, каждый поворот."],
    "mood": "тип настроения и энергии видео: тёплый деревенский юмор / агрессивный спор / абсурд / сатира / wholesome",
    "what_makes_viral": "2-3 предложения — что именно делает это видео вирусным, какой контраст/ожидание цепляет"
  },` : ''}
...
${soloMode ? `  "dialogue_A_ru": "15-30 слов монолог, макс 2 символа |. Персонаж говорит прямо в камеру",
  "dialogue_B_ru": null,
  "dialogue_A2_ru": null,` : `  "dialogue_A_ru": "${remake_mode ? '100% ДОСЛОВНАЯ РАСШИФРОВКА речи ПЕРВОГО ГОВОРЯЩЕГО из оригинального видео (per_speaker_timing[0]). КАЖДОЕ слово как есть. Мат = мат. Диалект = диалект. НЕ менять, НЕ сокращать, НЕ улучшать, НЕ перефразировать. Макс 2 символа |' : '6-15 слов, макс 1 символ |, НЕ начинай с Зато'}",
  "dialogue_B_ru": "${remake_mode ? '100% ДОСЛОВНАЯ РАСШИФРОВКА речи ВТОРОГО ГОВОРЯЩЕГО из оригинального видео (per_speaker_timing[1]). КАЖДОЕ слово как есть. Мат = мат. Диалект = диалект. killer word = ПОСЛЕДНЕЕ слово последней реплики. Макс 2 символа |. Если в видео один говорящий — null' : '6-18 слов, макс 1 символ |, killer word ПОСЛЕДНЕЕ (если нет добивки), НЕ начинай с Зато'}",
  "dialogue_A2_ru": "${remake_mode ? 'Если в оригинале есть третья реплика/добивка — ДОСЛОВНО. null если нет' : 'ДОБИВКА от A — 1-4 слова, короткая финальная фраза. null если добивки нет. Используй ТОЛЬКО если структура оригинала предполагает добивку или если она усиливает комедию'}",`}
  ...
  "photo_scene_en": "${remake_mode ? (isVideoPlaceholder ? 'ТОЧНАЯ КОПИЯ ПЕРВОГО КАДРА оригинала на АНГЛИЙСКОМ. Hyper-realistic [composition type from video: Two-Shot/Three-Shot/Solo/Group]. Используй ВЕСЬ свой original_video_analysis:\n- Локация: location_exact (КАЖДЫЙ предмет, материал, цвет, текстура — НЕ УПРОЩАЙ).\n- Персонажи: people_description — опиши КАЖДОГО человека ОТДЕЛЬНО (возраст, одежда ТОЧНО по цвету и материалу, позиция: сидит/стоит/лежит, body_positions).\n- Предметы в руках: props_in_hands — ТОЧНО что держит каждый (газета с надписью X, стакан, и т.д.).\n- Текст на предметах: text_on_objects — если виден текст/логотип — он ОБЯЗАН быть в промпте.\n- Камера: camera_type + camera_framing + camera_angle.\n- Свет: lighting_exact (направление, тип, температура, тени).\n- Заканчивай: Character, clothing, and environment are strictly identical to the source video. Atmosphere: [from mood]. 8k resolution. --ar 9:16.\n- 250-350 слов EN + negative prompt (NO text overlays, NO watermarks, NO UI).' : 'ТОЧНАЯ КОПИЯ ПЕРВОГО КАДРА оригинала на АНГЛИЙСКОМ. Hyper-realistic [composition type]. Используй ВЕСЬ свой original_video_analysis для локации (location_exact), камеры (camera_type+framing+angle), света (lighting_exact), предметов (props_in_hands, text_on_objects). Но ПЕРСОНАЖЕЙ замени: для КАЖДОГО — ДОСЛОВНО вставь character_en + wardrobe_anchor + signature_element из Identity Lock. Позиции (сидит/стоит) и предметы в руках — ИЗ ОРИГИНАЛА. Заканчивай: Character clothing from Identity Lock, environment strictly identical to source video. 250-350 слов EN + negative prompt.') : 'ОЧЕНЬ ДЛИННЫЙ И ПОДРОБНЫЙ промпт 250-350 слов EN. Medium shot photo capturing the EXACT HOOK MOMENT (frame 0). НЕБАНАЛЬНЫЙ ракурс (не стандартный two-shot). Устройство НЕВИДИМО. ОБЯЗАТЕЛЬНО подробно: схема кадра и POV, микромимика каждого персонажа (положение бровей, рта, глаз), реквизит в руке A, конкретные детали фона (минимум 3 предмета), освещение (цветовая температура, направление, тени), текстуры кожи и одежды, атмосфера. + negative prompt'}",
  "video_emotion_arc": {
    "hook_en": "${remake_mode ? 'ТАЙМКОД ИЗ ОРИГИНАЛА (per_speaker_timing + action_timeline): Опиши первые 0.5-1с ТОЧНО — кто что делает, какой жест/движение создаёт стоп-скролл, кто в кадре, какой ракурс. 60-80 слов EN' : '0.0-0.7s: ПОДРОБНО 60-80 слов EN: стоп-скролл визуальный хук. Персонаж A УЖЕ В ДВИЖЕНИИ. Опиши: что делает A (жест, взгляд в линзу, реквизит), что делает B (side-eye, микромимика), камера (micro push-in), звук (вдох/щелчок/шорох). No text on screen'}",
${soloMode ? `    "monologue_en": "0.7-7.0s: описание монолога на английском — один персонаж, прямо в камеру, killer word ≈ 6.8s",` : `    "act_A_en": "${remake_mode ? 'ТАЙМКОД ИЗ ОРИГИНАЛА (per_speaker_timing): Кто говорит первым? ОПИШИ: что он/она говорит (audio sync: ДОСЛОВНЫЙ текст), жесты, мимика, поза. Кто в этот момент СТРОГО МОЛЧИТ и что делает (смотрит, улыбается, кивает). 80-120 слов EN' : '0.7-3.5s: ПОДРОБНО 80-100 слов EN: A говорит провокацию. Опиши: артикуляция A (жесты руками, наклон корпуса, мимика на ключевых словах), B МОЛЧИТ рот СТРОГО закрыт (только: моргание каждые 4-6с, side-eye, nostril flare, eyebrow shift, finger tap). Камера: micro-movement. Дыхание, вдох перед фразой. No text on screen'}",
    "act_B_en": "${remake_mode ? 'ТАЙМКОД ИЗ ОРИГИНАЛА (per_speaker_timing): Кто говорит вторым? ОПИШИ: что он/она говорит (audio sync: ДОСЛОВНЫЙ текст с killer word), жесты, мимика, поза. Кто СТРОГО МОЛЧИТ. Если в видео 3+ говорящих — опиши КАЖДОГО по очереди с таймкодами. 80-120 слов EN' : '3.5-7.0s: ПОДРОБНО 80-100 слов EN: B отвечает панчлайном. 0.25с ПАУЗА перед ответом (tension beat — B expression меняется: loaded micro-smirk). B: голос снижается перед killer word → 0.2с пауза → взрыв артикуляции → jaw snap → глаза сужаются. A МОЛЧИТ рот СТРОГО закрыт: mid-gesture freeze → глаза расширяются. KW≈6.8s. Camera: micro push-in на killer word. No text on screen'}",`}
    "release_en": "${!enableLaughter ? '7.0-8.0s: БЕЗ СМЕХА. Речь заканчивается, тишина. Замершие выражения лиц после панчлайна. Rewatch-bait 0.3s. НЕ ПИШИ смех, хихиканье, усмешки — ТОЛЬКО тишина' : remake_mode ? 'ТАЙМКОД ИЗ ОРИГИНАЛА: Опиши финал ТОЧНО как в видео — если все смеются, опиши ФИЗИКУ смеха КАЖДОГО (трясущиеся плечи, запрокинутая голова, открытый рот, слёзы). Если тишина — замершие лица. Камера: shake/статика. Rewatch-bait. 60-80 слов EN' : scene_hint ? 'ФИНАЛ СОГЛАСНО ИНСТРУКЦИИ ПОЛЬЗОВАТЕЛЯ (см. выше). Если пользователь указал без смеха — НЕ ПИШИ смех. Rewatch-bait 0.3s' : 'ПОДРОБНО 60-80 слов EN: АБСОЛЮТНЫЙ НОЛЬ СЛОВ. Только натуральный заразительный ржач. Опиши физику: трясущиеся плечи, запрокинутая голова, слёзы, хриплый звук. Камера трясётся от смеха. 7.7-8.0с: rewatch-bait micro-expression. LOOP LOCK: финальная поза совместима с кадром 0. No text on screen'}"
  },
  "video_atmosphere_en": "${remake_mode ? 'ТОЧНАЯ КОПИЯ атмосферы оригинала на английском: ВСЕ звуки локации из видео (ветер, птицы, скрип, шаги, музыка), реверб по размеру помещения (улица = открытый, комната = закрытый), room tone, шорохи одежды при движении, дыхание говорящих. Свет: ТОЧНОЕ направление и температура из lighting_exact. RAW-эстетика: ISO шум, пересветы. Камера: micro-jitter если с рук. 100-150 слов EN' : '120-180 слов на английском. ПОДРОБНО: конкретные звуки локации (минимум 3), реверб, room tone, микрофоника телефона, дыхание, шорохи одежды, клики слюны. Свет: направление, температура, тени. RAW-эстетика: ISO шум, пересветы, JPEG-компрессия. Камера: micro-jitter, breathing oscillation, OIS/EIS. Частицы: пыль/пар в свете'}",
  "end_frame_en": "${remake_mode ? 'ПОДРОБНЫЙ промпт ФИНАЛЬНОГО КАДРА оригинала (последние 0.5с) на английском, 150-200 слов. ТОЧНАЯ КОПИЯ последнего момента видео. Опиши КАЖДОГО человека: поза в этот момент, выражение лица (смех/удивление/тишина), положение рук и тела. Если все смеются — опиши ФИЗИКУ: трясущиеся плечи, запрокинутая голова, открытый рот с видимыми зубами/дёснами, слёзы на глазах. Фон = тот же что в начале (location_exact). Свет = lighting_exact. Камера: shake если оригинал с рук. 100% texture consistency. NO TEXT. 8k resolution. --ar 9:16' : 'ПОДРОБНЫЙ промпт финального кадра (7.5-8.0с) на английском, 150-200 слов. Точное описание: физика смеха (трясущиеся плечи, запрокинутая голова, слёзы на глазах, открытый рот), позы обоих персонажей, камера (shake от смеха, slight defocus), свет на лицах в момент смеха, микродетали (морщины от улыбки, блестящие глаза, пот). Rewatch-bait: неоднозначное micro-expression в последние 0.3с. LOOP LOCK: финальная поза совместима с кадром 0 для авто-лупа. Negative: no text, no overlays'}",
  "trend_today_ru": "${remake_mode ? 'null' : 'Подробно объясни (3-5 предложений): 1) какой мемный вайб/конфликт/интонация актуальны ИМЕННО СЕГОДНЯ, 2) почему эта шутка ощущается как сегодняшняя а не старая, 3) в чём свежесть механики'}",
  "camera_scheme_ru": "${remake_mode ? 'null' : 'Объясни выбранную схему кадра (2-3 предложения): почему она небанальная, почему реалистичная, как усиливает шутку. Например: shoulder POV через плечо B, off-balance framing, через предмет на переднем плане'}",
  ...
  "remake_veo_prompt_en": "[ПОКАДРОВАЯ 1:1 КОПИЯ ОРИГИНАЛА — ВСЕ ДАННЫЕ ИЗ original_video_analysis]\n\n[БЛОК 1 — СЦЕНА И ЛОКАЦИЯ (80-120 слов EN): Используй СВОЙ original_video_analysis.location_exact — УЛЬТРА-ДЕТАЛЬНОЕ описание ТОЧНОГО места из оригинала. КАЖДЫЙ предмет, материал, цвет, текстура: если в видео 'бревенчатый сруб с голубыми резными наличниками, деревянная лавка, штакетник, берёзы' — пиши ИМЕННО ЭТО, НЕ 'wooden barn'. props_in_hands: что держит каждый человек (газета, стакан, телефон). text_on_objects: ВЕСЬ видимый текст/логотипы (если газета с надписью 'JOY CASINO' — ОБЯЗАТЕЛЬНО упомяни). Освещение = lighting_exact. Минимум 3 конкретных фоновых звука локации.]\n\n[БЛОК 2 — КАМЕРА И ТЕХНИКА (40-60 слов EN): Тип композиции: Solo-Shot / Two-Shot / Three-Shot / Group из original_video_analysis.num_people. camera_type + camera_framing + camera_angle + camera_movement — ТОЧНО как в оригинале. Handheld micro-jitter и breathing oscillation если оригинал с рук. RAW phone aesthetic: sensor noise ISO 800-1600, JPEG artifacts, blown highlights. Устройство невидимо.]\n\n[БЛОК 3 — ВСЕ ПЕРСОНАЖИ (100-200 слов EN, зависит от num_people): ${isVideoPlaceholder ? 'Опиши КАЖДОГО человека из original_video_analysis.people_description ОТДЕЛЬНЫМ параграфом. Для КАЖДОГО: возраст, пол, телосложение, лицо (морщины, борода, черты), волосы (цвет, длина, стиль), одежда (ТОЧНО: цвет+материал+фасон КАЖДОГО предмета), аксессуары, обувь. Позиция тела: body_positions (сидит на лавке / стоит перед ними / наклонился). Предмет в руках: props_in_hands. Если 3 человека — 3 параграфа. НЕ ПРОПУСКАЙ ни одного человека.' : `Замени внешность на Identity Lock, но СОХРАНИ позиции (body_positions) и предметы (props_in_hands) из оригинала. Для КАЖДОГО персонажа: ДОСЛОВНО character_en + wardrobe_anchor + signature_element. Позиция тела = из оригинала.`} Hyper-realism: visible pores, birthmarks, wrinkles, age spots, sweat. 8k texture detail.]\n\n[БЛОК 4 — ДЕЙСТВИЕ ПО СЕКУНДАМ (200-350 слов EN — ГЛАВНЫЙ БЛОК): Используй original_video_analysis.per_speaker_timing + action_timeline. ОПИШИ КАЖДУЮ СЕКУНДУ видео ОТДЕЛЬНЫМ таймкодом. Формат:\n**[X.X-Y.Ys]:** КТО говорит (имя/описание), ЧТО говорит (audio sync: exact Russian text), КАК говорит (жесты, мимика, наклон, поворот). КТО в этот момент СТРОГО МОЛЧИТ (mouth closed, sealed lips) и ЧТО делает (смотрит, улыбается, кивает). Камера: движение/статика.\n\nПРИМЕР формата:\n**[0.0-2.5s]:** The Standing Man (Person 3) speaks excitedly, gesturing with right hand (audio sync: 'Она вчера была моя, а сегодня Мишкина!'). The couple on the bench is STRICTLY SILENT, looking up at him and smiling. Handheld camera, slight shake.\n**[2.5-7.0s]:** The Old Woman (Person 2) speaks her punchline loudly, looking directly at him (audio sync: 'Хрен у Мишки как сосна на картине Шишкина!'). Her mouth movements are 100% synchronized. The Man with newspaper listens and smiles. Standing Man grins.\n**[7.0-8.0s]:** All three burst into loud genuine contagious laugh. Camera shakes from operator's laughter.\n\nКЛЮЧЕВЫЕ ПРАВИЛА: 1) strict character isolation — ТОЛЬКО ОДИН человек говорит в каждый момент, ОСТАЛЬНЫЕ СТРОГО МОЛЧАТ (sealed lips). 2) 100% syllable-level lip-sync для КАЖДОГО говорящего. 3) Таймкоды = РЕАЛЬНАЯ длительность оригинала. 4) КАЖДЫЙ жест из action_timeline.]\n\n[БЛОК 5 — ФИНАЛ И РЕАКЦИЯ (60-80 слов EN): Опиши ТОЧНО как заканчивается оригинал. Если все смеются — опиши ФИЗИКУ смеха каждого: трясущиеся плечи, запрокинутая голова, открытый рот, слёзы. Если тишина — опиши замершие лица. Камера: shake от смеха / статика. 100% texture consistency с предыдущими кадрами. Rewatch-bait в последние 0.3s.]\n\n[БЛОК 6 — TECH + NEGATIVE (40-60 слов EN): 9:16 vertical. Длительность = РЕАЛЬНАЯ длительность оригинала из per_speaker_timing. RAW smartphone footage: sensor noise ISO 800-1600, JPEG artifacts, imperfect auto white-balance, blown highlights +0.5-1.5EV. Room tone из оригинала (original_video_analysis.mood). NEGATIVE: NO text, NO subtitles, NO captions, NO watermarks, NO UI, NO REC, NO smooth skin, NO studio lighting, NO CGI, NO plastic skin, NO AI artifacts, NO filters.]"
...
// ─── POST /api/photo-to-prompt — Image analysis → detailed recreation prompt ──────────
app.post('/api/photo-to-prompt', authMiddleware, async (req, res) => {
  ...
  const { image, mime, style_hint, lang } = req.body;
  if (!image) {
  ...
  const systemPrompt = `Ты — экспертный AI-аналитик изображений и мастер написания промптов для генерации изображений.

Твоя задача: проанализировать загруженное фото и создать ДЕТАЛИЗИРОВАННЫЙ промпт для воссоздания этого изображения в Google Labs ImageFX (модель Imagen 3 / Flow).

ПРАВИЛА:
1. Промпт должен быть на АНГЛИЙСКОМ языке (ImageFX работает лучше на английском)
2. Промпт должен быть 150-350 слов — максимально детальный
3. Структура промпта:
   - SUBJECT: Главный объект/персонаж — внешность, поза, выражение лица, одежда, аксессуары
   - SETTING/BACKGROUND: Окружение, локация, предметы на фоне
   - LIGHTING: Тип освещения, направление, цветовая температура, тени, блики
   - COMPOSITION: Ракурс камеры, кадрирование, фокус, глубина резкости
   - STYLE: Художественный стиль, цветовая палитра, настроение, текстура
   - TECHNICAL: Качество, детализация (photorealistic / illustration / etc)
4. Используй конкретные описания, а не абстрактные ("warm golden hour sunlight from the left" вместо "nice lighting")
5. Добавь негативный промпт (что НЕ должно быть)
${style_hint ? `6. Учти пожелание пользователя по стилю: "${style_hint}"` : ''}

Ответь СТРОГО в формате JSON:
{
  "prompt_en": "полный детальный промпт на английском для ImageFX",
  "negative_prompt_en": "что исключить (на английском)",
  "prompt_ru": "тот же промпт переведённый на русский",
  "style_tags": ["тег1", "тег2", "тег3"],
  "detected_style": "краткое описание обнаруженного стиля фото",
  "tips": ["совет 1 для лучшего результата в ImageFX", "совет 2"],
  "complexity": "simple | medium | complex"
}

КРИТИЧНО: Отвечай ТОЛЬКО валидным JSON. Без markdown. Без блоков кода.`;

  try {
    const parts = [
      { text: systemPrompt },
      { text: '\n\n[ЗАГРУЖЕННОЕ ФОТО ДЛЯ АНАЛИЗА — рассмотри каждую деталь]' },
    ...
    if (!resp.ok) {
      ...
      console.error('[PHOTO-TO-PROMPT] Gemini error:', msg);
      return res.status(resp.status).json({ error: `Ошибка AI: ${msg}` });
    }

    const text = data.candidates?.[0]?.content?.parts?.[0]?.text;
    if (!text) {
      return res.status(502).json({ error: 'AI не вернул ответ. Попробуй ещё раз.' });
    }
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        return res.status(502).json({ error: 'AI вернул некорректный формат.' });
      }
    }

    console.log(`[PHOTO-TO-PROMPT] ✓ Generated prompt (${result.complexity || '?'}) for user ${userId}`);
    res.json({ result });
  } catch (e) {
    console.error('[PHOTO-TO-PROMPT] Error:', e.message);
    if (e.name === 'AbortError') {
      return res.status(504).json({ error: 'Таймаут: AI думает слишком долго. Попробуй ещё раз.' });
    }
...
════════════════════════════════════════════════════════════════`;
    ...
    if (video_file) {
      parts.push({
        text: `\n\n[🎬 ПРИКРЕПЛЁННОЕ ОРИГИНАЛЬНОЕ ВИДЕО — ОБЯЗАТЕЛЬНЫЙ ПОКАДРОВЫЙ АНАЛИЗ

═══ АБСОЛЮТНЫЙ ПРИОРИТЕТ №1: СНАЧАЛА СМОТРИ, ПОТОМ ПИШИ ═══
ПОСМОТРИ ВИДЕО ПОЛНОСТЬЮ 2 РАЗА. Первый раз — на слух (речь). Второй раз — на глаз (действия, сцена).
ЗАПОЛНИ original_video_analysis В JSON — это ОБЯЗАТЕЛЬНОЕ поле. Без него генерация = БРАК.

═══ ПРИОРИТЕТ №2: ДОСЛОВНАЯ РАСШИФРОВКА РЕЧИ ═══
ПРОСЛУШАЙ КАЖДОЕ СЛОВО. Расшифруй речь ДОСЛОВНО — каждое слово, каждый звук, каждую паузу.
НЕ ПЕРЕСКАЗЫВАЙ. НЕ УЛУЧШАЙ. НЕ СОКРАЩАЙ. НЕ ПРИДУМЫВАЙ НОВЫЙ ДИАЛОГ.
dialogue_A_ru = ТОЧНАЯ РАСШИФРОВКА первого говорящего. Если он сказал "Хрен у Мишки" — пиши "Хрен у Мишки".
dialogue_B_ru = ТОЧНАЯ РАСШИФРОВКА второго говорящего. Если она сказала матерное слово — пиши матерное слово.
ТЕСТ: прочитай свою расшифровку вслух — она СОВПАДАЕТ с аудио? Если нет — ПЕРЕДЕЛАЙ.

═══ ПРИОРИТЕТ №3: УЛЬТРА-ДЕТАЛЬНЫЙ ПОКАДРОВЫЙ АНАЛИЗ ═══
1) СЦЕНА: КОНКРЕТНОЕ место (не "barn" а "бревенчатый сруб с голубыми резными наличниками, деревянная лавка, штакетник"). Каждый предмет, каждый материал, каждый цвет фона.
2) ПЕРСОНАЖИ: ТОЧНОЕ описание КАЖДОГО человека ОТДЕЛЬНО — возраст (точно!), пол, рост, одежда (цвет+материал+фасон КАЖДОГО предмета одежды), причёска (цвет+длина), аксессуары (очки, платок, украшения), поза (сидит/стоит/наклонился), позиция в кадре (слева/справа/в центре).
3) ПРЕДМЕТЫ В РУКАХ: что держит КАЖДЫЙ человек — газета, стакан, телефон, ничего. ТЕКСТ на предметах: если на газете написано "JOY CASINO" — ЗАПИШИ "JOY CASINO".
4) КАМЕРА: ТОЧНЫЙ ракурс — selfie/со стороны/сверху? Крупный/средний/общий? Статичная/с рук/на штативе? Движение камеры (панорама, наезд, отъезд)?
5) ДЕЙСТВИЯ: что делает КАЖДЫЙ человек в КАЖДУЮ секунду? Жест, шаг, наклон, поворот, жестикуляция рукой — покадрово по секундам.
6) ИЗОЛЯЦИЯ РЕЧИ: в КАЖДЫЙ момент времени ТОЛЬКО ОДИН человек говорит. Все остальные СТРОГО МОЛЧАТ (рот закрыт). Запиши КТО говорит с X по Y секунду.
7) МИМИКА: выражения лиц КАЖДОГО — когда улыбка, когда удивление, когда смех, когда брови вверх, открытый рот.
8) СВЕТ: ТОЧНОЕ направление (слева/справа/сверху/сзади), тип (солнце/лампа/пасмурно), температура, тени, блики.
9) ЗВУК: фоновые шумы (ветер, птицы, скрип двери, шаги, музыка, шум улицы).
10) ТЕМП: длительность КАЖДОЙ реплики в секундах, длительность пауз, момент панчлайна (killer word).
11) ЭНЕРГИЯ: эмоциональная кривая — спокойное начало → нарастание → кульминация → взрыв/смех.
12) КОЛИЧЕСТВО ЛЮДЕЙ: сколько людей в кадре? Кто говорит? Кто молчит? Кто входит/выходит? Если 3 человека — опиши всех 3.
13) ФИНАЛ: как именно заканчивается видео — смех/тишина/реакция? Поза каждого человека в последнем кадре.

═══ ПРИОРИТЕТ №4: ТОЧНАЯ КОПИЯ ═══
ЕСЛИ ПОЛЬЗОВАТЕЛЬ НЕ ВЫБРАЛ СВОИХ ПЕРСОНАЖЕЙ — описывай людей ИЗ ВИДЕО как есть (возраст, внешность, одежда — всё из видео).
ЕСЛИ ПОЛЬЗОВАТЕЛЬ ВЫБРАЛ ПЕРСОНАЖЕЙ — замени ТОЛЬКО внешность и одежду на наших. ВСЁ ОСТАЛЬНОЕ (действия, локация, ракурс, свет, диалог, темп, энергия) = ТОЧНАЯ КОПИЯ ОРИГИНАЛА.
Локация = ТА ЖЕ. Если в видео деревня с избой — описывай деревню с избой, НЕ меняй на "barn" или "кухню".
Камера = ТА ЖЕ. Если снимают со стороны — со стороны. Если selfie — selfie.
Число людей = ТО ЖЕ. Если в видео 3 человека — описывай 3 человека.]`
      });
      parts.push({
    ...
    // Video/remake mode: validate critical fields
    if (isRemakeMode) {
      if (!geminiResult.remake_veo_prompt_en) {
        console.warn('[REMAKE] Missing remake_veo_prompt_en — Gemini did not return VEO prompt for remake');
      } else if (geminiResult.remake_veo_prompt_en.split(/\s+/).length < 100) {
        console.warn('[REMAKE] remake_veo_prompt_en too short:', geminiResult.remake_veo_prompt_en.split(/\s+/).length, 'words (need 250+)');
      }
      if (!geminiResult.video_emotion_arc) {
        console.warn('[REMAKE] Missing video_emotion_arc — no emotion arc for video');
      }
    }
