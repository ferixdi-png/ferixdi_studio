...
• Символ | обозначает естественную паузу-вдох внутри реплики. СТРОГО МАКСИМУМ 1 символ | на реплику! Если пауза не нужна — НЕ СТАВЬ | вообще. ЗАПРЕЩЕНО ставить 2 и более | в одной реплике
...
• B = СПОКОЙНОЕ УНИЧТОЖЕНИЕ: неожиданный угол НО СТРОГО ПО ТЕМЕ А! B отвечает на ТУ ЖЕ тему что поднял A, просто находит в ней неожиданный поворот. B НЕ ИМЕЕТ ПРАВА уходить в другую тему — если A говорит про фамилии, B отвечает про фамилии. Если A про цены, B про цены. НИКОГДА не меняй тему между A и B!
• Killer word = ПОСЛЕДНЕЕ слово B. Оно ПЕРЕВОРАЧИВАЕТ контекст ЭТОЙ ЖЕ темы. Зритель думал одно, а killer word показывает другое — но в рамках ТОЙ ЖЕ темы
• ЗАПРЕТ: не начинай B с «Зато...» — это шаблонно и скучно. Каждый ответ B использует РАЗНУЮ технику юмора. B НИКОГДА не уходит в другую тему — это убивает комедию
• Техники B (ВСЕ в рамках темы A!): абсурдное сравнение («Курица живёт лучше пенсионера»), callback к словам A («Ты туда и масло кладёшь»), переворот статуса («Бумер построил дом где твой вайфай»), deadpan логика, эскалация в абсурд той же темы
...
• ГЛАВНОЕ: A и B спорят об ОДНОМ И ТОМ ЖЕ. Это ДИАЛОГ, а не два монолога на разные темы
