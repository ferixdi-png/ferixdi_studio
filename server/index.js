...
{${remake_mode ? `
  "original_video_analysis": {
    "num_people": "ЧИСЛО — сколько людей в кадре (1, 2, 3...)",
    "people_description": ["ПОДРОБНО опиши КАЖДОГО человека из видео: возраст, пол, телосложение, одежда (цвет+материал+фасон), причёска, лицо, аксессуары. 30-50 слов на человека"],
    "location_exact": "ТОЧНОЕ описание места из видео — не категория ('barn'), а конкретика ('бревенчатый сруб с голубыми резными наличниками, деревянная лавка, штакетник справа, голые ветки, остатки снега'). 40-60 слов",
    "camera_type": "selfie / third_person_static / third_person_handheld / drone / tripod",
    "camera_framing": "close_up / medium / medium_wide / wide / full",
    "camera_angle": "eye_level / below / above / dutch / shoulder_pov",
    "lighting_exact": "ТОЧНОЕ описание света: направление, тип (естественный/искусственный), температура (тёплый золотой час / холодный пасмурный), качество теней. 20-30 слов",
    "dialogue_transcription": ["ДОСЛОВНАЯ расшифровка КАЖДОЙ реплики из видео в порядке произнесения. КАЖДОЕ слово. Мат = мат. Диалект = диалект."],
    "action_timeline": ["0-2s: мужчина подходит к лавке", "2-5s: бабушка отвечает присказкой", "5-8s: все трое смеются"],
    "mood": "тип настроения и энергии видео: тёплый деревенский юмор / агрессивный спор / абсурд",
    "what_makes_viral": "1 предложение — что именно делает это видео вирусным"
  },` : ''}
...
${soloMode ? `  "dialogue_A_ru": "15-30 слов монолог, макс 2 символа |. Персонаж говорит прямо в камеру",
  "dialogue_B_ru": null,
  "dialogue_A2_ru": null,` : `  "dialogue_A_ru": "${remake_mode ? '100% ДОСЛОВНАЯ РАСШИФРОВКА речи первого говорящего из оригинального видео. КАЖДОЕ слово как есть. Не менять, не сокращать, не улучшать. Макс 2 символа |' : '6-15 слов, макс 1 символ |, НЕ начинай с Зато'}",
  "dialogue_B_ru": "${remake_mode ? '100% ДОСЛОВНАЯ РАСШИФРОВКА речи второго говорящего из оригинального видео. КАЖДОЕ слово как есть. killer word = ПОСЛЕДНЕЕ слово. Макс 2 символа |. Если в видео один говорящий — null' : '6-18 слов, макс 1 символ |, killer word ПОСЛЕДНЕЕ (если нет добивки), НЕ начинай с Зато'}",
  ...
  "photo_scene_en": "${remake_mode ? (isVideoPlaceholder ? 'ТОЧНАЯ КОПИЯ первого кадра оригинала. Используй СВОЙ original_video_analysis: location_exact (ТОЧНАЯ локация из видео — каждый предмет, материал, цвет), camera_type + camera_framing + camera_angle (ТОЧНЫЙ ракурс из видео), lighting_exact (ТОЧНЫЙ свет из видео). Персонажей описывай ТОЧНО как в видео (people_description). НЕ ЗАМЕНЯЙ локацию, НЕ УПРОЩАЙ, НЕ ИСПОЛЬЗУЙ generic описания. Если в видео деревня с избой и резными наличниками — описывай ИМЕННО ЭТО. 200-300 слов EN + negative prompt' : 'ТОЧНАЯ КОПИЯ первого кадра оригинала. Используй СВОЙ original_video_analysis: location_exact (ТОЧНАЯ локация из видео), camera_type + camera_framing + camera_angle (ТОЧНЫЙ ракурс), lighting_exact (ТОЧНЫЙ свет). Но ПЕРСОНАЖЕЙ замени: для КАЖДОГО — ДОСЛОВНО вставь character_en + wardrobe_anchor + signature_element из Identity Lock. Локация, ракурс, свет, композиция, предметы фона = из ОРИГИНАЛА. 200-300 слов EN + negative prompt') : 'ОЧЕНЬ ДЛИННЫЙ И ПОДРОБНЫЙ промпт 250-350 слов EN. Medium shot photo capturing the EXACT HOOK MOMENT (frame 0). НЕБАНАЛЬНЫЙ ракурс (не стандартный two-shot). Устройство НЕВИДИМО. ОБЯЗАТЕЛЬНО подробно: схема кадра и POV, микромимика каждого персонажа (положение бровей, рта, глаз), реквизит в руке A, конкретные детали фона (минимум 3 предмета), свет (направление, температура, тени на лице), текстура кожи и одежды, поза и жесты. + negative prompt'}",
  "video_emotion_arc": {
    "hook_en": "${remake_mode ? 'ТАЙМКОД ИЗ ОРИГИНАЛА (original_video_analysis.action_timeline): СКОПИРУЙ первые 0.5-1с оригинала ТОЧНО — тот же хук, тот же жест, тот же ракурс. Стоп-скролл момент как в оригинале' : '0.0-0.7s: ПОДРОБНО 60-80 слов EN: стоп-скролл визуальный хук. Персонаж A УЖЕ В ДВИЖЕНИИ. Опиши: что делает A (жест, взгляд в линзу, реквизит), что делает B (side-eye, микромимика), камера (micro push-in), звук (вдох/щелчок/шорох). No text on screen'}",
${soloMode ? `    "monologue_en": "0.7-7.0s: описание монолога на английском — один персонаж, прямо в камеру, killer word ≈ 6.8s",` : `    "act_A_en": "${remake_mode ? 'ТАЙМКОД ИЗ ОРИГИНАЛА: СКОПИРУЙ первую часть действия из original_video_analysis.action_timeline ТОЧНО — те же слова (dialogue_transcription), те же жесты, тот же темп, те же паузы. Мимика и позы ТОЧНО как в видео' : '0.7-3.5s: ПОДРОБНО 80-100 слов EN: A говорит провокацию. Опиши: артикуляция A (жесты руками, наклон корпуса, мимика на ключевых словах), B МОЛЧИТ рот СТРОГО закрыт (только: моргание каждые 4-6с, side-eye, nostril flare, eyebrow shift, finger tap). Камера: micro-movement. Дыхание, вдох перед фразой. No text on screen'}",
    "act_B_en": "${remake_mode ? 'ТАЙМКОД ИЗ ОРИГИНАЛА: СКОПИРУЙ вторую часть действия из original_video_analysis.action_timeline ТОЧНО — те же слова, тот же ритм доставки, те же паузы. killer word в ТОМ ЖЕ месте как в оригинале' : '3.5-7.0s: ПОДРОБНО 80-100 слов EN: B отвечает панчлайном. 0.25с ПАУЗА перед ответом (tension beat — B expression меняется: loaded micro-smirk). B: голос снижается перед killer word → 0.2с пауза → взрыв артикуляции → jaw snap → глаза сужаются. A МОЛЧИТ рот СТРОГО закрыт: mid-gesture freeze → глаза расширяются. KW≈6.8s. Camera: micro push-in на killer word. No text on screen'}",`}
    "release_en": "${!enableLaughter ? '7.0-8.0s: БЕЗ СМЕХА. Речь заканчивается, тишина. Замершие выражения лиц после панчлайна. Rewatch-bait 0.3s. НЕ ПИШИ смех, хихиканье, усмешки — ТОЛЬКО тишина' : remake_mode ? 'ТАЙМКОД ИЗ ОРИГИНАЛА: СКОПИРУЙ финал из original_video_analysis.action_timeline ТОЧНО — та же реакция, тот же смех/тишина, та же энергия. Rewatch-bait как в оригинале' : scene_hint ? 'ФИНАЛ СОГЛАСНО ИНСТРУКЦИИ ПОЛЬЗОВАТЕЛЯ (см. выше). Если пользователь указал без смеха — НЕ ПИШИ смех. Rewatch-bait 0.3s' : 'ПОДРОБНО 60-80 слов EN: АБСОЛЮТНЫЙ НОЛЬ СЛОВ. Только натуральный заразительный ржач. Опиши физику: трясущиеся плечи, запрокинутая голова, слёзы, хриплый звук. Камера трясётся от смеха. 7.7-8.0с: rewatch-bait micro-expression. LOOP LOCK: финальная поза совместима с кадром 0. No text on screen'}"
  },
  "video_atmosphere_en": "${remake_mode ? 'СКОПИРУЙ атмосферу оригинала: звуки локации, реверб по размеру помещения, room tone, шорохи, фоновый шум. 80-120 слов EN' : '120-180 слов на английском. ПОДРОБНО: конкретные звуки локации (минимум 3), реверб, room tone, микрофоника телефона, дыхание, шорохи одежды, клики слюны. Свет: направление, температура, тени. RAW-эстетика: ISO шум, пересветы, JPEG-компрессия. Камера: micro-jitter, breathing oscillation, OIS/EIS. Частицы: пыль/пар в свете'}",
  "end_frame_en": "${remake_mode ? 'null' : 'ПОДРОБНЫЙ промпт финального кадра (7.5-8.0с) на английском, 150-200 слов. Точное описание: физика смеха (трясущиеся плечи, запрокинутая голова, слёзы на глазах, открытый рот), позы обоих персонажей, камера (shake от смеха, slight defocus), свет на лицах в момент смеха, микродетали (морщины от улыбки, блестящие глаза, пот). Rewatch-bait: неоднозначное micro-expression в последние 0.3с. LOOP LOCK: финальная поза совместима с кадром 0 для авто-лупа. Negative: no text, no overlays'}",
  ...
  "remake_veo_prompt_en": "[ПОКАДРОВАЯ КОПИЯ ОРИГИНАЛА — ВСЕ ДАННЫЕ ИЗ original_video_analysis]\n\n[БЛОК 1 — СЦЕНА И ЛОКАЦИЯ (70-90 слов): Используй СВОЙ original_video_analysis.location_exact — ТОЧНОЕ место из оригинала с КАЖДЫМ предметом, материалом, цветом, текстурой. НЕ УПРОЩАЙ: если в видео 'бревенчатый сруб с голубыми резными наличниками, деревянная лавка, штакетник' — пиши ИМЕННО ЭТО, НЕ 'wooden barn'. Освещение = original_video_analysis.lighting_exact (направление, тип, температура, тени). Минимум 2 конкретных микро-звука этой локации из видео.]\n\n[БЛОК 2 — РАКУРС КАМЕРЫ (35-45 слов): Используй original_video_analysis.camera_type + camera_framing + camera_angle — ТОЧНЫЙ ракурс из оригинала. Если в оригинале со стороны — НЕ ПИШИ selfie. Если общий план — НЕ ПИШИ крупный. Handheld micro-jitter если оригинал с рук. RAW phone aesthetic: ISO noise in shadows, blown highlights.]\n\n[БЛОК 3 — ПЕРСОНАЖ A (80-120 слов): ${isVideoPlaceholder ? 'Используй original_video_analysis.people_description[0] — опиши ТОЧНО как в видео: возраст, пол, телосложение, лицо, волосы, одежда (цвет+материал+фасон), аксессуары. НЕ ЗАМЕНЯЙ внешность, НЕ ЗАМЕНЯЙ одежду.' : `ВСТАВЬ ДОСЛОВНО: ${charA.prompt_tokens?.character_en || charA.appearance_ru || charA.name_ru}. Wardrobe: ${charA.identity_anchors?.wardrobe_anchor || '—'}. Signature: ${charA.identity_anchors?.signature_element || '—'}.`} Hyper-realism: visible pores, birthmarks, wrinkles, sweat on temples. Поза и положение ТОЧНО как в оригинале.]\n\n[БЛОК 4 — ПЕРСОНАЖ B (80-120 слов): ${isVideoPlaceholder ? 'Используй original_video_analysis.people_description[1]. Если в видео нет второго человека — ПРОПУСТИ этот блок. Опиши ТОЧНО как в видео: возраст, пол, телосложение, лицо, волосы, одежда, аксессуары. НЕ ЗАМЕНЯЙ.' : `ВСТАВЬ ДОСЛОВНО: ${charB.prompt_tokens?.character_en || charB.appearance_ru || charB.name_ru}. Wardrobe: ${charB.identity_anchors?.wardrobe_anchor || '—'}. Signature: ${charB.identity_anchors?.signature_element || '—'}.`} Hyper-realism: visible pores, birthmarks, wrinkles, sweat. Поза ТОЧНО как в оригинале.]\n\n[БЛОК 5 — ДЕЙСТВИЕ ПО ТАЙМКОДАМ (150-200 слов): Используй original_video_analysis.action_timeline — ПОКАДРОВАЯ КОПИЯ каждого действия, жеста, поворота головы, наклона, шага из оригинала. Таймкоды = РЕАЛЬНАЯ длительность оригинального видео (НЕ ХАРДКОДЬ 8 секунд если видео длиннее/короче). Каждую секунду опиши: кто что делает, куда смотрит, что говорит, какая мимика. Perfect syllable-level lip-sync с original_video_analysis.dialogue_transcription. Если в видео есть паузы — сохрани паузы. Если есть ускорение — сохрани ускорение.]\n\n[БЛОК 6 — TECH + NEGATIVE (40-50 слов): 9:16 vertical. Длительность = original_video_analysis.action_timeline (РЕАЛЬНАЯ длительность оригинала). RAW smartphone footage: sensor noise ISO 800-1600, JPEG artifacts, imperfect auto white-balance, blown highlights +0.5-1.5EV. Room tone и фоновые звуки из оригинала (original_video_analysis.mood). NO text, NO subtitles, NO captions, NO watermarks, NO UI, NO REC, NO smooth skin, NO studio lighting, NO CGI, NO plastic skin, NO AI artifacts.]"` : ''}${product_info?.description_en || ctx.hasProductImage ? `,
...
═══ ПРИОРИТЕТ №3: ПОКАДРОВЫЙ ВИЗУАЛЬНЫЙ АНАЛИЗ ═══
1) СЦЕНА: КОНКРЕТНОЕ место (не "barn" а "бревенчатый сруб с голубыми наличниками"). Каждый предмет, каждая деталь фона.
2) ПЕРСОНАЖИ: ТОЧНОЕ описание каждого человека — возраст, пол, одежда (цвет, материал, фасон), причёска, аксессуары, поза.
3) КАМЕРА: ТОЧНЫЙ ракурс — selfie/со стороны/сверху? Крупный/средний/общий? Статичная/с рук/на штативе?
4) ДЕЙСТВИЯ: что делает КАЖДЫЙ человек в КАЖДУЮ секунду? Жест, шаг, наклон, поворот — покадрово.
5) МИМИКА: выражения лиц — когда улыбка, когда удивление, когда смех, когда брови вверх.
6) СВЕТ: ТОЧНОЕ направление (слева/справа/сверху/сзади), температура (тёплый золотой час / холодный), тени.
7) ЗВУК: фоновые шумы (ветер, птицы, скрип, шаги, музыка).
8) ТЕМП: длительность каждой реплики в секундах, длительность пауз, момент панчлайна.
9) ЭНЕРГИЯ: эмоциональная кривая — спокойное начало → нарастание → кульминация → взрыв/смех.
10) КОЛИЧЕСТВО ЛЮДЕЙ: сколько людей в кадре? Кто говорит? Кто молчит? Кто входит/выходит?
