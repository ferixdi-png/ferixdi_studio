...
ФОРМАТ ОТВЕТА — строго JSON:
{
  ...
  "dialogue_B_ru": "СТРОГО 6-12 слов, макс 1 символ |, killer word ПОСЛЕДНЕЕ (если нет добивки), НЕ начинай с Зато",
...
app.post('/api/trends', authMiddleware, async (req, res) => {
  ...
  // Автоматический календарь событий и праздников РФ
  const calendarHints = [];
  // Крупные праздники и события (формат: [месяц, день, диапазон_дней_до, описание])
  const calendar = [
    [1, 1, 3, 'Новый год, каникулы, оливье, подарки, итоги года'],
    [1, 7, 2, 'Рождество, православные традиции'],
    [1, 25, 2, 'Татьянин день, день студента'],
    [2, 14, 3, 'День святого Валентина, отношения, подарки'],
    [2, 23, 5, 'День защитника Отечества, мужской праздник, подарки мужчинам'],
    [3, 8, 5, '8 Марта, женский день, подарки, цветы, поздравления'],
    [2, 20, 10, 'Масленица (конец февраля — начало марта), блины, зима vs весна, чучело, прощёное воскресенье'],
    [3, 1, 3, 'Масленица (конец февраля — начало марта), блины, зима vs весна, чучело, прощёное воскресенье'],
    [4, 1, 2, 'День дурака, розыгрыши, приколы'],
    [4, 12, 2, 'День космонавтики'],
    [5, 1, 3, 'Первомай, майские праздники, дача, шашлыки'],
    [5, 9, 5, 'День Победы, память, парад'],
    [6, 1, 2, 'День защиты детей, начало лета'],
    [6, 12, 2, 'День России'],
    [9, 1, 5, '1 сентября, школа, дети, учёба, линейка'],
    [10, 1, 3, 'День пожилого человека, бабушки и дедушки'],
    [10, 5, 3, 'День учителя'],
    [11, 4, 3, 'День народного единства'],
    [12, 31, 7, 'Новый год, подготовка, ёлки, подарки, итоги года'],
  ];

  // Сезонные контексты
  const seasonCtx = month >= 3 && month <= 5 ? 'весна — огород, дача, аллергия, смена гардероба, ремонт'
    : month >= 6 && month <= 8 ? 'лето — отпуск, дача, жара, дети на каникулах, шашлыки'
    : month >= 9 && month <= 11 ? 'осень — школа, урожай, простуда, тёмные вечера, осенняя депрессия'
    : 'зима — холод, Новый год, каникулы, снег, морозы, горячий чай';

  for (const [m, d, range, desc] of calendar) {
    const diff = (m === month) ? d - day : -999;
    if (diff >= -1 && diff <= range) {
      calendarHints.push(desc);
    }
  }
  const calendarBlock = calendarHints.length > 0
    ? `\n🗓 БЛИЖАЙШИЕ ПРАЗДНИКИ/СОБЫТИЯ: ${calendarHints.join('; ')}\nВключи 2-3 темы связанные с этими событиями — они дают МАКСИМАЛЬНЫЙ охват в Reels!`
    : '';

  const prompt = `ДАТА: ${weekday}, ${today}
СЕЗОН: ${seasonCtx}${calendarBlock}

Ты — продюсер вирусного контента для Instagram Reels в России. Твоя задача — дать автору 10 КОНКРЕТНЫХ идей для 8-секундных AI-видео, которые ЗАЛЕТЯТ ИМЕННО СЕГОДНЯ.

═══ ПРАВИЛО №1 — ПРИВЯЗКА К СЕГОДНЯШНЕМУ ДНЮ ═══
Каждая идея должна быть актуальна ИМЕННО на ${today}:
• Праздник/событие этой недели → обыграй его (но НЕ банально «с праздником!», а с КОНФЛИКТОМ и ЮМОРОМ)
• Сезонная боль → что БЕСИТ людей прямо сейчас (цены, погода, очереди, пробки)
• Вирусная новость за последние 48 часов → что обсуждают в чатах и комментариях
• Мем дня → что шарят в телеграм-каналах и тиктоке

═══ ПРАВИЛО №2 — НЕ БАНАЛЬНЫЕ ТЕМЫ, А ВИРУСНЫЕ КОНФЛИКТЫ ═══
❌ ПЛОХО: «Масленица» (просто название праздника — скучно, 0 вирусности)
✅ ХОРОШО: «Бабка сожгла чучело зимы — а отопление не включили» (конфликт + боль + юмор)

❌ ПЛОХО: «8 Марта» (банальщина)
✅ ХОРОШО: «Муж подарил сковородку на 8 Марта — жена вручила ему чемодан» (конфликт + узнаваемость)

❌ ПЛОХО: «Цены растут» (общее место)
✅ ХОРОШО: «Бабка купила 3 яйца и попросила рассрочку на кассе» (конкретика + абсурд)

Формула вирусного видео: УЗНАВАЕМАЯ СИТУАЦИЯ + АБСУРДНЫЙ ПОВОРОТ + БОЛЬНАЯ ТЕМА = РЕПОСТЫ

═══ ПРАВИЛО №3 — КАЖДАЯ ИДЕЯ = ГОТОВЫЙ СЦЕНАРИЙ ═══
Не просто «тема», а КОНКРЕТНАЯ сцена: кто A, кто B, что говорит A, что отвечает B, какой панчлайн.
Зритель должен УЗНАТЬ себя, засмеяться и СКИНУТЬ видео в чат.

═══ ИСТОЧНИКИ (что анализировать) ═══
• Что обсуждают в родительских чатах, рабочих чатах, семейных группах
• Яндекс.Новости, РИА — что вызвало волну комментариев
• Telegram-каналы: Mash, Shot, Baza, Лентач, Двач — что завирусилось
• TikTok/Reels тренды в РФ
• Сезонные боли: что БЕСИТ людей именно в ${weekday} ${today}

═══ ИСКЛЮЧИ ═══
• Спортивные матчи и счёт
• Фамилии без контекста
• Погоду (если нет комедийного угла)
• Политику без юмора
• Банальные поздравления без конфликта

═══ ФОРМАТ КАЖДОЙ ИДЕИ ═══
1. topic — цепляющее название (3-8 слов, как заголовок Reels)
2. why_trending — почему это горячо ИМЕННО СЕГОДНЯ ${today} (1 предложение)
3. comedy_angle — как обыграть: конкретная ситуация с конфликтом двух персонажей
4. example_idea — ГОТОВЫЙ мини-сценарий: «A (бабка): "реплика" → B (дед): "реплика с панчлайном"» — 6-10 слов на реплику
5. virality — оценка от 1 до 10 (8+ = будут репостить, 6-7 = посмотрят, <6 = не бери)

ФОРМАТ — строго JSON массив:
[
  {
    "topic": "цепляющая тема",
    "why_trending": "почему горячо именно сегодня",
    "comedy_angle": "конкретный конфликт двух персонажей",
    "example_idea": "A (бабка): 'реплика 6-10 слов' → B (дед): 'панчлайн 6-12 слов'",
    "virality": 8
  }
]

ВАЖНО: Минимум 3 темы должны быть с вирусностью 8+. Если нет горячих новостей — бери СЕЗОННЫЕ БОЛИ которые бесят людей прямо сейчас.
