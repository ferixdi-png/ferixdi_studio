...
function initPromoCode() {
  ...
  // Show saved state
  if (isPromoValid()) {
    status.innerHTML = '<span class="neon-text-green">? �����-��� �������</span>';
    input.placeholder = '��������';
  ...
  btn.addEventListener('click', async () => {
    ...
    if (!key) { status.innerHTML = '<span class="text-red-400">������� �����-���</span>'; return; }
    ...
    if (hash === _PH) {
      ...
      status.innerHTML = '<span class="text-red-400">? �������� �����-���</span>';
      log('WARN', '�����', '�������� �����-���');
    }
    btn.disabled = false;
    btn.textContent = '������������';
...
function initAccountSystem() {
  ...
  loginBtn?.addEventListener('click', async () => {
    ...
    if (!username || !password) { statusEl.innerHTML = '<span class="text-red-400">��������� ��� ����</span>'; return; }
    ...
    try {
      ...
      if (resp.ok && data.jwt) {
        ...
        statusEl.innerHTML = '<span class="neon-text-green">? ���� ��������!</span>';
        log('OK', '�������', `����: ${data.username}`);
        updateAccountUI();
      } else {
        statusEl.innerHTML = `<span class="text-red-400">${data.error || '������ �����'}</span>`;
      }
    } catch { statusEl.innerHTML = '<span class="text-red-400">������ ����������</span>'; }
    loginBtn.disabled = false; loginBtn.textContent = '�����';
  });
  ...
  registerBtn?.addEventListener('click', async () => {
    ...
    if (!username || !password) { statusEl.innerHTML = '<span class="text-red-400">��������� ��� ����</span>'; return; }
    if (!promoHash) { statusEl.innerHTML = '<span class="text-red-400">������� ����������� �����-��� ����</span>'; return; }
    ...
    registerBtn.disabled = false; registerBtn.textContent = '������� �������';
  });
...
function initApp() {
  ...
  Promise.all(loadPromises).then(() => {
    ...
    log('OK', '������������������', `Initial load completed in ${loadTime.toFixed(2)}ms`);
  });
...
async function loadServerCustomLocations() {
  try {
    ...
    if (!resp.ok) { log('WARN', 'GH-�������', `������ ������� ${resp.status}`); return; }
    const serverLocs = await resp.json();
    if (!Array.isArray(serverLocs)) return;
    if (!serverLocs.length) { log('INFO', 'GH-�������', '0 ���������������� ������� �� �������'); return; }
    ...
    log('OK', 'GH-�������', `? ${serverLocs.length} �� �������, ${added} ����� ���������${names.length ? ': ' + names.join(', ') : ''}`);
  } catch (e) {
    log('WARN', 'GH-�������', '������ ���������� � ���������� ��������� ���');
  }
}
...
function renderLocations(filterGroup = '') {
  ...
  ` + locs.map(l => {
    ...
    <div class="loc-card ${sel ? 'selected ring-2 ring-violet-500' : ''}" data-loc-id="${l.id}">
      ...
      <button class="select-loc w-full py-2 rounded-lg text-[11px] font-bold transition-all border ${sel ? 'bg-violet-600 text-white border-violet-500 shadow-lg shadow-violet-500/20' : 'bg-violet-600/10 text-violet-300 border-violet-500/20 hover:bg-violet-600/25 hover:border-violet-500/40'}" data-loc-id="${l.id}">${sel ? '? �������' : '?? �������'}</button>
      <button class="copy-loc-prompt text-[9px] px-2 py-1 rounded-md font-medium transition-all bg-gold/10 text-gold hover:bg-gold/20 border border-gold/30 w-full mt-1.5 flex items-center justify-center gap-1" data-id="${l.id}" title="����������� ���������������� ������ ��� Veo">
        <span>??</span> ������
      </button>
      ${l._custom ? `<button onclick="deleteCustomLoc('${l.id}')" class="text-[9px] px-2 py-1 rounded-md font-medium bg-red-500/10 text-red-400 hover:bg-red-500/20 border border-red-500/20 w-full mt-1 flex items-center justify-center gap-1">?? �������</button>` : ''}
    </div>`;
  }).join('');
...
function updateLocationInfo() {
  ...
  info.innerHTML = `<div class="flex items-center gap-2 flex-wrap"><span class="text-violet-400 font-medium">?? ${loc.name_ru}</span>${tags}<button onclick="deselectLocation()" class="text-[10px] text-red-400/60 hover:text-red-400 transition-colors ml-1" title="�������� �������">? ��������</button></div><div class="text-[10px] text-gray-500 mt-1">${loc.tagline_ru}</div>`;
}
...
function initLocationPicker() {
  document.getElementById('loc-grid')?.addEventListener('click', (e) => {
    ...
    renderLocations(document.getElementById('loc-group-filter')?.value || '');
    renderLocationsBrowse(document.getElementById('loc-browse-group-filter')?.value || '');
    log('INFO', '�������', state.selectedLocation ? `�������: ${state.locations.find(l => l.id === state.selectedLocation)?.name_ru}` : '����-�����');
    updateProgress();
    _scheduleDraftSave();
  });
...
const EN_RU_DICT = {
  // hook_style
  'thrusts phone screen at camera': '����� ������� �������� � ������',
  'slams palm flat on table': '������� ������� �� �����',
  'slow deliberate head turn toward camera': '��������� ������� ������ � ������',
  'adjusts glasses and peers over them': '���������� ���� � ������� ������',
  'points finger directly at camera': '����� ������� ����� � ������',
  'leans forward conspiratorially': '����������� ����� ��������������',
  'crosses arms and raises one eyebrow': '���������� ���� � ��������� �����',
  'waves dismissively': '������������ ����������������',
  'grabs other person by sleeve': '������� ������� �� �����',
  'raises both hands in disbelief': '��������� ��� ���� � ����������',
  'slaps own knee': '������� ���� �� ������',
  'wags finger at camera': '������ ������� � ������',
  'dramatic gasp with hand on chest': '������������� ����� � ����� �� �����',
  'leans back and squints': '������������ ����� � �������',
  'rubs hands together': '�������� ����',
  'snaps fingers': '������� ��������',
  'taps temple knowingly': '������ �� ����� �� ������� ����',
  'pulls out phone dramatically': '������ ������� � ������',
  'shakes head slowly': '�������� ������ �������',
  'claps once loudly': '���� ������� ������',
  // laugh_style
  'wheezing cackle that turns into cough': '�������� ����� ����������� � ������',
  'grudging one-sided smirk': '��������� ������� ����� ������� ���',
  'explosive belly laugh shaking whole body': '�������� ����� �� ������, ������� �� ����',
  'silent shoulder shake with closed eyes': '���������� ������ ������� � ��������� �������',
  'quiet chuckle': '����� ������',
  'loud burst': '������� ����� �����',
  'snort laugh': '��������� ����',
  'giggle behind hand': '��������� �� �������',
  ...
  'cackle': '����������� �����',
  // signature_element
  'turquoise clip-on earrings': '��������� ������-������',
  'reading glasses dangling on beaded cord': '���� ��� ������ �� �������� �������',
  'bright hand-knitted shawl draped over shoulders': '����� ������� ���� �� ������',
  'vintage gold-rimmed spectacles on chain': '��������� ���� � ������� ������ �� �������',
  'gold dental crown': '������� �������',
  'amber pendant': '�������� �����',
  'flat cap': '�����-������������',
  'bold earrings': '������� ������',
  'pearl stud earrings': '��������� ������-��������',
  // micro_gesture
  'dramatic hand wave with spread fingers': '����������� ����� ����� � �������������� ��������',
  'arms crossed with slow disapproving nod': '���� ��������, ��������� ��������������� �����',
  'finger jabbing the air like conductor\'s baton': '����� ������� � ������ ��� ���������� ��������',
  'slow head shake': '��������� ����������� �������',
  'dramatic hand wave': '����������� ����� �����',
  'grins deliberately to flash gold teeth as punctuation': '������� ��������, ��������� ������� ����',
};
...
async function loadCharacters() {
  ...
  if (cached && cacheTime && (now - parseInt(cacheTime)) < 3600000) {
    try {
      ...
      log('OK', '������', `��������� ${state.characters.length} ���������� �� ����`);
      populateFilters();
      renderCharacters();
      // Background refresh
...
async function loadServerCustomCharacters() {
  try {
    ...
    if (!serverChars.length) { log('INFO', 'GH-���������', '0 ���������������� ���������� �� �������'); return; }
    ...
    log('OK', 'GH-���������', `? ${serverChars.length} �� �������, ${added} ����� ���������${names.length ? ': ' + names.join(', ') : ''}`);
  } catch (e) {
    log('WARN', 'GH-���������', '������ ���������� � ���������� ��������� ���');
  }
}
...
function renderCharacters(filter = {}) {
  ...
  grid.innerHTML = _slice.map(c => {
    ...
    const compatRu = { meme: '���', conflict: '��������', chaotic: '����', calm: '���������', balanced: '������' };
    const paceRu = { fast: '�������', normal: '�������', slow: '���������' };
    ...
    <div class="char-card ${selCls}" data-id="${c.id}">
      ...
      <div class="text-[10px] text-gray-500 mb-2 flex flex-wrap gap-x-2">
        ...
        <span>?? ��� ${c.swear_level}/3</span>
        <span>${c.role_default === 'A' ? '???' : '???'} ${c.role_default === 'A' ? '����������' : '��������'}</span>
      </div>

      <!-- Select buttons � large & clear -->
      <div class="flex gap-2 mb-2">
        <button class="select-a flex-1 py-2.5 rounded-lg text-[12px] font-bold transition-all border ${isA ? 'bg-violet-600 text-white border-violet-500 shadow-lg shadow-violet-500/20' : 'bg-violet-600/10 text-violet-300 border-violet-500/20 hover:bg-violet-600/25 hover:border-violet-500/40'}" data-id="${c.id}">${isA ? '? ������ A' : '??? ������� A'}</button>
        <button class="select-b flex-1 py-2.5 rounded-lg text-[12px] font-bold transition-all border ${isB ? 'bg-indigo-600 text-white border-indigo-500 shadow-lg shadow-indigo-500/20' : 'bg-indigo-600/10 text-indigo-300 border-indigo-500/20 hover:bg-indigo-600/25 hover:border-indigo-500/40'}" data-id="${c.id}">${isB ? '? ������ B' : '??? ������� B'}</button>
      </div>

      <!-- Copy Prompt Button -->
      <button class="copy-char-prompt text-[10px] px-2 py-1.5 rounded-md font-medium transition-all bg-gold/10 text-gold hover:bg-gold/20 border border-gold/30 w-full flex items-center justify-center gap-1" data-id="${c.id}" title="����������� ���������������� ������ ��� Veo">
        <span>??</span> ���������� ������
      </button>
      ${c._custom ? `<button onclick="deleteCustomChar('${c.id}')" class="text-[10px] px-2 py-1 rounded-md font-medium bg-red-500/10 text-red-400 hover:bg-red-500/20 border border-red-500/20 w-full mt-1 flex items-center justify-center gap-1">?? �������</button>` : ''}

      <!-- Expandable detail -->
      <details class="group">
        <summary class="text-[11px] text-gray-500 cursor-pointer hover:text-gray-300 transition-colors select-none">��������� ?</summary>
        <div class="mt-2 space-y-2.5 text-[11px] border-t border-gray-800/60 pt-2.5">

          ${c.vibe_archetype ? `<div class="mb-1.5"><span class="text-violet-400 font-medium">?? �������:</span> <span class="text-gray-200 font-medium">${c.vibe_archetype}</span></div>` : ''}

          ${c.speech_style_ru ? `<div><span class="text-violet-400 font-medium">?? ����:</span> <span class="text-gray-300">${c.speech_style_ru}</span></div>` : ''}

          ${anchors.signature_element ? `<div><span class="text-violet-400 font-medium">? �����:</span> <span class="text-gray-300">${translateEnRu(anchors.signature_element)}</span></div>` : ''}

          ${anchors.micro_gesture ? `<div><span class="text-violet-400 font-medium">?? ����:</span> <span class="text-gray-300">${translateEnRu(anchors.micro_gesture)}</span></div>` : ''}

          ${c.modifiers?.hook_style ? `<div><span class="text-violet-400 font-medium">?? ���:</span> <span class="text-gray-300">${translateEnRu(c.modifiers.hook_style)}</span></div>` : ''}
          ${c.modifiers?.laugh_style ? `<div><span class="text-violet-400 font-medium">?? ����:</span> <span class="text-gray-300">${translateEnRu(c.modifiers.laugh_style)}</span></div>` : ''}

          <div class="mt-2">
            <div class="text-violet-400 font-medium mb-1">?? ���������:</div>
    ...
    <div id="char-load-more" class="col-span-full text-center py-3">
      <button onclick="loadMoreCharacters()" class="text-[11px] text-cyan-400 hover:text-cyan-300 border border-cyan-500/20 rounded-lg px-5 py-2 hover:bg-cyan-500/10 transition-colors">
        ��� ��������� (${chars.length - _VS_PAGE} �� ${chars.length})
      </button>
    </div>` : '');
...
function loadMoreCharacters() {
  ...
  tempDiv.innerHTML = newChars.map(c => {
    ...
    const paceRu = { fast: '�������', normal: '�������', slow: '���������' };
    const anchors = c.identity_anchors || {};
    return `<div class="char-card ${selCls}" data-id="${c.id}"><div class="flex items-center justify-between mb-1"><div class="flex items-center gap-2 min-w-0">${getAvatarImg(c.id)}<span class="text-sm font-bold text-white truncate">${c.numeric_id ? `<span class="text-[10px] text-gray-500 font-mono mr-1">#${c.numeric_id}</span>` : ''}${c.name_ru}</span></div><span class="tag text-[10px] ${tagCls} flex-shrink-0">${compatRu[c.compatibility] || c.compatibility}</span></div>${c.tagline_ru ? `<div class="text-[11px] text-violet-300/90 mb-1.5 leading-snug">${c.tagline_ru}</div>` : ''}<div class="text-[10px] text-gray-500 mb-2 flex flex-wrap gap-x-2"><span>?? ${c.group}</span><span>? ${paceRu[c.speech_pace] || c.speech_pace}</span><span>?? ��� ${c.swear_level}/3</span><span>${c.role_default === 'A' ? '???' : '???'} ${c.role_default === 'A' ? '����������' : '��������'}</span></div><div class="flex gap-2 mb-2"><button class="select-a flex-1 py-2.5 rounded-lg text-[12px] font-bold transition-all border ${isA ? 'bg-violet-600 text-white border-violet-500' : 'bg-violet-600/10 text-violet-300 border-violet-500/20 hover:bg-violet-600/25'}" data-id="${c.id}">${isA ? '? ������ A' : '??? ������� A'}</button><button class="select-b flex-1 py-2.5 rounded-lg text-[12px] font-bold transition-all border ${isB ? 'bg-indigo-600 text-white border-indigo-500' : 'bg-indigo-600/10 text-indigo-300 border-indigo-500/20 hover:bg-indigo-600/25'}" data-id="${c.id}">${isB ? '? ������ B' : '??? ������� B'}</button></div><button class="copy-char-prompt text-[10px] px-2 py-1.5 rounded-md font-medium bg-gold/10 text-gold hover:bg-gold/20 border border-gold/30 w-full flex items-center justify-center gap-1" data-id="${c.id}"><span>??</span> ���������� ������</button></div>`;
  }).join('');
  ...
  if (hasMore) {
    ...
    newBtn.innerHTML = `<button onclick="loadMoreCharacters()" class="text-[11px] text-cyan-400 hover:text-cyan-300 border border-cyan-500/20 rounded-lg px-5 py-2 hover:bg-cyan-500/10 transition-colors">��� ��������� (${remaining} �� ${state._charRenderAll.length})</button>`;
    grid.appendChild(newBtn);
  }

  log('OK', 'LAZY', `���������: �������� ${state._charRenderPage}, �������� ${slice.length}/${state._charRenderAll.length}`);
}
...
function selectChar(role, id) {
  ...
  updateCharDisplay();
  renderCharacters(getCurrentFilters());
  log('INFO', '���������', `${role}: ${char.name_ru} (${char.compatibility})`);
  updateReadiness();
  _scheduleDraftSave();
}
...
function autoSelectCharactersForCategory(categoryRu, topicRu = '') {
  ...
  const preferredGroups = categoryHints[categoryRu] || ['�����', '����'];
  ...
  log('OK', '����������', `�������: ${charA.name_ru} ? ${charB.name_ru} ��� ��������� "${categoryRu}"`);
  return true;
}
...
function updateCharDisplay() {
  ...
  if (state.selectedA && state.selectedB) {
    ...
    let label = '���������������� ����';
    if (combos.includes('chaotic') && combos.includes('calm')) label = '?? �������� ����!';
    else if (combos.every(c => c === 'meme')) label = '?? ���-����';
    else if (combos.every(c => c === 'conflict')) label = '? ��������!';
    else if (combos.includes('chaotic')) label = '?? ����!';
    if (badge) { badge.classList.remove('hidden'); badge.querySelector('.tag').textContent = label; }
  } else {
    if (badge) badge.classList.add('hidden');
  }

  // Show/hide "�����" button � show when at least A is selected
  const goBtn = document.getElementById('btn-go-generate');
  if (goBtn) {
    if (state.selectedA) {
      goBtn.classList.remove('hidden');
      goBtn.textContent = state.selectedB ? '����� > ������� � ������ �������' : '����� > ����-����� (��� B)';
    } else {
...
function updateSmartMatch() {
  ...
  if (charA && charB) {
    ...
    details.push({ label: '?? ����� ����', value: chemScore.score, text: chemScore.text });
  ...
  if (topic && (charA || charB)) {
    ...
    details.push({ label: '?? ���� + ���������', value: topicScore.score, text: topicScore.text });
  ...
  if (loc && (charA || charB)) {
    ...
    details.push({ label: '?? ������� + ���������', value: locScore.score, text: locScore.text });
  ...
  if (charA && charB) {
    ...
    details.push({ label: '?? ������ �����', value: roleScore.score, text: roleScore.text });
...
function calcPairChemistry(a, b) {
  ...
  if (greatCombos[c] !== undefined) { score += greatCombos[c]; text = '�������� ������ ������ �������'; }
  else if (okCombos[c] !== undefined) { score += okCombos[c]; text = '���������� ���������, ��������'; }
  else if (weakCombos[c] !== undefined) { score += weakCombos[c]; text = '���������� ����� � ���� ���������'; tip = '�������� ���� � ������������ ������� (����+���������, ���+��������)'; }
  else { score += 10; text = '����������� ���������'; }

  // Speech pace contrast bonus
  if (a.speech_pace !== b.speech_pace) { score += 10; text += ', ���� ���� �����������'; }
...
function calcTopicRelevance(topic, charA, charB) {
  ...
  if (total === 0) return { score: 60, text: '�� ������� ���������', tip: '' };
  ...
  if (matched === total) {
    ...
    if (weak) tip = `${weak.name_ru} (${weak.group}) �� ����� ������ � ����� �${topic.slice(0, 30)}...� � �� AI ����� �������� ��������`;
  } else {
    ...
    if (groups.length > 0) {
      tip = `��� ���� �${topic.slice(0, 25)}...� ����� ��������: ${groups.slice(0, 3).join(', ')}`;
    } else {
      tip = '���� ������������� � ����� ��������� ��������, �� �������� ������ ������';
      score = 60;
      text = '������������� ���� � �������� ����� ���������';
...
function calcLocationMatch(loc, charA, charB) {
  ...
  const chars = [charA, charB].filter(Boolean);
  if (chars.length === 0) return { score: 60, text: '�� ������� ���������', tip: '' };
  ...
  if (matched === chars.length) {
    ...
    score = 35 + Math.floor(Math.random() * 15);
    text = `��������� ��������� ��� ${loc.name_ru}`;
    tip = `${loc.name_ru} ������ �������� ���: ${affinity.slice(0, 3).join(', ')} � �� �������� ��������� �� �� ���� ����� ���� ������!`;
  }
...
function autoSelectRandomPair() {
  ...
  updateCharDisplay();
  log('INFO', '����������', `��������� ����: ${chars[idxA].name_ru} ? ${chars[idxB].name_ru}`);
  return true;
}
...
function navigateTo(section) {
  ...
  if (!_lazyInited.has(section)) {
    ...
    if (lazyFn) {
      try { lazyFn(); log('OK', 'LAZY', `������ �${section}� ����������������`); }
...
function initNavigation() {
  ...
  // "�����" button on characters > go to generate (step 4: location + generate)
  document.getElementById('btn-go-generate')?.addEventListener('click', () => {
    if (!state.selectedA) {
      showNotification('?? ������� �������� ���� �� ������ ��������� (A)', 'warning');
      return;
    }
    navigateTo('generate');
  });

  // "�����" button on content > go to characters
...
function selectGenerationMode(mode) {
  ...
  if (display && nameEl && continueBtn) {
    ...
    continueBtn.innerHTML = `<span>����� > ������� �������</span><span>></span>`;
...
async function loadTrendingIdeas() {
  ...
  // Show loading state
  grid.innerHTML = '<div class="text-xs text-gray-500 text-center">?? ��������� ���������� ����...</div>';
  ...
  try {
    ...
    if (data.trends && data.trends.length > 0) {
      grid.innerHTML = data.trends.map((trend, i) => `
        <div class="glass-panel p-3 border-l-2 border-emerald-500/40 cursor-pointer hover:bg-emerald-500/5 transition-all trending-idea-card" data-trend="${trend.topic}">
          ...
          <div class="text-[10px] text-gray-500 mt-1">${trend.viral_score}% ����������</div>
        </div>
      `).join('');
    ...
    grid.innerHTML = '<div class="text-xs text-red-400 text-center">? ������ �������� ����</div>';
...
async function loadTrendingIdeasMain() {
  ...
  grid.innerHTML = '<div class="text-xs text-gray-500 text-center py-3">?? ��������� ���������� ����...</div>';
...
function selectTrendingIdea(topic) {
  ...
  if (grid) {
    ...
    notification.textContent = `? ������� ����: ${topic}`;
...
function getCharacterRecommendations(topicText) {
  ...
  // ��� � ����������
  if (topicLower.includes('���') || topicLower.includes('����������') || topicLower.includes('���������') || 
      topicLower.includes('����') || topicLower.includes('����') || topicLower.includes('�����')) {
    recommendations.push(
      { id: 'babka_zina', reason: '������ ��������� � �������� ��� ��� ��� ����� � ������' },
      { id: 'babka_valya', reason: '������ ������ � ��������� ���� � �����������' },
      { id: 'ded_boris', reason: '������ ������ � ��������� ���������� �� ���' },
      { id: 'ded_stepan', reason: '������ � ���������� ������ � ������� ���������' }
    );
  }
  
  // ���� � ��������
  else if (topicLower.includes('����') || topicLower.includes('������') || topicLower.includes('��������') || 
             topicLower.includes('�������')) {
    recommendations.push(
      { id: 'babka_zina', reason: '��������� � ������� �� ����� � ��������' },
      { id: 'mama_regina', reason: 'CEO ��������� ����� � �������� �������' },
      { id: 'ded_matvey', reason: 'ٸ���� � ��������� ���������� � �������' },
      { id: 'papa_slava', reason: '��������� � ������ ���� �� ��������' }
    );
  }
  ...
  // ������ ���������
    ...
    recommendations.push(
      { id: 'babka_zina', reason: '������������ ����� � �������� ���������' },
      { id: 'ded_fyodor', reason: '���������� ��� � �������� � �������' },
      { id: 'doch_yana', reason: '����-���� � �������� �����' },
      { id: 'papa_artyom', reason: '������� � ������� � ����������� ����' }
    );
  }
  ...
  // �������� � �����������
             ...
             topicLower.includes('��������')) {
    recommendations.push(
      { id: 'mama_lyuba', reason: '�������� � �������� ��������' },
      { id: 'mama_alyona', reason: '������� ��������� � ������� ������ � ��������' },
      { id: 'papa_oleg', reason: '��������� � ������� ������ � ��������' },
      { id: 'ded_zakhar', reason: '������� ���� � ����� ��� ��������' }
    );
  }
  ...
  // ���� � ������
    ...
    recommendations.push(
      ...
      { id: 'ded_stepan', reason: '������ � ������������ � ����' },
      { id: 'mama_lyuba', reason: '�������� � ������ ��������' },
      { id: 'papa_kostya', reason: '����� � ���������� ������ �� ����' }
    );
  }
  
  // AI � ����������
  else if (topicLower.includes('��������') || topicLower.includes('ai') || topicLower.includes('��������') || 
             topicLower.includes('�����')) {
    recommendations.push(
      { id: 'papa_oleg', reason: '��������� � ������� �� �����������' },
      { id: 'papa_artyom', reason: '������� � ����������� �����-������' },
  ...
  return recommendations.slice(0, 4); // �������� 4 ������������
}
...
function updateSurpriseCharSlots() {
  ...
  if (aDisplay) {
    aDisplay.textContent = state.selectedA ? `${state.selectedA.name_ru}` : '�� ������';
    aDisplay.className = state.selectedA ? 'text-[11px] text-white' : 'text-[11px] text-gray-400';
  }
  if (bDisplay) {
    bDisplay.textContent = state.selectedB ? `${state.selectedB.name_ru}` : '�����������';
...
function initModeSwitcher() {
  document.querySelectorAll('#section-advanced .mode-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      updateReadiness();
      log('INFO', '�����', `����: ${mode === 'idea' ? '����' : mode === 'script' ? '������' : '�����'}`);
    });
  });
  ...
  document.getElementById('idea-input')?.addEventListener('paste', (e) => {
    setTimeout(() => {
      ...
      if (text.includes('instagram.com/')) {
        ...
        if (sceneHint && !sceneHint.value) sceneHint.value = `������ �����: ${text}`;
        e.target.value = '';
      }
    }, 100);
  });
  ...
  document.querySelectorAll('.toggle-track').forEach(track => {
    track.addEventListener('click', () => {
      ...
      if (opt && opt in state.options) {
        ...
        log('INFO', '�����', `${opt} = ${state.options[opt]}`);
      }
    });
  });
}
...
function handleVideoFile(file) {
  if (!file.type.startsWith('video/')) { log('WARN', '�����', '�� ���������'); return; }
  const sizeMB = file.size / 1024 / 1024;
  if (sizeMB > 50) {
    showNotification('? ���� ������ 50 ��. ���� ����� �� ��������.', 'error');
    log('WARN', '�����', `���� ������ 50 MB (${sizeMB.toFixed(1)} MB)`);
    return;
  }
  if (sizeMB > 20) {
    showNotification(`?? ������� ����� (${sizeMB.toFixed(1)} ��) � �������� ����� ������. ����������� �� 20 ��.`, 'warning');
    log('WARN', '�����', `������� ����: ${sizeMB.toFixed(1)} MB`);
  }
  ...
  reader.onload = () => {
    ...
    log('OK', '�����', `?? ����� ������������ (${(file.size / 1024 / 1024).toFixed(1)} MB) � ������ � �������`);
  };
  ...
  video.onseeked = () => {
    ...
    log('OK', '�����', `?? ���������: ${file.name} (${state.videoMeta.duration}�) � ������ � �������`);
    updateReadiness();
  };
...
async function autoMatchCast() {
  ...
  const apiBase = localStorage.getItem('ferixdi_api_url') || DEFAULT_API_URL;
  if (!token) { log('WARN', '������', '��� �����������'); if (btn) { btn.disabled = false; btn.textContent = '?? ��������� ���������� � ������� �������������'; } return; }
  ...
  try {
    ...
    if (result.location_id) {
      ...
      if (loc) {
        ...
        reasons.push(`<strong>�������:</strong> ${loc.name_ru} � ${result.location_reason || ''}`);
        log('OK', '������', `�������: ${loc.name_ru}`);
        renderLocations(document.getElementById('loc-group-filter')?.value || '');
        renderLocationsBrowse(document.getElementById('loc-browse-group-filter')?.value || '');
      }
    ...
    if (resultEl && reasons.length) {
      resultEl.innerHTML = '?? <strong>AI ��������:</strong><br>' + reasons.join('<br>');
      resultEl.classList.remove('hidden');
    }
    log('OK', '������', `������ � ${reasons.length} ��������� ���������`);
  } catch (e) {
    log('ERR', '������', `������: ${e.message}`);
  } finally {
...
function initVideoRefImageDropzoneMain() {
  ...
  const handleRefFile = (file) => {
    if (!file.type.startsWith('image/')) { log('WARN', '���', '����� �����������'); return; }
    if (file.size > 10 * 1024 * 1024) { log('WARN', '���', '���� > 10 ��'); return; }
    const reader = new FileReader();
    reader.onload = (e) => {
      ...
      if (nameEl) nameEl.textContent = `? ${file.name} (${(file.size/1024).toFixed(0)} ��)`;
      if (previewEl) previewEl.classList.remove('hidden');
      if (innerEl) innerEl.innerHTML = '<div class="text-[10px] text-violet-400">? �������� ��������</div>';
      log('OK', '���', `��������-����: ${file.name}`);
    };
    reader.readAsDataURL(file);
  };
  ...
  document.getElementById('video-ref-clear-main')?.addEventListener('click', (e) => {
    ...
    document.getElementById('video-ref-inner-main').innerHTML = '<div class="text-base mb-0.5">\uD83D\uDDBC</div><div class="text-[10px] text-gray-500">JPG / PNG � �� 10 ��</div>';
    log('INFO', '���', '��������-���� ������');
  });
}
...
function startRateLimitCountdown() {
  ...
  const update = () => {
    ...
    showGenStatus(`?? �����: 1 ������ � ������. �������� ${remaining}�`, 'text-amber-400');
  };
  update();
...
function updateReadiness() {
  ...
  if (allReady) {
    ...
    btn.innerHTML = '<span class="flex items-center justify-center gap-2">?? ������� ������<span class="text-xs opacity-60">Ctrl+Enter</span></span>';
  } else {
    btn.disabled = true;
    btn.classList.add('opacity-50', 'cursor-not-allowed');
    const missing = [];
    if (!checks.mode) missing.push('�����');
    if (!checks.chars) missing.push('���������');
    if (!checks.content) missing.push('�������');
    if (!checks.promo) missing.push('�����-���');
    btn.innerHTML = `<span class="flex items-center justify-center gap-2">?? �� �������: ${missing.join(', ')}</span>`;
  }
  ...
  _updateCheckItem('readiness-mode', checks.mode,
    state.generationMode ? _modeLabel(state.generationMode) : '����� ���������',
    checks.mode ? '' : '< �������� �� ���� 1',
  ...
  const charsLabel = checks.chars
    ? (isSurpriseAuto && !state.selectedA ? 'AI ������� ����������' : state.selectedB ? `${state.selectedA.name_ru} ? ${state.selectedB.name_ru}` : state.selectedA ? `${state.selectedA.name_ru} (����)` : 'AI ������� ����������')
    : '�������� A (������� 1)';
  _updateCheckItem('readiness-chars', checks.chars,
    charsLabel,
    checks.chars ? (isSurpriseAuto && !state.selectedA ? '���� (AI �������)' : '') : '< �������� �� ���� 3',
  ...
  const locName = locSelected ? (state.locations.find(l => l.id === state.selectedLocation)?.name_ru || '�������') : '���� (AI �������)';
...
function _contentLabel() {
  ...
  if (state.generationMode === 'suggested') {
    ...
    return v ? `"${v.slice(0, 30)}${v.length > 30 ? '...' : ''}"` : 'AI ������� ����';
  }
  if (state.generationMode === 'script') return '������ �����';
  if (state.generationMode === 'video') return state.videoMeta ? `�����: ${state.videoMeta.name}` : '';
  return '';
}

function _modeLabel(m) {
  return { idea: '?? ���� ����', suggested: '?? ������� ����', script: '?? ���� ������', video: '?? �� �����' }[m] || m;
}
...
async function handleProductFile(file) {
  // �������� �����-���� ����� �������� ������
  if (!isPromoValid()) {
    showProductStatus('?? ��� ������� ������ ����� �����-���. ������� ��� � ������� ����������.', 'text-amber-400');
    log('WARN', '�����', '�����-��� �� ����� � ������ ������ ������������');
    return;
  }

  if (!file.type.startsWith('image/')) {
    showProductStatus('����� ���� (JPG, PNG, WebP)', 'text-red-400');
    return;
  }
  if (file.size > 10 * 1024 * 1024) {
    showProductStatus('���� ������� ������� (����. 10 ��)', 'text-red-400');
    return;
  }
  ...
  reader.onload = async (e) => {
    ...
    document.getElementById('product-preview-zone').innerHTML = `
      ...
      <div class="text-[10px] text-gray-500 mt-1">${(file.size / 1024).toFixed(0)} ��</div>
    `;
    ...
    showProductStatus('? AI ����������� �����...', 'text-gray-400');
    ...
    try {
      ...
      if (!resp.ok) {
        showProductStatus(`? ${data.error || '������'}`, 'text-red-400');
        return;
      }
      ...
      document.getElementById('product-tokens').textContent = data.tokens ? `${data.tokens} �������` : '';
      showProductStatus('', 'hidden');

    } catch (e) {
      showProductStatus(`? ������� ������: ${e.message}`, 'text-red-400');
    }
  };
...
function initPostGenPhoto() {
  ...
  document.getElementById('post-photo-mode-ref')?.addEventListener('click', () => {
    ...
    document.getElementById('post-photo-label').textContent = '������� ����-�������� (�����, ����������, ��������)';
...
async function handlePostGenPhoto(file) {
  if (!_postPhotoMode) {
    showPostPhotoStatus('������� ������ ���: �������� ��� �����', 'text-amber-400');
    return;
  }
  if (!isPromoValid()) {
    showPostPhotoStatus('��� ������� ���� ����� �����-���', 'text-amber-400');
  ...
  if (!file.type.startsWith('image/')) {
    showPostPhotoStatus('����� ���� (JPG, PNG, WebP)', 'text-red-400');
    return;
  }
  if (file.size > 10 * 1024 * 1024) {
    showPostPhotoStatus('���� ������� ������� (����. 10 ��)', 'text-red-400');
    return;
  }
  ...
  reader.onload = async (e) => {
    ...
    showPostPhotoStatus('AI ����������� ����...', 'text-violet-400 animate-pulse');
    ...
    try {
      ...
      if (!resp.ok) {
        showPostPhotoStatus(`${data.error || '������'}`, 'text-red-400');
        return;
      }
      ...
      log('OK', 'POST-PHOTO', `AI ������ ���� (${_postPhotoMode}): ${data.description_en.slice(0, 80)}...`);

    } catch (err) {
      showPostPhotoStatus(`������� ������: ${err.message}`, 'text-red-400');
    }
  };
  reader.readAsDataURL(file);
}
...
function applyPostGenPhoto() {
  ...
  if (info.mode === 'product') {
    ...
    if (veoProdBadge) {
      ...
      const prodImg = `<img src="data:${info.mime_type};base64,${info.image_base64}" class="w-10 h-10 rounded object-cover border border-emerald-500/30 flex-shrink-0" alt="�����">`;
      const prodDesc = desc.length > 120 ? desc.slice(0, 120) + '...' : desc;
      veoProdBadge.innerHTML = `
        <div class="flex items-start gap-2">
          ${prodImg}
          <div class="min-w-0">
            <div class="text-[10px] font-bold text-emerald-400">?? ����� �������� � ������ ?</div>
            <div class="text-[9px] text-gray-400 leading-tight mt-0.5">${escapeHtml(prodDesc)}</div>
            <div class="text-[9px] text-emerald-500/60 mt-0.5">������ ��� �� ����������� ����</div>
          </div>
        </div>`;
    }

    showPostPhotoStatus('����� �������� �� ��� �������!', 'text-emerald-400');
    log('OK', 'POST-PHOTO', '����� ������� � ��������');
    ...
    showPostPhotoStatus('�������� �������� �� ��� �������!', 'text-violet-400');
    log('OK', 'POST-PHOTO', '�������� ������� � �������� � ������� � state');
  }
  ...
  if (applyBtn) {
    ...
    setTimeout(() => { applyBtn.textContent = '? ��������� � �������'; applyBtn.disabled = false; }, 2000);
  }
}
...
function renderPreflight(localResult) {
  ...
  // Translate risk
  const riskRu = { high: '�������', medium: '�������', low: '������' };

  // Build pillar summaries (short) � user-friendly terms
  const pillars = [
    { icon: '??', name: '���������', val: `${lm.mood} � ${lm.sources || '1 ��������'}`, detail: lm.style?.slice(0, 60) + '...' },
    { icon: '??', name: '������', val: '�����-�����', detail: `��������: ${cin.optics?.focal_length || '24-28��'} � ���������: ${cin.optics?.aperture || 'f/1.9-2.2'}` },
    { icon: '??', name: '������', val: '������ ������', detail: '������������ �����-�������� ��������' },
    { icon: '??', name: '��������', val: '����� � �������', detail: '�������� 3-5� � ������� 3-4� � ����������� ��������' },
    { icon: '??', name: '����', val: '׸���� ����', detail: `������� ?25� � ��������� �� ����` },
    { icon: '??', name: '������', val: '4 ���� �������', detail: `���: ����� � ������ � ������������ �������� ����` },
    { icon: '??', name: '����������', val: `����. ${cin.frame_cleanliness?.detail_budget || '7'} �������`, detail: `60-70% ��������� � ������ 9:16` },
    { icon: '??', name: '�����������', val: '������������ ��������', detail: '����, �������, �������� ����, �����' },
    { icon: '??', name: '����', val: '������������ ����', detail: `��� ���������� � ������ � 5 ��� ����` },
    { icon: '??', name: '����', val: '������ � ��������', detail: `�������� 35-60�� � ��� -20/-30��` },
    { icon: '??', name: '������', val: '����� ���', detail: `�������: ?80% � ������ � ������` },
    { icon: '??', name: '������', val: '��������', detail: `80>90>60>95>100>70% � ����-��������` },
  ];
    ...
    <div class="glass-panel p-5 space-y-4 border-l-2 border-cyan-400/40">
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          ...
          <div>
            <div class="text-xs font-semibold text-cyan-400 tracking-wide">��������� ������</div>
            <div class="text-[10px] text-gray-500">FERIXDI AI �������� ������ �� ����� ����������</div>
          </div>
        </div>
      ...
      <div class="grid grid-cols-2 gap-2">
        <div class="bg-black/30 rounded-lg p-2.5">
          <div class="text-[9px] text-gray-500 uppercase tracking-wider mb-1">���������</div>
          <div class="text-[11px] text-cyan-300">${ctx.soloMode ? (charA.name_ru || 'A') + ' (����)' : (charA.name_ru || 'A') + ' <span class="text-gray-600">?</span> ' + (charB.name_ru || 'B')}</div>
        ...
        <div class="bg-black/30 rounded-lg p-2.5">
          <div class="text-[9px] text-gray-500 uppercase tracking-wider mb-1">�������</div>
          <div class="text-[11px] ${riskColor}">${riskIcon} ${est.total || '8.0'}� � ����: ${riskRu[est.risk] || est.risk || '�'}</div>
        </div>
      </div>

      <!-- Wardrobe -->
      <div class="bg-black/30 rounded-lg p-2.5">
        <div class="text-[9px] text-gray-500 uppercase tracking-wider mb-1.5">��������</div>
      ...
      <div>
        <div class="flex items-center justify-between mb-2">
          <div class="text-[9px] text-gray-500 uppercase tracking-wider">12 ���������� �������� � �������������� ���������</div>
          <button id="preflight-toggle-pillars" class="text-[10px] text-cyan-400/60 hover:text-cyan-400 transition-colors cursor-pointer">���������� ?</button>
        </div>
      ...
      <!-- Engagement preview -->
      <div class="bg-black/30 rounded-lg p-2.5">
        <div class="text-[9px] text-gray-500 uppercase tracking-wider mb-1.5">���������� � Instagram</div>
        <div class="flex gap-3 text-[10px]">
          <div><span class="text-gray-500">���:</span> <span class="text-gray-300">${ctx.hookAction?.action_ru?.slice(0, 30) || '�'}</span></div>
          <div><span class="text-gray-500">��������:</span> <span class="text-gray-300">${ctx.propAnchor?.slice(0, 25) || '�'}</span></div>
        </div>
        <div class="text-[10px] text-gray-500 mt-1">�������: ${localResult.log?.engagement?.hashtag_count || '~18'} �� � ��������� + ������ + ������ �������</div>
      </div>
      ...
      <div id="preflight-status" class="text-center py-2 rounded-lg text-xs font-medium bg-cyan-500/8 text-cyan-400 border border-cyan-500/15">
        <span class="inline-block animate-pulse mr-1">?</span> FERIXDI AI ���������� �������...
      </div>
    </div>
  `;
...
function displayResult(result) {
  ...
  const _tabDefaults = { veo: '?? ������ ��� Veo', photo: '?? ���� (���� 0)', video: '?? ����� JSON', insta: '?? �����', ru: '???? ����', blueprint: '?? ����' };
  ...
  document.getElementById('veo-prompt-text').textContent = result.veo_prompt || '(������ �� ������������)';
  ...
  if (veoProdBadge) {
    ...
    if (pi?.description_en) {
      veoProdBadge.classList.remove('hidden');
      const prodImg = pi.image_base64 ? `<img src="data:${pi.mime_type || 'image/jpeg'};base64,${pi.image_base64}" class="w-10 h-10 rounded object-cover border border-emerald-500/30 flex-shrink-0" alt="�����">` : '';
    ...
    if (hasRef) {
      ...
      veoRefBadge.innerHTML = `
        <div class="flex items-start gap-2">
          ...
          <div class="min-w-0">
            <div class="text-[10px] font-bold text-violet-400">?? �������� ����� � ������� ?</div>
  ...
  if (result.warnings?.length > 0) {
    ...
    const infoWarnings = result.warnings.filter(w => w.includes('��� ���������') || w.includes('�������') || w.includes('���������'));
    const actionWarnings = result.warnings.filter(w => w.includes('������� �������') || w.includes('��������'));
    const systemWarnings = result.warnings.filter(w => w.includes('������') || w.includes('�� ������'));
    ...
    if (infoWarnings.length > 0) {
      warningsHtml += '<div class="mb-2"><div class="text-xs font-semibold text-cyan-400 mb-1">?? ����������:</div>';
      warningsHtml += infoWarnings.map(w => `<div class="text-xs text-cyan-300">?? ${escapeHtml(w)}</div>`).join('');
      warningsHtml += '</div>';
    }
    
    if (actionWarnings.length > 0) {
      warningsHtml += '<div class="mb-2"><div class="text-xs font-semibold text-amber-400 mb-1">?? ��������������:</div>';
      warningsHtml += actionWarnings.map(w => `<div class="text-xs text-amber-300">?? ${escapeHtml(w)}</div>`).join('');
      warningsHtml += '</div>';
    }
    
    if (systemWarnings.length > 0) {
      warningsHtml += '<div class="mb-2"><div class="text-xs font-semibold text-orange-400 mb-1">?? �������:</div>';
      warningsHtml += systemWarnings.map(w => `<div class="text-xs text-orange-300">?? ${escapeHtml(w)}</div>`).join('');
      warningsHtml += '</div>';
    }
    
    if (otherWarnings.length > 0) {
      warningsHtml += '<div class="mb-2"><div class="text-xs font-semibold text-gray-400 mb-1">?? ������:</div>';
  ...
  if (ruTabBtn) ruTabBtn.textContent = '???? ����';
  ...
  if (state._currentSeries) {
    try {
      ...
      if (s) {
        ...
        s.episodes.push(ep);
        saveSeries(series);
        log('OK', 'SERIES', `������ #${s.episodes.length} ������� � ����� "${s.name}"`);
      }
  ...
  log('OK', '���������', `${ver} ����� ������! ������������: ${result.duration_estimate?.total || '?'}�, ����: ${result.duration_estimate?.risk || '?'}`);
  if (result.auto_fixes?.length > 0) {
    result.auto_fixes.forEach(f => log('INFO', '����', f));
  }
}
...
function populateContextBlock(result) {
  ...
  // Meta grid
  metaEl.innerHTML = `
    <div class="bg-black/20 rounded p-2"><span class="text-gray-500">���������:</span> <span class="text-gray-200">${cat.ru || '�'}</span></div>
    <div class="bg-black/20 rounded p-2"><span class="text-gray-500">�������:</span> <span class="text-gray-200">${est.total || '8.0'}� � ${est.risk || '�'}</span></div>
    <div class="bg-black/20 rounded p-2"><span class="text-gray-500">���:</span> <span class="text-gray-200">${ctx.hookAction?.action_ru?.slice(0, 35) || '�'}</span></div>
    <div class="bg-black/20 rounded p-2"><span class="text-gray-500">���������:</span> <span class="text-gray-200">${engage.viral_title?.slice(0, 45) || '�'}${engage.viral_title?.length > 45 ? '...' : ''}</span></div>
  `;
}
...
function populateInstaTab(result) {
  ...
  const copyLabel = isEN ? 'Copy' : '����������';
  const copiedLabel = isEN ? '? Copied' : '? �����������';
  ...
  el.innerHTML = `
    ...
    <div class="glass-panel p-4 relative">
      ...
      <div class="text-[10px] text-amber-400 font-semibold uppercase tracking-wider mb-2">?? ${isEN ? 'Viral Title' : '�������� ���������'}</div>
      <div class="copy-target text-sm text-gray-100 font-medium leading-relaxed">${escapeHtml(viralTitle)}</div>
      <div class="text-[9px] text-gray-600 mt-2">${isEN ? 'Paste as Reels caption � hooks viewers in the feed' : '������ ��� ��������� Reels � ������� � �����'}</div>
    </div>

    <!-- Share Bait (video description for forwarding) -->
    <div class="glass-panel p-4 relative border-l-2 border-orange-400/40">
      <button class="copy-btn" onclick="navigator.clipboard.writeText(this.closest('.glass-panel').querySelector('.copy-target').textContent.trim());this.textContent='${copiedLabel}';setTimeout(()=>this.textContent='${copyLabel}',1500)">${copyLabel}</button>
      <div class="text-[10px] text-orange-400 font-semibold uppercase tracking-wider mb-2">?? ${isEN ? 'Video Description � share bait' : '�������� ����� � ��� ���������'}</div>
      <div class="copy-target text-sm text-gray-100 font-medium leading-relaxed">${escapeHtml(shareBait)}</div>
      <div class="text-[9px] text-gray-600 mt-2">${isEN ? 'Send to a friend with this line � bait for shares' : '����� ����� � ���� ������ � ���� �� ��������� � ��������� �����'}</div>
    </div>

    <!-- Instagram Caption (full post text) -->
    ${instaCaption ? `<div class="glass-panel p-4 relative border-l-2 border-pink-400/40">
      <button class="copy-btn" onclick="navigator.clipboard.writeText(this.closest('.glass-panel').querySelector('.copy-target').textContent.trim());this.textContent='${copiedLabel}';setTimeout(()=>this.textContent='${copyLabel}',1500)">${copyLabel}</button>
      <div class="text-[10px] text-pink-400 font-semibold uppercase tracking-wider mb-2">?? ${isEN ? 'Full Caption (description)' : '������ ����� �������� (caption)'}</div>
      <div class="copy-target text-sm text-gray-100 leading-relaxed">${escapeHtml(instaCaption)}</div>
      <div class="text-[9px] text-gray-600 mt-2">${isEN ? 'Paste as Reels description � includes CTA' : '������ � �������� Reels � ��� � CTA � ������'}</div>
    </div>` : ''}
    ...
    ${instaHookTexts.length > 0 ? `<div class="glass-panel p-4 relative">
      ...
      <div class="text-[10px] text-rose-400 font-semibold uppercase tracking-wider mb-2">?? ${isEN ? 'Hook Texts (on-screen)' : '������-���� (�� ����� � ������)'}</div>
      ...
      <div class="text-[9px] text-gray-600 mt-2">${isEN ? 'Place one of these as text overlay in the first 0.5s' : '������ ���� �� ���� ���� ������� � ������ 0.5 ��� �����'}</div>
    </div>` : ''}

    <!-- Hashtags -->
    <div class="glass-panel p-4 relative">
      <button class="copy-btn" onclick="navigator.clipboard.writeText(this.closest('.glass-panel').querySelector('.copy-target').textContent.trim());this.textContent='${copiedLabel}';setTimeout(()=>this.textContent='${copyLabel}',1500)">${copyLabel}</button>
      <div class="text-[10px] text-cyan-400 font-semibold uppercase tracking-wider mb-2"># ${isEN ? `Hashtags � ${hashtags.length}` : `������� � ${hashtags.length} ��`}</div>
      ...
      ${seriesTag ? `<div class="text-[9px] text-violet-400 mt-2">${isEN ? 'Series' : '�����'}: ${escapeHtml(seriesTag)}</div>` : ''}
      <div class="text-[9px] text-gray-600 mt-1">${isEN ? 'Paste in the first comment or in description' : '������ � ������ ����������� ��� � ��������'}</div>
    </div>

    <!-- Pin Comment (bait for shares) -->
    <div class="glass-panel p-4 relative">
      <button class="copy-btn" onclick="navigator.clipboard.writeText(this.closest('.glass-panel').querySelector('.copy-target').textContent.trim());this.textContent='${copiedLabel}';setTimeout(()=>this.textContent='${copyLabel}',1500)">${copyLabel}</button>
      <div class="text-[10px] text-emerald-400 font-semibold uppercase tracking-wider mb-2">?? ${isEN ? 'Pinned Comment' : '����������� �����������'}</div>
      <div class="copy-target text-sm text-gray-200 leading-relaxed">${escapeHtml(pinComment)}</div>
      <div class="text-[9px] text-gray-600 mt-2">${isEN ? 'Pin this � triggers shares and saves' : '������� � ����������� ��������� � ����������'}</div>
    </div>

    <!-- First Comment -->
    <div class="glass-panel p-4 relative">
      <button class="copy-btn" onclick="navigator.clipboard.writeText(this.closest('.glass-panel').querySelector('.copy-target').textContent.trim());this.textContent='${copiedLabel}';setTimeout(()=>this.textContent='${copyLabel}',1500)">${copyLabel}</button>
      <div class="text-[10px] text-violet-400 font-semibold uppercase tracking-wider mb-2">?? ${isEN ? 'First Comment' : '������ �����������'}</div>
      <div class="copy-target text-sm text-gray-200 leading-relaxed">${escapeHtml(firstComment)}</div>
      <div class="text-[9px] text-gray-600 mt-2">${isEN ? 'Post right after publishing � sparks discussion' : '������ ����� ����� ���������� � ��������� ����������'}</div>
    </div>

    <!-- Engagement Tip -->
    ${instaEngagementTip ? `<div class="glass-panel p-4 relative border-l-2 border-teal-400/40">
      <div class="text-[10px] text-teal-400 font-semibold uppercase tracking-wider mb-2">?? ${isEN ? 'Engagement Tip' : '������� ��� �������'}</div>
    ...
    <div class="bg-gradient-to-r from-violet-500/8 to-cyan-500/8 rounded-lg p-4 border border-violet-500/15">
      <div class="text-[10px] text-violet-400 font-semibold uppercase tracking-wider mb-2">?? ${isEN ? 'Instagram Publishing Order' : '������� ���������� � Instagram'}</div>
      <div class="text-xs text-gray-300 leading-relaxed space-y-1.5">
        ...
        ` : `
        <div>1. <span class="text-amber-300 font-medium">������ ����� ��������</span> > ������ � �������� Reels (caption). ������ ���������, ��� ��������!</div>
        <div>2. <span class="text-gray-200 font-medium">���������</span> Reels</div>
        <div>3. <span class="text-cyan-300 font-medium">�������</span> > ������ ������ ����������� � ��������� (IG �� ����� �����)</div>
        <div>4. <span class="text-emerald-300 font-medium">������</span> > ������ ������ ������� � ������� ��� (����������� �������� �������)</div>
        <div>5. <span class="text-violet-300 font-medium">������ �������</span> > ������ ������ ������� ����� 1-2 ��� (��������� ����������)</div>
        `}
      </div>
      <div class="text-[9px] text-gray-500 mt-3">${isEN ? 'Series' : '�����'}: ${charA.id === charB.id ? (charA.name_ru || 'A') + ' (����)' : (charA.name_ru || 'A') + ' ? ' + (charB.name_ru || 'B')} � ${isEN ? 'use one series tag for all videos' : '��������� ���� �������� ��� �� ��� �����'}</div>
    </div>
  `;
}
...
async function callAIEngine(apiContext) {
  ...
  try {
    ...
    if (e.name === 'AbortError') throw new Error('�������: AI �� ������� �� 90 ������. ���������� �����.');
    throw e;
  } finally {
...
function initGenerate() {
  document.getElementById('btn-generate')?.addEventListener('click', async () => {
    ...
    if (!state.generationMode) {
      showGenStatus('?? ������� �������� ����� ��������� �� ���� 1', 'text-orange-400');
      navigateTo('generation-mode');
      return;
    }
    ...
    if (!state.selectedA && state.generationMode !== 'video') {
      showGenStatus('?? ������� �������� ���� �� ������ ��������� �� ���� 3', 'text-orange-400');
      navigateTo('characters');
      return;
    }
    ...
    if (state.generationMode === 'script') {
      ...
      if (!scriptA && !scriptB) {
        showGenStatus('?? ������ ���� �� ���� ������� (A ��� B)', 'text-orange-400');
        return;
      }
      ...
      if (scriptA && scriptA.split(/\s+/).length > maxWordsA) {
        showGenStatus(`?? ������� A ������� ������� (${scriptA.split(/\s+/).length} ����). ��������: ${maxWordsA} ����`, 'text-orange-400');
        return;
      }
      if (scriptB && scriptB.split(/\s+/).length > maxWordsB) {
        showGenStatus(`?? ������� B ������� ������� (${scriptB.split(/\s+/).length} ����). ��������: ${maxWordsB} ����`, 'text-orange-400');
        return;
      }
    }
    ...
    if (state.generationMode === 'video') {
      ...
      if (sceneHint && sceneHint.length > 200) {
        showGenStatus('?? �������� ����� ������� ������� (�������� 200 ��������). ��������� �����.', 'text-orange-400');
        return;
      }
    }
    ...
    try {
      ...
      showGenStatus(`? ������ ���������: ${e.message}`, 'text-red-400');
    ...
    if (isApiMode && localResult._apiContext) {
      btn.textContent = '? AI �������� ������...';
      setGenProgress(35, '?? FERIXDI AI �������� ������ � �����... (15-30�)');
      ...
      try {
        ...
        if (aiData) {
          clearInterval(_genProgressTimer);
          setGenProgress(100, '? ������ ������! �������� � ������ � Google Flow');
          setTimeout(resetGenProgress, 1800);
          const merged = mergeAIResult(localResult, aiData);
          log('OK', 'AI', '������ � ����� ������');
          updatePreflightStatus('? ������ � ������ ������ � �������� � ������ � Google Flow', 'bg-emerald-500/8 text-emerald-400 border border-emerald-500/15');
          saveGenerationHistory(merged);
          displayResult(merged);
        } else {
          // No JWT token � try to auto-auth and show local result for now
          log('WARN', 'AI', '��� ������ � ��������� ��������� ���������');
          updatePreflightStatus('?? ��� ������ � ������� ��������� ������', 'bg-amber-500/8 text-amber-400 border border-amber-500/15');
        ...
        resetGenProgress();
        state._generationInFlight = false;
        log('ERR', 'AI', `������ API: ${apiErr.message}`);
        updatePreflightStatus(`? ������ ���������: ${apiErr.message?.slice(0, 60) || '�����������'}`, 'bg-red-500/8 text-red-400 border border-red-500/15');
        showGenStatus('', '');
        document.getElementById('gen-results')?.classList.remove('hidden');
        ...
        if (apiErr.message?.includes('429') || apiErr.message?.includes('rate limit') || apiErr.message?.includes('�����')) {
          // Extract wait time and start countdown
          const waitMatch = apiErr.message?.match(/(\d+)\s*���/);
          ...
          errorTitle = '����� ��������� ��������';
          errorDesc = '��������� ����� ���������. ���������� ����� ��� �������� � ���������.';
          errorAction = '��������� ������� ��� ��������� � @ferixdiii � Telegram';
          errorIcon = '??';
          errorButtons = `
          ...
          errorTitle = '������ ������ �������';
          errorDesc = escapeHtml(apiErr.message || '�������������� ������');
          errorAction = '���������� ����� ����� ��������� ������';
          errorIcon = '??';
          errorButtons = `
            <button onclick="document.getElementById('gen-error-overlay')?.remove();document.getElementById('btn-generate')?.click()" class="px-4 py-2 bg-amber-500/20 text-amber-400 rounded-lg hover:bg-amber-500/30 transition-colors text-sm">
      ...
      updatePreflightStatus(hasPromo ? '?? ��������� ��������� � AI-������ ����������' : '?? ����-����� � ������� �����-��� ��� ������ ���������', 'bg-gray-500/8 text-gray-400 border border-gray-500/15');
      
      // Add helpful info about local vs AI generation
      if (!hasPromo) {
        localResult.warnings = localResult.warnings || [];
        localResult.warnings.push('��� ��������� ����������� �������� � FERIXDI AI ������� �����-��� � ������� "���������"');
      } else {
        localResult.warnings = localResult.warnings || [];
        localResult.warnings.push('AI-������ �������� ���������� � ������� ��������� ������');
      }
...
function initTranslate() {
  document.getElementById('btn-translate-en')?.addEventListener('click', async () => {
    ...
    if (!result) {
      log('ERR', 'TRANSLATE', 'state.lastResult ������ � ��� ������ ��� ��������');
    ...
    if (!token) {
      log('ERR', 'TRANSLATE', 'JWT ����� ����������� � ����������������...');
      // Auto-retry auth before giving up
      await autoAuth();
      token = localStorage.getItem('ferixdi_jwt');
      if (!token) {
        btn.innerHTML = '? ��� ������ � ������� �����-��� � ����������';
        setTimeout(() => { btn.innerHTML = '???? ��������� �� English'; btn.disabled = false; }, 2500);
        return;
      }
      log('OK', 'TRANSLATE', 'JWT ������� ����� ���������������');
    }

    try {
      log('INFO', 'TRANSLATE', `���������� ������ �� ${apiUrl}/api/translate...`);
      ...
      btn.innerHTML = '? English ������!';
      log('OK', 'TRANSLATE', `��������� �� English: A="${en.dialogue_A_en?.slice(0, 40)}..." B="${en.dialogue_B_en?.slice(0, 40)}..."`);
      showNotification('???? ���� ������� ����������� �� ���������� � ������, �����-�����, �������, ��������!', 'success');

      setTimeout(() => { btn.innerHTML = '???? ��������� �� English'; btn.disabled = false; }, 3000);

    } catch (e) {
      log('ERR', 'TRANSLATE', e.message);
      btn.innerHTML = `? ${e.message?.slice(0, 40) || '������'}`;
      setTimeout(() => { btn.innerHTML = '???? ��������� �� English'; btn.disabled = false; }, 3000);
    }
  });
}
...
function renderQCGate(qc) {
    ...
    <div class="space-y-3">
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          ...
          <span class="text-[10px] text-gray-600 font-mono">${qc.total} ��������</span>
        </div>
        <div class="flex items-center gap-2">
      ...
      <div id="qc-status-badge" class="text-center py-1.5 rounded-lg text-xs font-medium ${hasIssues ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20' : 'bg-emerald-500/10 neon-text-green border border-emerald-500/20'}">
        ${hasIssues ? `?? ������� ${failedChecks.length} ${failedChecks.length === 1 ? '��������' : failedChecks.length < 5 ? '��������' : '�������'} � ����� ��������� �������������` : '? ��� �������� �������� � ������ ����� � �������������'}
      </div>
      ...
      ${hasIssues ? `
        <button id="qc-fix-btn" class="w-full py-2.5 rounded-lg text-xs font-bold tracking-wide transition-all duration-300 bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white shadow-lg shadow-violet-500/20">
          ? ��������� ${failedChecks.length} ${failedChecks.length === 1 ? '��������' : failedChecks.length < 5 ? '��������' : '�������'} �������������
        </button>
  ...
  // Log
  if (hasIssues) {
    log('WARN', 'QC', `${qc.passed}/${qc.total} � ������� ${failedChecks.length} �������`);
  } else {
    log('OK', 'QC', `${qc.passed}/${qc.total} � �� �����`);
  }
  ...
  if (fixBtn) {
    fixBtn.addEventListener('click', () => {
      ...
      fixBtn.innerHTML = '<span class="inline-block animate-spin mr-1">??</span> ���������� � ���������...';
      ...
      failedChecks.forEach((check, i) => {
        setTimeout(() => {
          ...
          // Add to fix log
          if (fixLog) {
            fixLog.innerHTML += `<div class="text-[10px] text-emerald-400/80 flex items-start gap-1.5"><span class="mt-0.5">?</span><span><strong>${check.name_ru}</strong> � ${check.desc_fix || '����������'}</span></div>`;
          }
          ...
          if (i === failedChecks.length - 1) {
            setTimeout(() => {
              ...
              log('OK', 'QC', `��� ${failedChecks.length} ������� ���������� > ${qc.total}/${qc.total}`);
            }, 300);
          }
        }, delay * (i + 1));
      });
...
function updateCacheStats() {
  ...
  if (el) el.textContent = `���: ${stats.locations} | ����: ${stats.props} | ������: ${stats.wardrobes}`;
}
...
function applyDialogueUpdate(newA, newB) {
  ...
  if (vp?.dialogue) {
    ...
    if (isSolo) {
      ...
      const lastWord = newA.split(/\s+/).pop()?.replace(/[^\u0430-\u044f\u0451a-z]/gi, '') || '����';
      vp.dialogue.killer_word = lastWord;
    } else {
      vp.dialogue.final_B_ru = newB;
      const lastWord = newB.split(/\s+/).pop()?.replace(/[^\u0430-\u044f\u0451a-z]/gi, '') || '����';
      vp.dialogue.killer_word = lastWord;
    }
  }
...
function updateEditorEstimates() {
  ...
  if (isSolo) {
    ...
    document.getElementById('editor-est-a').innerHTML = `<span class="${overA ? 'text-red-400' : wordsA > 30 ? 'text-orange-400' : 'text-gray-500'}">${estA.duration}� / 6.8� � ${wordsA} ����${overA ? ' � �� ������!' : wordsA > 30 ? ' � �����' : ''}</span>`;
    const estBEl = document.getElementById('editor-est-b');
    if (estBEl) estBEl.innerHTML = '<span class="text-gray-600">� ���� �</span>';

    const riskColor = risk === 'high' ? 'text-red-400' : risk === 'medium' ? 'text-yellow-400' : 'neon-text-green';
    const riskLabel = risk === 'high' ? '?? ����������' : risk === 'medium' ? '?? ������' : '? ��';
    document.getElementById('editor-total').innerHTML = `<span class="${riskColor}">�������: ${estA.duration.toFixed(2)}� / 6.3� ${riskLabel}</span>`;

    const badge = document.getElementById('editor-timing-badge');
    if (badge) {
      badge.textContent = `${estA.duration.toFixed(1)}�`;
    ...
    document.getElementById('editor-est-a').innerHTML = `<span class="${overA ? 'text-red-400' : wordsA > 10 ? 'text-orange-400' : 'text-gray-500'}">${estA.duration}� / 3.3� � ${wordsA} ����${overA ? ' � �� ������!' : wordsA > 10 ? ' � �����' : ''}</span>`;
    document.getElementById('editor-est-b').innerHTML = `<span class="${overB ? 'text-red-400' : wordsB > 12 ? 'text-orange-400' : 'text-gray-500'}">${estB.duration}� / 4.0� � ${wordsB} ����${overB ? ' � �� ������!' : wordsB > 12 ? ' � �����' : ''}</span>`;

    const riskColor = risk === 'high' ? 'text-red-400' : risk === 'medium' ? 'text-yellow-400' : 'neon-text-green';
    const riskLabel = risk === 'high' ? '?? ����������' : risk === 'medium' ? '?? ������' : '? ��';
    document.getElementById('editor-total').innerHTML = `<span class="${riskColor}">����: ${total.toFixed(2)}� / 6.3� ${riskLabel}</span>`;
...
function initDialogueEditor() {
  ...
  document.getElementById('editor-auto-trim')?.addEventListener('click', () => {
    ...
    if (result.trimmed) {
      ...
      log('OK', '��������', `����-����������: ${result.auto_fixes.length} �����������`);
    } else {
      const fixesEl = document.getElementById('editor-fixes');
  ...
  document.getElementById('editor-apply')?.addEventListener('click', () => {
    ...
    log('OK', '��������', `������ �������. ������: ${state.lastResult.duration_estimate.total}�`);
  });
}
...
async function fetchTrends() {
  if (!isPromoValid()) {
    ...
    if (st) { st.classList.remove('hidden'); st.innerHTML = '<span class="text-red-400">?? ��� ������� � ������� ����� �����-���. ������� � ���������� > ����� ���.</span>'; }
    return;
  }
  ...
  const nicheNames = {
    ...
    tech: 'tech � AI', beauty: '�������', finance: '�������', education: '�����������',
    relationships: '���������', travel: '�����������', food: '���',
    parenting: '������������', realestate: '������������'
  };
  const nicheName = nicheNames[selectedNiche] || '�������������';

  btn.disabled = true;
  btn.innerHTML = '<span class="animate-pulse">?</span> FERIXDI AI ���� ������...';
  st.classList.remove('hidden');
  st.innerHTML = `<span class="text-gray-400 animate-pulse">FERIXDI AI ���� <span class="text-cyan-400">${nicheName}</span> ����...</span>`;
  ...
  try {
    ...
    try {
      ...
      st.innerHTML = `<span class="text-red-400">? ������ ������ ������������ ����� (${resp.status}). �������� ��� ���.</span>`;
      log('ERR', '������', `JSON parse error: ${parseErr.message}, status: ${resp.status}`);
      btn.disabled = false;
    ...
    if (!resp.ok) {
      st.innerHTML = `<span class="text-red-400">? ${escapeHtml(data.error || `������ ������� (${resp.status})`)}</span>`;
      btn.disabled = false;
      btn.innerHTML = '<span>??</span> ����������� ��� ���';
      return;
    }
    ...
    const groundedBadge = data.grounded
      ? '<span class="text-[9px] bg-emerald-500/15 text-emerald-400 px-1.5 py-0.5 rounded ml-2">?? ������</span>'
      : '<span class="text-[9px] bg-gray-500/15 text-gray-500 px-1.5 py-0.5 rounded ml-2">?? AI-������</span>';
    const nicheBadge = niche !== 'universal'
      ...
      : '';
    st.innerHTML = `<span class="text-emerald-400">? ${_trendsData.length} ���� � ${escapeHtml(data.weekday || '')}, ${escapeHtml(data.date)}</span>${groundedBadge}${nicheBadge}`;
    ...
    // Render
    _renderTrends();
    log('OK', '������', `��������� ${_trendsData.length} ����${data.grounded ? ' (������)' : ''}`);
  } catch (e) {
    st.innerHTML = `<span class="text-red-400">? ������ ����: ${escapeHtml(e.message)}</span>`;
    log('ERR', '������', e.message);
  }
...
function _renderTrendStats() {
  ...
  el.innerHTML = `
    <div class="trend-stat"><span>??</span> <span class="trend-stat-value">${_trendsData.length}</span> ����</div>
    ...
    <div class="trend-stat"><span>?</span> <span class="trend-stat-value">${_trendsSaved.length}</span> ����</div>
  `;
}
...
function _renderTrends() {
  ...
  const catMeta = {
    hot:    { icon: '??', label: '������� �������',  border: 'border-red-500/30',    bg: 'bg-red-500/5',    badge: 'bg-red-500/20 text-red-400',    glow: 'hover:border-red-500/50' },
    pain:   { icon: '??', label: '������ ����',       border: 'border-amber-500/30',  bg: 'bg-amber-500/5',  badge: 'bg-amber-500/20 text-amber-400',glow: 'hover:border-amber-500/50' },
    format: { icon: '??', label: '�������� ������',   border: 'border-violet-500/30', bg: 'bg-violet-500/5', badge: 'bg-violet-500/20 text-violet-400', glow: 'hover:border-violet-500/50' },
  };
  ...
  if (!items.length) {
    res.innerHTML = '<div class="text-center text-xs text-gray-500 py-8">������ �� ������� �� ���� ��������</div>';
    return;
  }
    ...
    <div class="trend-card rounded-xl p-4 space-y-3 border ${cm.border} ${cm.bg} ${cm.glow}" style="animation-delay:${delay}ms" data-idx="${origIdx}" data-cat="${t.category}">
      ...
      <div class="flex items-start gap-3">
        ...
        <div class="flex flex-col items-end gap-1 flex-shrink-0">
          <span class="trend-bookmark ${saved ? 'saved' : ''}" data-topic="${_escForAttr(t.topic)}" title="��������� ����">?</span>
      ...
      <div class="text-[11px] text-gray-300 bg-black/30 rounded-lg px-3 py-2 border-l-2 border-cyan-500/30">
        <span class="text-cyan-400/80 font-semibold">?? ������ ������:</span> ${escapeHtml(t.trend_context || t.why_trending)}
      </div>` : ''}
      ...
      <div class="trend-dialogue bg-black/40 rounded-xl p-3.5 space-y-2 border border-white/[0.03]">
        <div class="flex items-center justify-between mb-0.5">
          <div class="text-[10px] text-gray-500 font-semibold">?? ������� ������:</div>
          <div class="flex items-center gap-2">
            ...
            <button class="trend-copy-both text-[9px] px-2 py-0.5 rounded-full bg-white/5 text-gray-400 hover:bg-cyan-500/15 hover:text-cyan-400 border border-white/10 transition-colors" data-a="${_escForAttr(t.dialogue_A)}" data-b="${_escForAttr(t.dialogue_B)}" title="����������� ���">?? ���</button>
          </div>
        </div>
        <div class="flex items-start gap-2 group">
          ...
          <div class="flex items-center gap-1 flex-shrink-0">
            <span class="text-[8px] text-gray-600 font-mono">${_wordCount(t.dialogue_A)}��</span>
            <button class="trend-copy-line" data-line="${_escForAttr(t.dialogue_A)}" title="����������� ������� A">??</button>
          </div>
        </div>
        <div class="flex items-start gap-2 group">
          <div class="flex-1 text-[11px]"><span class="text-violet-400 font-bold">B:</span> <span class="text-gray-200">�${dialogB}�</span></div>
          <div class="flex items-center gap-1 flex-shrink-0">
            <span class="text-[8px] ${_wordCount(t.dialogue_B) > 18 ? 'text-red-400' : 'text-gray-600'} font-mono">${_wordCount(t.dialogue_B)}��</span>
            <button class="trend-copy-line" data-line="${_escForAttr(t.dialogue_B)}" title="����������� ������� B">??</button>
          </div>
        </div>
      </div>
      ...
      <div class="flex gap-2 flex-wrap pt-0.5">
        <button class="text-[11px] px-4 py-2 rounded-lg bg-gradient-to-r from-emerald-500/15 to-cyan-500/15 text-emerald-300 hover:from-emerald-500/25 hover:to-cyan-500/25 transition-all font-bold border border-emerald-500/25 quick-generate-trend" data-trend-index="${origIdx}" data-category="${_escForAttr(t.category)}" data-topic="${_escForAttr(t.topic)}" data-dialogue-a="${_escForAttr(t.dialogue_A)}" data-dialogue-b="${_escForAttr(t.dialogue_B)}">?? ������� ��������� <span class="text-[9px] opacity-60">����-������</span></button>
        <button class="text-[10px] px-3 py-1.5 rounded-lg bg-cyan-500/10 text-cyan-400 hover:bg-cyan-500/20 transition-colors font-semibold border border-cyan-500/15 trend-use-idea" data-idea="${_escForAttr(t.topic + ': ' + (t.comedy_angle || ''))}">?? ��� ����</button>
        <button class="text-[10px] px-3 py-1.5 rounded-lg bg-violet-500/10 text-violet-400 hover:bg-violet-500/20 transition-colors font-semibold border border-violet-500/15 trend-use-script" data-a="${_escForAttr(t.dialogue_A)}" data-b="${_escForAttr(t.dialogue_B)}">? �������� ������</button>
      </div>
    </div>`;
  });
...
async function quickGenerateFromTrend(category, topic, dialogueA, dialogueB) {
  ...
  if (a) a.value = dialogueA;
  if (b) b.value = dialogueB;
  showNotification(`? ���������: ${state.selectedA.name_ru} ? ${state.selectedB.name_ru}`, 'success');
  log('OK', '������� ���������', `${state.selectedA.name_ru} ? ${state.selectedB.name_ru} ��� "${topic.slice(0, 40)}"`);
  navigateTo('generate');
  ...
  if (notice) {
    ...
    notice.innerHTML = `
      <div class="glass-panel p-4 border-l-2 border-emerald-500/40 space-y-2">
        <div class="flex items-center justify-between">
          <div class="text-sm font-semibold text-emerald-400">?? ������������� ���������</div>
          <button onclick="navigateTo('characters')" class="text-xs text-cyan-400 hover:text-cyan-300 transition-colors">?? �������� �����</button>
        </div>
        <div class="text-xs text-gray-300">
          ...
          <div class="text-[11px] text-gray-500">AI ������ ��� ���� ��� �������� ���������� ��� ��������� "${category}" � ${state.selectedA.compatibility} + ${state.selectedB.compatibility} = ����������� ��������</div>
        </div>
      </div>
    `;
  }
...
function initTrends() {
  ...
  resEl.addEventListener('click', async (e) => {
    ...
    if (qgBtn) {
      ...
      qgBtn.disabled = true;
      qgBtn.innerHTML = '<span class="animate-pulse">?</span> ������ ����������...';
      await quickGenerateFromTrend(category || '', topic || '', dialogueA || '', dialogueB || '');
      qgBtn.disabled = false;
      qgBtn.innerHTML = '? ������!';
      setTimeout(() => { qgBtn.innerHTML = '?? ������� ��������� <span class="text-[9px] opacity-60">����-������</span>'; }, 2000);
      return;
    }
    ...
    if (copyBothBtn) {
      ...
      navigator.clipboard.writeText(`A: �${a}�\nB: �${b}�`).then(() => {
        sfx.copy();
        copyBothBtn.textContent = '? �����������';
        setTimeout(() => { copyBothBtn.innerHTML = '?? ���'; }, 1400);
      });
      return;
    }
...
function renderLocationsBrowse(filterGroup = '') {
  ...
  grid.innerHTML = `
    <div class="loc-card ${autoSelB ? 'selected ring-2 ring-violet-500' : ''}" data-loc-id="">
      ...
      <div class="text-[11px] font-medium text-violet-300">����</div>
      <div class="text-[10px] text-gray-500 mb-2">AI �������</div>
      <button class="select-loc w-full py-2 rounded-lg text-[11px] font-bold transition-all border ${autoSelB ? 'bg-violet-600 text-white border-violet-500 shadow-lg shadow-violet-500/20' : 'bg-violet-600/10 text-violet-300 border-violet-500/20 hover:bg-violet-600/25 hover:border-violet-500/40'}" data-loc-id="">${autoSelB ? '? �������' : '?? �������'}</button>
    </div>
  ` + locs.map(l => {
    const sel = state.selectedLocation === l.id;
    const moodIcon = l.mood === 'nostalgic warmth' ? '??' : l.mood === 'sterile tension' ? '??' : l.mood === 'organic chaos' ? '??' : l.mood === 'dramatic intimacy' ? '???' : '??';
    ...
    <div class="loc-card ${sel ? 'selected ring-2 ring-violet-500' : ''}" data-loc-id="${l.id}">
      ...
      <button class="select-loc w-full py-2 rounded-lg text-[11px] font-bold transition-all border mt-2 ${sel ? 'bg-violet-600 text-white border-violet-500 shadow-lg shadow-violet-500/20' : 'bg-violet-600/10 text-violet-300 border-violet-500/20 hover:bg-violet-600/25 hover:border-violet-500/40'}" data-loc-id="${l.id}">${sel ? '? �������' : '?? �������'}</button>
      <button class="copy-loc-prompt text-[9px] px-2 py-1 rounded-md font-medium transition-all bg-gold/10 text-gold hover:bg-gold/20 border border-gold/30 w-full mt-1.5 flex items-center justify-center gap-1" data-id="${l.id}" title="����������� ���������������� ������ ��� Veo">
        <span>??</span> ������
      </button>
    </div>`;
...
function initConsultation() {
  ...
  btn.addEventListener('click', async () => {
    ...
    if (!question || question.length < 3) {
      if (statusEl) { statusEl.classList.remove('hidden'); statusEl.innerHTML = '<span class="text-orange-400">?? �������� ������ (������� 3 �������)</span>'; }
      return;
    }
    ...
    if (statusEl) { statusEl.classList.remove('hidden'); statusEl.innerHTML = '<span class="text-emerald-400 animate-pulse">?? �����...</span>'; }
    ...
    if (state.generationMode) context.mode = { idea: '���� ����', suggested: '������� ����', script: '���� ������', video: '����� �����', meme: '���-������' }[state.generationMode] || state.generationMode;
...
function saveCurrentState(silent = false) {
  try {
    ...
    if (!silent) showNotification('?? ��������� ���������', 'success');
...
function _restoreDraftUI() {
  ...
  log('OK', '��������', `������������ ��������${state.selectedA ? ` � ${state.selectedA.name_ru}` : ''}${state.selectedB ? ` ? ${state.selectedB.name_ru}` : ''}`);
}
...
function updateProgress() {
  ...
  if (state.generationMode) {
    const modeNames = { idea: '?? ���� ����', suggested: '?? ������� ����', script: '?? ���� ������', video: '?? �� �����' };
  ...
  if (state.generationMode === 'idea') {
    ...
    contentText = suggestedInput.trim() ? suggestedInput.slice(0, 25) + (suggestedInput.length > 25 ? '...' : '') : '? AI ������� ����';
  ...
  if (state.selectedLocation) {
    const loc = state.locations.find(l => l.id === state.selectedLocation);
    if (locValue) locValue.textContent = loc ? loc.name_ru.slice(0, 25) : '�������';
    ...
    if (locValue) locValue.textContent = '���� (AI �������)';
  }
...
function resetAll() {
  ...
  document.querySelectorAll('.progress-value').forEach(el => {
    if (el.closest('#progress-mode')) el.textContent = '�� ������';
    else if (el.closest('#progress-content')) el.textContent = '�� ������';
    else if (el.closest('#progress-location')) el.textContent = '�� �������';
  });
...
?? ������� ����������
���: ${char.name_ru} (${char.name_en || char.id})
������: ${char.group}
�������: ${char.vibe_archetype || '�� ������'}
���� �� ���������: ${char.role_default === 'A' ? '??? ����������' : '??? ����������'}
�������������: ${char.compatibility}
...
?? ���������� ��������
${tokens.character_en || char.appearance_ru || '�� �������'}

? �������� �������� �������������
������ ����: ${anchors.face_silhouette || '�� ������'}
��������� �������: ${anchors.signature_element || '�� ������'}
�����-����: ${anchors.micro_gesture || '�� ������'}
��������-�����: ${anchors.wardrobe_anchor || '�� ������'}

?? ���� � ���������
����� ����: ${char.speech_style_ru || '�� ������'}
���� ����: ${char.speech_pace || 'normal'} (${char.speech_pace === 'fast' ? '~3.5 ����/���' : char.speech_pace === 'slow' ? '~2.0 ����/���' : '~2.5-3.0 ����/���'})
������� ����: ${char.swear_level || 0}/3
���������: ${char.behavior_ru || '�� �������'}
��������� �����: ${(char.signature_words_ru || []).join(', ') || '�� �������'}

?? ������������ ��� �����
���-�����: ${modifiers.hook_style || '�� ������'}
����� �����: ${modifiers.laugh_style || '�� ������'}

?? �������� ����
${char.world_aesthetic || '�������������'}
...
function copyCharacterPrompt(charId) {
  ...
  copyToClipboardWithFeedback(prompt, '��������', charId);
}
...
?? ������� ����������
��������: ${loc.name_ru} (${loc.name_en || loc.id})
������: ${loc.group}
����: ${(loc.tags || []).join(', ')}
��������: ${loc.tagline_ru || '�� �������'}
...
function copyLocationPrompt(locId) {
  ...
  copyToClipboardWithFeedback(prompt, '�������', locId);
}
...
function copyToClipboardWithFeedback(text, type, id) {
  navigator.clipboard.writeText(text)
    .then(() => {
      sfx.copy();
      const char = type === '��������' ? state.characters.find(c => c.id === id) : null;
      const loc = type === '�������' ? state.locations.find(l => l.id === id) : null;
      const name = char?.name_ru || loc?.name_ru || id;
      
      showNotification(`? ������ ����������: ${name}`, 'success');
      log('OK', '�����', `${type} "${name}" ���������� � ����� (${text.length} ��������)`);
      ...
      <div class="glass-panel p-4 space-y-2 border-l-2 border-amber-500/30">
        <div class="flex items-center justify-between">
          ...
          <div class="flex gap-2">
            ${epCount > 0 ? `<button class="series-eps-btn text-[10px] px-3 py-1 rounded bg-amber-500/15 text-amber-400 border border-amber-500/30 hover:bg-amber-500/25 transition-colors" data-idx="${i}">?? ${epCount} ��.</button>` : ''}
            <button class="series-gen-btn text-[10px] px-3 py-1 rounded bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500/25 transition-colors" data-idx="${i}">? ����� ������</button>
        ...
        ${s.style ? `<div class="text-[10px] text-gray-500">�����: ${escapeHtml(s.style)}</div>` : ''}
        <div class="text-[10px] text-gray-600">${epCount} ${epCount === 1 ? '������' : (epCount >= 2 && epCount <= 4) ? '�������' : '��������'}</div>
  ...
  return `
    <div class="ep-card bg-black/20 rounded-lg border border-gray-800/60 overflow-hidden">
      <div class="flex items-center justify-between px-3 py-2 bg-gray-900/40">
        ...
        <div class="flex items-center gap-1.5">
          <button class="ep-toggle-btn text-[9px] px-2 py-0.5 rounded text-amber-400 hover:bg-amber-500/10 transition-colors">? �������</button>
      ...
      <div class="ep-body hidden p-3 space-y-2">
        ${hasVeo ? `<div class="space-y-1">
          <div class="flex items-center justify-between">
            <span class="text-[9px] text-emerald-400 font-semibold uppercase tracking-wider">?? Veo ������</span>
            <button class="ep-copy-btn text-[9px] px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 hover:bg-emerald-500/20 transition-colors" data-copy="${veoSafe}">?? ����������</button>
          </div>
        ...
        </div>` : '<div class="text-[10px] text-gray-600 italic">? ������ �� ������� (������ ������)</div>'}
        ${hasRu ? `<div class="space-y-1">
          <div class="flex items-center justify-between">
            <span class="text-[9px] text-blue-400 font-semibold uppercase tracking-wider">???? ����</span>
            <button class="ep-copy-btn text-[9px] px-2 py-0.5 rounded bg-blue-500/10 text-blue-400 border border-blue-500/20 hover:bg-blue-500/20 transition-colors" data-copy="${ruSafe}">?? ����������</button>
          </div>
        ...
        ${hasInsta ? `<div class="space-y-1.5">
          <span class="text-[9px] text-pink-400 font-semibold uppercase tracking-wider">?? �����-�����</span>
          ${viralTitle ? `<div class="flex items-center justify-between bg-black/20 rounded px-2 py-1.5">
            <div><span class="text-[9px] text-amber-400">?? ���������:</span> <span class="text-[10px] text-gray-300">${escapeHtml(viralTitle)}</span></div>
          ...
          ${shareBait ? `<div class="flex items-center justify-between bg-black/20 rounded px-2 py-1.5">
            <div><span class="text-[9px] text-orange-400">?? ��������:</span> <span class="text-[10px] text-gray-300">${escapeHtml(shareBait.length > 80 ? shareBait.slice(0, 80) + '...' : shareBait)}</span></div>
          ...
          ${pinComment ? `<div class="flex items-center justify-between bg-black/20 rounded px-2 py-1.5">
            <div><span class="text-[9px] text-rose-400">?? ������:</span> <span class="text-[10px] text-gray-300">${escapeHtml(pinComment.length > 80 ? pinComment.slice(0, 80) + '...' : pinComment)}</span></div>
...
function createSeries() {
  if (!isPromoValid()) { showNotification('?? ����� �����-���', 'error'); return; }
  ...
  if (!name) { showNotification('������� �������� �����', 'error'); return; }
  if (!charA || !charB) { showNotification('�������� ����� ����������', 'error'); return; }
  if (charA === charB) { showNotification('��������� ������ ���� ������', 'error'); return; }
...
function deleteSeries(idx) {
  const series = getSeries();
  if (!confirm(`������� ����� "${series[idx]?.name}"?`)) return;
  series.splice(idx, 1);
  saveSeries(series);
  renderSeriesList();
  showNotification('����� �������', 'info');
...
function generateFromSeries(idx) {
  ...
  const hint = s.style ? `���� �����: ${s.style}. ��� ������ #${(s.episodes?.length || 0) + 1}.` : '';
  const ideaInput = document.getElementById('idea-input');
  ...
  navigateTo('generate');
  updateReadiness?.();
  showNotification(`?? ����� "${s.name}" � ������ ����� ������`, 'success');
}
...
function renderHistory() {
  ...
  if (shownCount === 0) {
    list.innerHTML = '<div class="text-center text-xs text-gray-500 py-4">������ �� �������</div>';
    return;
  }
...
function exportHistory() {
  ...
  if (!history.length) { showNotification('������� ����� � ������ ��������������', 'info'); return; }
  ...
  a.click();
  URL.revokeObjectURL(url);
  showNotification(`? �������������� ${history.length} ���������`, 'success');
  log('OK', '�������', `�������������� ${history.length} �������`);
}
...
const VIRAL_SURPRISE_PRESETS = [
  // === AI � ���������� ===
  { topic: 'ChatGPT ������� �� ����� ��������� � ����� ������ ��� ���� �����', hook: 'A ������� �������� � ����� ����� �������', killer: '�����', share: '����� ���� ��� ��� ����� ChatGPT', pair: { groupA: ['babki'], groupB: ['parni','devushki'], compatA: ['chaotic','meme'] }, loc: ['kitchen','living_room'], cat: 'AI � ����������' },
  { topic: '��� ������ ��������� � ���������� ���� ������� �� ���������', hook: 'A ������������ ������� � ������ � �������� �������', killer: '���������', share: '������ �������', pair: { groupA: ['dedy'], groupB: ['babki'], compatA: ['meme','chaotic'] }, loc: ['living_room','kitchen'], cat: 'AI � ����������' },
  { topic: '����� ������ ��� ����� ��� �� ������� � ����� � ��������', hook: 'A ����������� ������� � ������', killer: '�������', share: '����� ������� ����� ��������', pair: { groupA: ['babki'], groupB: ['dedy','parni'], compatA: ['chaotic'] }, loc: ['kitchen','living_room'], cat: 'AI � ����������' },
  { topic: '���� ����� ���������� ��� �������� ���� � ������� ���� ����� 20 ���', hook: 'A ������ ������� �� ���� � �������� ����', killer: '�������', share: '����� ���� ����� ���������', pair: { groupA: ['mamy'], groupB: ['devushki'], compatA: ['conflict','chaotic'] }, loc: ['kitchen','living_room'], cat: 'AI � ����������' },
  { topic: '��� �������� Siri ��������� ���� � �� ������� ������', hook: 'A �������� � ��������� � ���', killer: '������', share: '������ ���� ��� ������������� � ���������', pair: { groupA: ['dedy'], groupB: ['babki'], compatA: ['meme'] }, loc: ['kitchen','car'], cat: 'AI � ����������' },
  { topic: '��������� ���������� ������� ����� �� �������� � ��������� ���', hook: 'A ����� ������� � ����� ����������', killer: '���', share: '����� ������� � ������� ��� ����� �� �������', pair: { groupA: ['babki'], groupB: ['devushki','parni'], compatA: ['meme','chaotic'] }, loc: ['living_room'], cat: 'AI � ����������' },
  { topic: '����� ������� ��������� ������ ����� � �� ���������� �������� ��������', hook: 'A ������� �� ����� � ���������� �����', killer: '��������', share: '����� ���� ��� �� ����� ��������', pair: { groupA: ['babki'], groupB: ['parni','devushki'], compatA: ['chaotic','conflict'] }, loc: ['kitchen'], cat: 'AI � ����������' },
  { topic: '��� ��������� ��������� �������� � ������ ������ � ��� ������ �����', hook: 'A ����������� � ������� � ������ �������', killer: '���������', share: '������ ���� ��� ������������� � ������', pair: { groupA: ['dedy'], groupB: ['babki','parni'], compatA: ['meme'] }, loc: ['living_room','kitchen'], cat: 'AI � ����������' },
  { topic: '������ �������� ����� ������� ����� � ��� � ����� ������ � �������', hook: 'A ��������� �� ������ � �������', killer: '�������', share: '����� �������', pair: { groupA: ['babki'], groupB: ['devushki'], compatA: ['chaotic'] }, loc: ['living_room','kitchen'], cat: 'AI � ����������' },
  { topic: '���� ��������� ChatGPT �������� ��� ���� � ����� ������� ����������', hook: 'A ������ � �������� � ��������', killer: '���������', share: '����� ������� ����� ���������', pair: { groupA: ['mamy'], groupB: ['dedy','parni'], compatA: ['meme','conflict'] }, loc: ['kitchen','living_room'], cat: 'AI � ����������' },
  { topic: '����� ��������� �� ������� ����� ����� � ����������� � �������', hook: 'A ��������� ������� � ��� � ���������', killer: '�������', share: '����� ���� ��� ����� �� ������� �����', pair: { groupA: ['babki'], groupB: ['parni','devushki'], compatA: ['meme','chaotic'] }, loc: ['kitchen','living_room'], cat: 'AI � ����������' },
  { topic: '��� ����� �����-������� � ������ �� ��� ��� �� ������', hook: 'A ����� �� ���� � ��������� �� ���������', killer: '����', share: '������ ���� � ���� �����-�������', pair: { groupA: ['dedy'], groupB: ['babki'], compatA: ['meme'] }, loc: ['living_room'], cat: 'AI � ����������' },

  // === ���� � �������� ===
  { topic: '��� �� 800? � ����� ��������� � �������� ��� �� ������', hook: 'A ������� ��� �� ���� � ����� �������', killer: '���������', share: '����� ���� ��� ������ ��� �� 50?', pair: { groupA: ['babki'], groupB: ['prodavtsy','sosedi'], compatA: ['chaotic','conflict'] }, loc: ['shop','market'], cat: '���� � ��������' },
  { topic: '��� ������ ��� �� �������� � ������ ��� ��������� �� �������', hook: 'A ������������� ��� � �� ������ �� ����', killer: '�������', share: '����� ���� ����� ��������', pair: { groupA: ['dedy'], groupB: ['babki','mamy'], compatA: ['meme','chaotic'] }, loc: ['kitchen','shop'], cat: '���� � ��������' },
  { topic: '���� ������ ������� �� 300? � �������� ������� ��� ��������������', hook: 'A ��������� ������� ��� �����', killer: '�������', share: '����� �������� ��� ���', pair: { groupA: ['mamy'], groupB: ['babki'], compatA: ['conflict'] }, loc: ['kitchen'], cat: '���� � ��������' },
  { topic: '����� ���������� ���� 1990 � 2026 � ������ ��� ����� ������', hook: 'A �������� ������ � ����� ����������', killer: '�������', share: '����� ���� ����� ���������', pair: { groupA: ['babki'], groupB: ['devushki','parni'], compatA: ['chaotic','meme'] }, loc: ['kitchen','shop'], cat: '���� � ��������' },
  { topic: '������ ������� ���� �� 600? � ��� ��������� ������� ������ ������', hook: 'A ��������� ���� ����������� � ���', killer: '������', share: '������ ���� ��� �������� ���� ������ ����', pair: { groupA: ['dedy'], groupB: ['parni'], compatA: ['conflict','meme'] }, loc: ['cafe'], cat: '���� � ��������' },
  { topic: '����� ������� ������ ����� � ������ ��� ��� ���� �� ������', hook: 'A ������� ������ � �������� � ������ ������', killer: '������', share: '����� ���� ��� �������� ������ �����', pair: { groupA: ['babki'], groupB: ['prodavtsy'], compatA: ['chaotic'] }, loc: ['shop','market'], cat: '���� � ��������' },
  { topic: '��� ������ ���� �� ������ � ����� ��������� �� ���������', hook: 'A ������� ����� �� ������ �� ����', killer: '���������', share: '����� �������������', pair: { groupA: ['dedy'], groupB: ['parni','babki'], compatA: ['meme'] }, loc: ['car','kitchen'], cat: '���� � ��������' },
  { topic: '������ ����� ��� ��� ���� �� 2000? � ����� � ���� �� �����������', hook: 'A ������� � ����� � ��������� �����', killer: '���', share: '����� ��������� ����', pair: { groupA: ['babki'], groupB: ['devushki','parni'], compatA: ['meme','chaotic'] }, loc: ['kitchen','stairwell'], cat: '���� � ��������' },

  // === ������ ��������� ===
  { topic: '������ �������� ������ � ����� ������� ������', hook: 'A ������� ������ �� ���� � �������������', killer: '������', share: '����� ������� � ��������', pair: { groupA: ['babki'], groupB: ['devushki'], compatA: ['chaotic','conflict'] }, loc: ['bathroom','living_room'], cat: '������ ���������' },
  { topic: '��� ������ ������ ������ �� 15000? � ��������� ������ ���������', hook: 'A ������� ������ � ���� �����', killer: '������', share: '������ ���� ��� ����� ������ ������', pair: { groupA: ['dedy'], groupB: ['parni','devushki'], compatA: ['meme'] }, loc: ['living_room','shop'], cat: '������ ���������' },
  { topic: '����� �� �������� ��� ���� ������������ � �������� ������ ��� ��� �� ������', hook: 'A ����� � ������� ����� � ���� ����', killer: '�������', share: '����� ����������', pair: { groupA: ['babki'], groupB: ['parni'], compatA: ['conflict','chaotic'] }, loc: ['kitchen','living_room'], cat: '������ ���������' },
  { topic: '��� ���� ����� ������ ���� � ��� ������ ����� �� YouTube', hook: 'A ����������� ������� � ����� ������', killer: 'YouTube', share: '����� ���� ��� ����� �� �� YouTube', pair: { groupA: ['dedy'], groupB: ['parni'], compatA: ['conflict','meme'] }, loc: ['bathroom','kitchen'], cat: '������ ���������' },
  { topic: '����� ������� �������� � ������ � ���� ���������', hook: 'A ������������� ����� ��������', killer: '����', share: '����� ���� ��� ���������� �������� ������ ����', pair: { groupA: ['babki'], groupB: ['devushki'], compatA: ['chaotic'] }, loc: ['kitchen'], cat: '������ ���������' },
  { topic: '���� ������� ���� NFT � ��� ��������� �������� �� ����� � �����', hook: 'A ������ ������� ������� ����������', killer: '�����', share: '������ ���� ��� �������� NFT', pair: { groupA: ['dedy'], groupB: ['parni'], compatA: ['meme'] }, loc: ['living_room'], cat: '������ ���������' },
  { topic: '���� ������ ��� ����� ����������� ����� ��������', hook: 'A ��������� �� ������ ������ ������', killer: '��������', share: '����� ���� ��� ������������ � ����������', pair: { groupA: ['mamy'], groupB: ['devushki'], compatA: ['conflict'] }, loc: ['kitchen','living_room'], cat: '������ ���������' },
  { topic: '����� ������� ����� ������ � ������ ��� �� �������� � �����', hook: 'A ���������� �� ����� � ������', killer: '����', share: '����� ��������', pair: { groupA: ['babki'], groupB: ['devushki','parni'], compatA: ['chaotic','meme'] }, loc: ['living_room'], cat: '������ ���������' },
  { topic: '��� ��������� ��� ����������� ������ � ����� �� �����', hook: 'A �������� ���� � ���������� �� �����', killer: '�����', share: '������ ������ ������� �� ������� �� ����', pair: { groupA: ['dedy'], groupB: ['devushki'], compatA: ['meme'] }, loc: ['living_room','yard'], cat: '������ ���������' },
  { topic: '��� ������� ������ ����� � ����� ��� ��� ������������ ������', hook: 'A ������ ������ � ������� �� �������', killer: '������', share: '������ ���� ��� ���� �����', pair: { groupA: ['dedy'], groupB: ['devushki','parni'], compatA: ['meme'] }, loc: ['kitchen','cafe'], cat: '������ ���������' },
  { topic: '����� ������� �������-���� ������ � �������� ��� ����', hook: 'A ����������� � ������� � �������� ������', killer: '����', share: '����� ������', pair: { groupA: ['babki'], groupB: ['devushki'], compatA: ['chaotic','conflict'] }, loc: ['kitchen','cafe'], cat: '������ ���������' },
  { topic: '��� ����� ������ 1975 ���� � ������� ��� �� ����� � ����', hook: 'A �������� ������ � ���������� �������', killer: '����', share: '����� �������', pair: { groupA: ['dedy'], groupB: ['parni','devushki'], compatA: ['meme'] }, loc: ['living_room','bedroom'], cat: '������ ���������' },
  { topic: '�������� ��������� ����� ���� ������ � ��� �� ��� ��� �� ������', hook: 'A ������ ����� � ������� ������', killer: '������', share: '������ ��������� ������� �� �������� ��� �� �����������', pair: { groupA: ['parni'], groupB: ['babki','mamy'], compatA: ['meme'] }, loc: ['kitchen','living_room'], cat: '������ ���������' },

  // === ������� ������ ===
  { topic: '������������� � ��� ��������� ���� �����', hook: 'A ��������� ������� ��� ������', killer: '���', share: '����� ���� ��� ����� ������ �����', pair: { groupA: ['babki','mamy'], groupB: ['dedy','parni'], compatA: ['chaotic','conflict'] }, loc: ['living_room','kitchen'], cat: '������� ������' },
  { topic: '����� ����� ����� ����� � �������� � �������������', hook: 'A ������ ����� ����� ��������', killer: '�����', share: '����� ���� � ���� ��������� �����', pair: { groupA: ['babki'], groupB: ['dedy','parni'], compatA: ['chaotic','meme'] }, loc: ['bathroom','kitchen'], cat: '������� ������' },
  { topic: '��� ������ ���� ������� � �������� ����', hook: 'A ������� ���� �� ���', killer: '���', share: '������ ���� ��� ����� ���', pair: { groupA: ['dedy'], groupB: ['babki','mamy'], compatA: ['meme','chaotic'] }, loc: ['bathroom','kitchen'], cat: '������� ������' },
  { topic: '��� ������ ���� � ������ �������� �������', hook: 'A ���������� �� ������� ����� �� B', killer: '����', share: '����� ���� � ���� ��� �������', pair: { groupA: ['babki','mamy'], groupB: ['dedy','parni'], compatA: ['conflict'] }, loc: ['living_room','kitchen'], cat: '������� ������' },
  { topic: '��� ����� �� ��� ���� � ���� ��� ����� ������', hook: 'A ������� ������� �� �����', killer: '������', share: '����� ���� ������� �������� �� ��� ����', pair: { groupA: ['mamy'], groupB: ['dedy','parni'], compatA: ['chaotic','conflict'] }, loc: ['kitchen'], cat: '������� ������' },
  { topic: '����� ���������� ��� � �������� ���� ��� ����� �������', hook: 'A ������� � ����������', killer: '�������', share: '����� ���� ��� ������������', pair: { groupA: ['babki'], groupB: ['dedy'], compatA: ['meme','chaotic'] }, loc: ['kitchen'], cat: '������� ������' },
  { topic: '���� ��� ����� ������� � ��� ������� ���� �������', hook: 'A ������ ������ � ��������', killer: '�������', share: '����� ���� ��� ������� ���� �������', pair: { groupA: ['babki','mamy'], groupB: ['dedy','parni'], compatA: ['chaotic','meme','conflict'] }, loc: ['kitchen'], cat: '������� ������' },
  { topic: '��� ������ � ��� �������� ��� ������ �� ��������', hook: 'A �������� ������ � ������������ �����', killer: '������', share: '����� ���� ��� ������ � ��������', pair: { groupA: ['dedy','babki'], groupB: ['babki','dedy'], compatA: ['meme','conflict'] }, loc: ['bedroom','kitchen'], cat: '������� ������' },
  { topic: '����� 3 ���� ������ ���� � ��� �� ������', hook: 'A �������������� ������� � �������', killer: '������', share: '����� ���� ��� ������ ����', pair: { groupA: ['babki'], groupB: ['dedy','parni'], compatA: ['meme'] }, loc: ['living_room','kitchen'], cat: '������� ������' },
  { topic: '��� ����� �� �� ������ �� �������� � ��� ������� 4 ����', hook: 'A ��� ��� ������ ������', killer: '������', share: '������ ���� ��� ������ �������', pair: { groupA: ['dedy'], groupB: ['babki','mamy'], compatA: ['meme','chaotic'] }, loc: ['bathroom','kitchen'], cat: '������� ������' },
  { topic: '����� ����������� ���� � ������ ���� ����� �� ��������', hook: 'A ������ ��������� � ������', killer: '�����', share: '����� ���� ��� �� �� ����', pair: { groupA: ['babki'], groupB: ['devushki','parni'], compatA: ['chaotic','meme'] }, loc: ['cafe','kitchen'], cat: '������� ������' },
  { topic: '��������� ������ ��������� �� ������ ���������', hook: 'A ������������ ��������� � ��������� �����', killer: '���������', share: '����� ������� �� �����', pair: { groupA: ['chinovniki','biznes'], groupB: ['parni','devushki'], compatA: ['conflict'] }, loc: ['office'], cat: '������� ������' },
  { topic: '������� �������� ������ � �������� ������ ����������� � ������� ������', hook: 'A ����� � ������� � ������ �� �����', killer: '������', share: '����� ������� ��� �������� ���������', pair: { groupA: ['mamy'], groupB: ['uchitelya'], compatA: ['conflict','chaotic'] }, loc: ['school','office'], cat: '������� ������' },

  // === �������� ===
  { topic: '����� ���� ����� � ���������� ������ ��', hook: 'A ������ ����� ����� �� �����', killer: '����������', share: '����� ������� ������� ����� ����', pair: { groupA: ['babki'], groupB: ['doktory','sosedi'], compatA: ['chaotic','conflict'] }, loc: ['clinic','kitchen'], cat: '�������� � �����������' },
  { topic: '��� �������� �������� � ����� ��� �������� 3 ���', hook: 'A ���������� ������� ��� ��������', killer: '��������', share: '����� ���� ��� ������ ��������', pair: { groupA: ['dedy'], groupB: ['babki','mamy'], compatA: ['meme'] }, loc: ['living_room','clinic'], cat: '�������� � �����������' },
  { topic: '���� ����� �������� ���� � ������ ���������', hook: 'A ������ ����� ��� �����', killer: '��������', share: '����� ���� ������� ��������� �����', pair: { groupA: ['mamy'], groupB: ['parni'], compatA: ['chaotic','conflict'] }, loc: ['kitchen','living_room'], cat: '�������� � �����������' },
  { topic: '��� ������������ � ����� � � 45 �� ����� � ���', hook: 'A ���������� ���� � ������ �������', killer: '���', share: '����� ���� ��� ������ ������', pair: { groupA: ['dedy'], groupB: ['babki','mamy'], compatA: ['conflict','meme'] }, loc: ['kitchen','living_room'], cat: '�������� � �����������' },
  { topic: '����� ����� ����� ������������ ������� � ���������� ������������', hook: 'A ����������� ����� ��� �� ������', killer: '�������', share: '����� ���� ���� ������ ����������� ��������', pair: { groupA: ['babki'], groupB: ['parni','devushki'], compatA: ['chaotic','meme'] }, loc: ['kitchen','living_room'], cat: '�������� � �����������' },

  // === ��� ===
  { topic: '��������� �� ��������� � ��� � ����� �����', hook: 'A ������������� ��������� � ����������� ������', killer: '���', share: '����� ������ �� ��������', pair: { groupA: ['babki'], groupB: ['sosedi','chinovniki'], compatA: ['chaotic','conflict'] }, loc: ['stairwell','kitchen'], cat: '��� � ����������' },
  { topic: '����� ������� � ��� �������� �����', hook: 'A ���������� �� ������� � �������� ������', killer: '�����', share: '����� ������ ������', pair: { groupA: ['dedy','babki'], groupB: ['sosedi'], compatA: ['conflict','chaotic'] }, loc: ['stairwell','bathroom'], cat: '��� � ����������' },
  { topic: '����� ������� ���� � ������ �� ����������� �������', hook: 'A ����� ������������� � ������', killer: '�������', share: '����� ���� ��� �� ������ �� ����������', pair: { groupA: ['babki'], groupB: ['dedy'], compatA: ['chaotic','meme'] }, loc: ['kitchen','bathroom'], cat: '��� � ����������' },
  { topic: '��� ������ ���� �� �������� ��� ��������', hook: 'A ���� �� �������� �������', killer: '����', share: '����� ���� ��� �������� ������', pair: { groupA: ['dedy'], groupB: ['sosedi','parni'], compatA: ['meme'] }, loc: ['stairwell','yard'], cat: '��� � ����������' },
  { topic: '����� ������� ������� �������� ������ ����� �������', hook: 'A ������������� ������ ������ � ����', killer: '�����', share: '����� ������ � ��������', pair: { groupA: ['babki'], groupB: ['sosedi'], compatA: ['conflict','chaotic'] }, loc: ['stairwell'], cat: '��� � ����������' },

  // === ���� ===
  { topic: '����� ���������� ������� � ������� ������� � �������� �������', hook: 'A ��������� ������� �������� � �����', killer: '�������', share: '����� �������', pair: { groupA: ['babki'], groupB: ['sosedi','babki'], compatA: ['conflict','meme'] }, loc: ['dacha','yard'], cat: '���� � ������' },
  { topic: '��� �������� ������� �� ������ ���� � ������� ���� ������������', hook: 'A �������� ���� ��������� �������', killer: '����������', share: '������ ���� ��� ������ �� ���������', pair: { groupA: ['dedy'], groupB: ['babki'], compatA: ['meme','chaotic'] }, loc: ['dacha','yard'], cat: '���� � ������' },
  { topic: '������ ������� � ����� ����������� �������', hook: 'A ���������� ������ ������ � ������� ������', killer: '�������', share: '����� ������� � �������� ������', pair: { groupA: ['babki'], groupB: ['sosedi','dedy'], compatA: ['chaotic','conflict'] }, loc: ['dacha','yard'], cat: '���� � ������' },
  { topic: '������ �� ���� � ����� � ����� ������ �����', hook: 'A ���������� B ������ ����', killer: '�����', share: '����� ���� ��� ����� �� ���� � ���������', pair: { groupA: ['babki'], groupB: ['devushki'], compatA: ['chaotic','meme'] }, loc: ['dacha'], cat: '���� � ������' },
  { topic: '����� ���������� ������� � ������ ��� �����������', hook: 'A ���������� �� ��� ���� ������������ ��������', killer: '����������', share: '����� ���� � ���� ������� �������', pair: { groupA: ['babki'], groupB: ['dedy','parni'], compatA: ['chaotic'] }, loc: ['kitchen','living_room'], cat: '���� � ������' },
  { topic: '����� ��������� ������ �� ���� � ������� ������ 24/7 ��� ������', hook: 'A ����� � ��������� � ������������ ����', killer: '�����', share: '����� �������', pair: { groupA: ['babki'], groupB: ['dedy','parni'], compatA: ['meme','chaotic'] }, loc: ['dacha','kitchen'], cat: '���� � ������' },

  // === ������� ===
  { topic: '����� �������� �������� ���� � ������� ����������', hook: 'A ������� � ������� � ��������� �� ����', killer: '�������', share: '����� ���� ��� ������� � ����������', pair: { groupA: ['babki'], groupB: ['devushki','parni'], compatA: ['meme'] }, loc: ['kitchen','living_room'], cat: '������� � ������' },
  { topic: '��� ���� ������ � ������ �� ����', hook: 'A ������ ���� ����� ������� ��������', killer: '����������', share: '������ �������', pair: { groupA: ['dedy'], groupB: ['babki','parni'], compatA: ['meme','chaotic'] }, loc: ['kitchen'], cat: '������� � ������' },
  { topic: '���� �� ������ ����� � ����� ��-�� ��������', hook: 'A �������� ������� � ���� B � ����������', killer: '������', share: '����� ������� � ���������', pair: { groupA: ['mamy'], groupB: ['devushki'], compatA: ['chaotic','conflict'] }, loc: ['living_room','kitchen'], cat: '������� � ������' },
  { topic: '����� ������ ��� �������� ����� ������������ �� ��������', hook: 'A �������� ����� ���������', killer: '����', share: '����� ������� ������� ����� � ���������', pair: { groupA: ['babki'], groupB: ['parni','devushki'], compatA: ['meme','chaotic'] }, loc: ['kitchen','living_room'], cat: '������� � ������' },
  { topic: '������ ��������� ����� ����� � ��� ������ ������', hook: 'A �������� ������ ������� ���������', killer: '������', share: '����� �������', pair: { groupA: ['parni'], groupB: ['babki'], compatA: ['meme'] }, loc: ['kitchen','living_room'], cat: '������� � ������' },
  { topic: '�������� �� ��������� ����� ����� � ����� ���������� ���������������', hook: 'A ������ VHS ������� �� �����', killer: '�������', share: '������ ���� ��� ������ ������������', pair: { groupA: ['babki'], groupB: ['parni','devushki'], compatA: ['meme','chaotic'] }, loc: ['living_room'], cat: '������� � ������' },
  { topic: '����� �������� �� WB � ������ 15 ������� ������ �����', hook: 'A ����� ����� ����� �������', killer: '�������', share: '����� ���� ��� �� ����� ������������ �� WB', pair: { groupA: ['babki'], groupB: ['devushki','parni'], compatA: ['meme','chaotic'] }, loc: ['living_room','stairwell'], cat: '������� � ������' },
  { topic: '��� ������ ����� ������ � ������ ��� ����� �����', hook: 'A ����� � ������� � ���������', killer: '�����', share: '������ ���� ��� ����� � ��� ������ ����', pair: { groupA: ['dedy'], groupB: ['prodavtsy','devushki'], compatA: ['meme'] }, loc: ['shop'], cat: '������� � ������' },
  { topic: '���� �������� ����� � �������� ����� �� 3 ��������', hook: 'A ���������� ����������� ����� �� ��������', killer: '�����', share: '����� ���� ��� ����� ������� ������', pair: { groupA: ['mamy'], groupB: ['devushki','parni'], compatA: ['meme','conflict'] }, loc: ['kitchen','living_room'], cat: '������� � ������' },

  // === ��������� ===
  { topic: '���� ����� ���� ���� �� ���� ������� � ������', hook: 'A ������������ ������� ������� � B', killer: '����', share: '����� ���� ��� ������������', pair: { groupA: ['mamy'], groupB: ['dedy','parni'], compatA: ['conflict','chaotic'] }, loc: ['kitchen','bedroom'], cat: '���������' },
  { topic: '����� ���� �������� ���� � �� �����', hook: 'A ������� ���� B � �������������', killer: '����', share: '����� ������� ������ ����', pair: { groupA: ['babki'], groupB: ['devushki'], compatA: ['meme','chaotic'] }, loc: ['kitchen','living_room'], cat: '���������' },
  { topic: '���� ����������� ����� ����� ��� �� �������������', hook: 'A ������� �������� � ���������� ���� ��� HR', killer: '��������', share: '����� ����� ������� ���������� � �����', pair: { groupA: ['mamy'], groupB: ['parni'], compatA: ['conflict','chaotic'] }, loc: ['kitchen','living_room'], cat: '���������' },
  { topic: '�������� �������� � ������ ����� ������� �����������', hook: 'A ����������� ����������� � ������������', killer: '�����������', share: '����� �������� ��� ��������', pair: { groupA: ['babki'], groupB: ['mamy','devushki'], compatA: ['conflict'] }, loc: ['kitchen'], cat: '���������' },
  { topic: '������ ������� ������� �� 8 ����� � �� �������� ��������', hook: 'A ���������� ������� � ��������� �������', killer: '�������', share: '����� ���� ����� 8 �����', pair: { groupA: ['parni'], groupB: ['devushki','mamy'], compatA: ['meme'] }, loc: ['living_room','kitchen'], cat: '���������' },
  { topic: '��� ��� ����� �� ���������� � ����� ������������ ��-�� ����', hook: 'A �������� B � �������� �������', killer: '������', share: '������ �������', pair: { groupA: ['dedy'], groupB: ['parni'], compatA: ['meme'] }, loc: ['kitchen','yard'], cat: '���������' },
  { topic: '���� �������� ���������� � ����� ��� ��� ������� ���� ����', hook: 'A ������� ������ ��� ������', killer: '������', share: '����� ����������', pair: { groupA: ['mamy'], groupB: ['devushki','parni'], compatA: ['conflict','chaotic'] }, loc: ['kitchen','living_room'], cat: '���������' },
  { topic: '����� ����� ������� ���� �� ����� ��������� � � �� 50 ���', hook: 'A ����� � ������� �� ������� �� �����', killer: '�������', share: '����� ������� ����', pair: { groupA: ['babki'], groupB: ['dedy'], compatA: ['conflict','meme'] }, loc: ['kitchen','living_room'], cat: '���������' },

  // === ��������� ===
  { topic: '����� ���� �������� ��������� ����� ���������', hook: 'A ����������� � ���������� �����������', killer: '����', share: '����� ���� ��� ���� ��������', pair: { groupA: ['babki'], groupB: ['taksisty','sosedi'], compatA: ['chaotic','conflict'] }, loc: ['car','bus_stop'], cat: '��������� � ������' },
  { topic: '��� vs ��������� � ��� ����� ����� ������', hook: 'A ��������� ��������� ����������', killer: '�����', share: '������ ���� ��� �� �������� ����������', pair: { groupA: ['dedy'], groupB: ['babki','mamy'], compatA: ['meme','chaotic'] }, loc: ['car'], cat: '��������� � ������' },
  { topic: '����� ������� ���������� ��������� ����� �����������', hook: 'A ����� � ������ �� ������', killer: '�����', share: '����� ���� ��� ����� �� ���������', pair: { groupA: ['babki'], groupB: ['taksisty','sosedi'], compatA: ['chaotic'] }, loc: ['car'], cat: '��������� � ������' },
  { topic: '��� ������������� � ����� �� ����� �������', hook: 'A �������� ������ � �������� �������', killer: '�����', share: '����� ���� ��� ����� ���������', pair: { groupA: ['dedy'], groupB: ['parni','mamy'], compatA: ['meme'] }, loc: ['yard','car'], cat: '��������� � ������' },
];
...
function initSurprise() {
  document.getElementById('btn-surprise')?.addEventListener('click', () => {
    if (!isPromoValid()) { showNotification('?? ����� �����-��� ��� ���������', 'error'); navigateTo('settings'); return; }

    const chars = state.characters;
    if (!chars || chars.length < 2) { showNotification('?? ��������� �� ���������', 'error'); return; }
    ...
    if (preset.hook) fullTopic += ` [���: ${preset.hook.replace(/\bA\b/g, nameA).replace(/\bB\b/g, nameB)}]`;
    ...
    log('OK', 'VIRAL_SURPRISE', `#${idx}/${VIRAL_SURPRISE_PRESETS.length} [${_lastSurpriseIndices.length}/${VIRAL_SURPRISE_PRESETS.length} used] uid:${uid} | "${preset.topic}" | ${nameA} ? ${nameB} | ���: ${preset.cat}`);
  });
}
...
async function generateABVariants() {
  ...
  if (btn) { btn.disabled = true; btn.textContent = '? AI ���������� 3 ��������...'; }
  ...
  try {
    const apiBase = localStorage.getItem('ferixdi_api_url') || DEFAULT_API_URL;
    const token = localStorage.getItem('ferixdi_jwt');
    if (!token) { showNotification('?? ��� ������ �����������', 'error'); return; }
    ...
    if (!resp.ok) {
      showNotification(data.error || '������ A/B ���������', 'error');
      return;
    }
    ...
    const variants = [
      { label: '�������', a: mainA, b: mainB, killer: mainKiller, active: true },
    ];

    // The new main dialogue from AI (variant B)
    if (ai.dialogue_A_ru) {
      variants.push({ label: '������� B', a: ai.dialogue_A_ru, b: ai.dialogue_B_ru || '�', killer: ai.killer_word || '' });
    }
    ...
    if (Array.isArray(ai.ab_variants)) {
      ai.ab_variants.forEach((v, i) => {
        if (v?.dialogue_A_ru && (abSolo || v?.dialogue_B_ru)) {
          variants.push({ label: labels[i] || `������� ${i + 3}`, a: v.dialogue_A_ru, b: abSolo ? '�' : (v.dialogue_B_ru || '�'), killer: v.killer_word || '' });
        }
      });
    }
    ...
    container.innerHTML = variants.map((v, i) => `
      <div class="p-3 rounded-lg border ${v.active ? 'border-emerald-500/40 bg-emerald-500/5' : 'border-gray-700 hover:border-amber-500/30'} cursor-pointer transition-colors ab-variant-card" data-idx="${i}">
        ...
        ${!v.active ? `<button class="ab-select-btn mt-1.5 text-[9px] px-2 py-1 rounded bg-amber-500/15 text-amber-400 border border-amber-500/30 hover:bg-amber-500/25 transition-colors" data-idx="${i}">? ������� ����</button>` : ''}
      </div>
    `).join('');
    ...
    container.querySelectorAll('.ab-select-btn').forEach(b => {
      b.addEventListener('click', (e) => {
        ...
        showNotification(`? ������ ${v.label}`, 'success');
      });
    });

    log('OK', 'A/B', `������������� ${variants.length} ��������� �� 1 ������`);
  } catch (err) {
    showNotification(`������: ${err.message}`, 'error');
  } finally {
    if (btn) { btn.disabled = false; btn.textContent = '?? ��� 3 ��������'; }
  }
}
...
async function createCustomCharacter() {
  if (!isPromoValid()) { showCCStatus('?? ����� �����-���', 'text-amber-400'); return; }
  ...
  if (!nameRu) { showCCStatus('������� ��� ���������', 'text-red-400'); return; }
  if (!appearance) { showCCStatus('������� ���������', 'text-red-400'); return; }

  showCCStatus('�������� ������ � ������ ���������...', 'text-cyan-400 animate-pulse');

  const id = 'custom_' + nameRu.toLowerCase().replace(/[^�-�a-z0-9]/gi, '_').replace(/_+/g, '_') + '_' + Date.now().toString(36);
  ...
  const _isMale = /���|���|���|����|man|male|boy/i.test(appearance + ' ' + group);
  ...
  const autoAnchors = {
    ...
    signature_element: appearance.split('.').find(s => /[�-�A-Z]{2,}/.test(s))?.trim() || 'distinctive feature',
    micro_gesture: 'natural expressive gestures',
    wardrobe_anchor: appearance.split('.').find(s => /�����|������|������|�����|�����|������|������|����|����|�����|�����|�����|���|����|apron|coat|dress|shirt|jacket/i.test(s))?.trim() || 'casual clothing',
    accessory_anchors: extractTokens(['����', '����', '�����', '�����', '�����', '���', '�����', '�����', '������', 'glasses', 'watch', 'ring', 'earring', 'bracelet', 'chain', 'pendant', 'brooch', 'cane']),
    footwear_anchor: appearance.split('.').find(s => /����|������|�����|�����|��������|shoes|boots|slippers|sneakers/i.test(s))?.trim() || 'worn comfortable footwear',
    headwear_anchor: appearance.split('.').find(s => /����|����|�����|������|����|�����|hat|cap|beret|headscarf|beanie/i.test(s))?.trim() || 'none',
    color_palette: extractTokens(['�����', '�����', '����', '����', '���', '�����', '������', '�����', '������', '�����', '�����', 'red', 'blue', 'green', 'black', 'white', 'grey', 'brown', 'gold', 'silver']),
    jewelry_anchors: appearance.split('.').find(s => /�����|�����|���|�����|�����|����|ring|earring|chain|bracelet|pendant|watch/i.test(s))?.trim() || 'none visible',
    glasses_anchor: /���|����|glass|spectacle|bifocal/i.test(appearance) ? appearance.split('.').find(s => /���|����|glass|spectacle/i.test(s))?.trim() || 'glasses' : 'none',
    nail_style_anchor: _isMale ? 'short trimmed nails' : 'neat manicured nails',
    fabric_texture_anchor: /����|silk/i.test(appearance) ? 'smooth silk' : /�����|wool|knit/i.test(appearance) ? 'coarse wool' : /����|cotton/i.test(appearance) ? 'soft cotton' : 'natural fabric',
    pattern_anchor: /������|floral/i.test(appearance) ? 'floral print' : /�����|stripe/i.test(appearance) ? 'striped' : /����|plaid|check/i.test(appearance) ? 'plaid checkered' : 'solid color',
    sleeve_style_anchor: /������.*�����|short.?sleeve/i.test(appearance) ? 'short sleeves' : 'long sleeves',
  };

  const isMale = _isMale;
  const autoBiology = {
    age: (appearance.match(/(\d{1,3})\s*(���|���|years?|yo\b)/i) || [])[1] || 'adult',
    height_build: appearance.split('.').find(s => /����|�����|����|���|����|�����|�����|tall|short|slim|large|massive/i.test(s))?.trim() || 'average build',
    skin_tokens: extractTokens(['�������', '����', '�����', '�����', '��������', 'wrinkles', 'skin', 'freckles', 'tan', 'pale']),
    skin_color_tokens: extractTokens(['�����', '�����', '�������', '������', 'olive', 'pale', 'tanned', 'porcelain', 'dark skin', 'fair']),
    wrinkle_map_tokens: extractTokens(['������', '������', '�����', 'wrinkle', 'crow', 'furrow', 'crease', 'lines']),
    eye_tokens: extractTokens(['�����', '������', 'eyes', 'gaze']),
    hair_tokens: extractTokens(['������', '��������', '�������', '������', '���', '�����', 'hair', 'beard', 'mustache', 'bald']),
    facial_hair_tokens: isMale ? extractTokens(['������', '���', '�����', '���������', 'beard', 'mustache', 'stubble', 'goatee']) : ['none'],
    nose_tokens: extractTokens(['���', 'nose']),
    mouth_tokens: extractTokens(['����', '���', '����', '�����', 'lips', 'mouth', 'teeth', 'smile']),
    ear_tokens: extractTokens(['��', '�����', 'ear', 'earring', 'lobe']),
    neck_tokens: extractTokens(['���', '�����', 'neck', 'throat', 'adam']),
    body_shape_tokens: extractTokens(['����', '����', '�����', '����', '���', 'shoulder', 'chest', 'belly', 'torso', 'hip']),
    hands_tokens: extractTokens(['����', '������', '�����', '�������', 'hands', 'fingers', 'ring', 'bracelet']),
    scar_mark_tokens: extractTokens(['����', '������', '����', '����', '�������', 'scar', 'birthmark', 'tattoo', 'mole', 'piercing']),
    posture_tokens: extractTokens(['�����', '����', '�����', '����', 'posture', 'stance']),
    gait_tokens: extractTokens(['������', '������', '�������', '��������', 'walk', 'shuffle', 'limp', 'stride']),
    ...
    voice_texture_tokens: isMale ? [speech?.includes('���') ? 'deep bass voice' : 'age-weathered male voice'] : [speech?.includes('����') ? 'thin high-pitched voice' : 'age-weathered female voice'],
    jaw_tokens: extractTokens(['������', 'jaw', 'jawline']),
    cheekbone_tokens: extractTokens(['����', 'cheekbone']),
    forehead_tokens: extractTokens(['���', 'forehead']),
    eyebrow_tokens: extractTokens(['����', 'eyebrow', 'brow']),
    lip_texture_tokens: extractTokens(['���', 'lip']),
    chin_tokens: extractTokens(['��������', 'chin']),
    nasolabial_tokens: ['age-appropriate nasolabial folds'],
    undereye_tokens: ['natural under-eye area'],
    shoulder_tokens: extractTokens(['����', 'shoulder']),
    teeth_tokens: extractTokens(['���', 'teeth', 'tooth']),
  ...
  try {
    ...
    if (token) {
      ...
      if (!resp.ok) {
        const err = await resp.json().catch(() => ({}));
        showCCStatus(err.error || '?? ������ ��������� �����-���� �� �������', 'text-red-400');
        log('ERR', 'CHAR-CREATE', `������ ��������: ${err.error || resp.status}`);
        return;
      }
    }
  } catch (e) {
    // Server unavailable � allow local creation as fallback
    log('WARN', 'CHAR-CREATE', `������ ����������, ������ ��������: ${e.message}`);
  }
  ...
  showCCStatus(`? �������� #${newChar.numeric_id} "${nameRu}" ������!`, 'text-emerald-400');
  showNotification(`? �������� #${newChar.numeric_id} "${nameRu}" �������� � �������`, 'success');
  log('OK', 'CHAR-CREATE', `������: #${newChar.numeric_id} ${nameRu} (${id})`);
}
...
function loadCustomCharacters() {
  try {
    ...
    if (customChars.length && state.characters) {
      ...
      if (added > 0) log('OK', 'CHAR-CUSTOM', `��������� ${added} ���������������� ����������`);
    }
...
async function createCustomLocation() {
  if (!isPromoValid()) { showLCStatus('?? ����� �����-���', 'text-amber-400'); return; }
  ...
  if (!nameRu) { showLCStatus('������� ��������', 'text-red-400'); return; }
  if (!scene) { showLCStatus('������� �����', 'text-red-400'); return; }

  showLCStatus('�������� ������...', 'text-cyan-400 animate-pulse');
  ...
  try {
    ...
    if (token) {
      ...
      if (!resp.ok) {
        const err = await resp.json().catch(() => ({}));
        showLCStatus(err.error || '?? ������ ��������� �����-���� �� �������', 'text-red-400');
        log('ERR', 'LOC-CREATE', `������ ��������: ${err.error || resp.status}`);
        return;
      }
    }
  } catch (e) {
    log('WARN', 'LOC-CREATE', `������ ����������, ������ ��������: ${e.message}`);
  }
  ...
  showLCStatus(`? ������� #${newLoc.numeric_id} "${nameRu}" �������!`, 'text-emerald-400');
  showNotification(`?? ������� #${newLoc.numeric_id} "${nameRu}" ���������`, 'success');
  log('OK', 'LOC-CREATE', `�������: #${newLoc.numeric_id} ${nameRu} (${id})`);
}
...
function loadCustomLocations() {
  try {
    ...
    if (customLocs.length && state.locations) {
      ...
      if (added > 0) log('OK', 'LOC-CUSTOM', `��������� ${added} ���������������� �������`);
    }
...
function renderEducation() {
  ...
  if (pubGuideWrap && d.publishing_guide) {
    ...
    if (missedEl && pg.missed_day_protocol) {
      missedEl.innerHTML = '<div class="text-[10px] text-amber-400 font-semibold mb-1">?? ��������� ���� � ��� ������:</div>' +
        pg.missed_day_protocol.map(m => {
          const severity = m.scenario.includes('2+') ? 'red' : m.scenario.includes('������') ? 'orange' : m.scenario.includes('2�3') ? 'yellow' : 'emerald';
          return `<div class="bg-${severity}-500/5 rounded-lg p-3 border border-${severity}-500/10"><div class="text-[11px] text-${severity}-300 font-semibold mb-1">${m.scenario}</div><div class="text-[10px] text-gray-400 mb-1"><span class="text-${severity}-400/70">�������:</span> ${m.impact}</div><div class="text-[10px] text-gray-300"><span class="font-medium">��������:</span> ${m.action}</div></div>`;
        }).join('');
    }
  ...
  if (profileGuideWrap && d.profile_guide) {
    ...
    if (elementsEl && pg.elements) {
      elementsEl.innerHTML = pg.elements.map(e =>
        `<div class="bg-purple-500/5 rounded-lg p-3 border border-purple-500/10 space-y-2">
          ...
          <div class="grid grid-cols-2 gap-2">
            <div class="rounded-lg p-2 bg-red-500/5 border border-red-500/10"><div class="text-[9px] text-red-400 font-semibold mb-1">? �����</div><div class="text-[10px] text-gray-500 leading-relaxed">${e.bad}</div></div>
            <div class="rounded-lg p-2 bg-emerald-500/5 border border-emerald-500/10"><div class="text-[9px] text-emerald-400 font-semibold mb-1">? ������</div><div class="text-[10px] text-gray-400 leading-relaxed">${e.good}</div></div>
          </div>
          <div class="text-[10px] text-gray-500 leading-relaxed"><span class="text-purple-400/70 font-medium">������:</span> ${e.why}</div>
          <div class="text-[10px] text-purple-300/80 leading-relaxed bg-purple-500/5 rounded p-2"><span class="font-medium">?? �����:</span> ${e.tip}</div>
        </div>`
      ).join('');
    }
    ...
    if (checklistEl && pg.checklist) {
      checklistEl.innerHTML = '<div class="text-[10px] text-purple-400 font-semibold mb-1">? ������� ���������� �������:</div>' +
  ...
  if (charGuideWrap && charGuideSteps && d.character_guide) {
    ...
    if (charGuideNiches && cg.niche_examples && cg.niche_examples.length) {
      charGuideNiches.innerHTML = '<div class="text-[10px] text-pink-400 font-semibold mb-1.5">������� �� �����:</div>' +
        cg.niche_examples.map(n =>
          `<div class="bg-pink-500/5 rounded-lg p-2.5 border border-pink-500/10"><div class="text-[11px] text-pink-200 font-medium">${n.niche}</div><div class="text-[10px] text-gray-400 mt-0.5"><span class="text-pink-300/80">�������:</span> ${n.formula}</div><div class="text-[10px] text-gray-500 mt-0.5">${n.why}</div></div>`
        ).join('');
    }
  ...
  if (lessonCount) lessonCount.textContent = `${d.lessons.length} ������`;
        ...
        <div class="flex items-start gap-3">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 mb-1">
              ...
              ${isRead ? '<span class="text-[9px] text-emerald-500/70 font-medium">���������</span>' : ''}
            </div>
...
function initPhotoPrompt() {
  ...
  genBtn.addEventListener('click', async () => {
    ...
    if (!isPromoValid()) {
      statusEl.innerHTML = '<span class="text-amber-400">?? ����� �����-��� � ����������</span>';
      statusEl.classList.remove('hidden');
      return;
    }
    ...
    try {
      ...
      const r = data.result;
      if (!r || !r.prompt_en) throw new Error('AI �� ������ ������');
      ...
      const cMap = { simple: ['�������', 'bg-emerald-500/15 text-emerald-400 border-emerald-500/25'], medium: ['�������', 'bg-amber-500/15 text-amber-400 border-amber-500/25'], complex: ['�������', 'bg-red-500/15 text-red-400 border-red-500/25'] };
      ...
      resultEl.classList.remove('hidden');
      statusEl.innerHTML = '<span class="text-emerald-400">? ������ �����! �������� � ������ � ImageFX</span>';
  ...
  if (copyAllBtn) {
    copyAllBtn.addEventListener('click', () => {
      ...
      const allText = _threadsData.map((p, i) => {
        ...
        const time = p.best_time?.time ? `\n? ������ �����: ${p.best_time.day || ''} ${p.best_time.time}` : '';
        return `${header}\n${original}\n${variants}${hashtags ? '\n\n??? �������: ' + hashtags : ''}${time}`;
      }).join('\n\n\n');
      navigator.clipboard.writeText(allText).then(() => {
        copyAllBtn.textContent = '? �����������!'; sfx.copy();
        setTimeout(() => { copyAllBtn.textContent = '?? ���������� ��'; }, 2000);
      });
    });
  }
  ...
  async function _doThreadsSearch() {
    ...
    if (statusEl) {
      ...
      statusEl.innerHTML = `<div class="flex items-center gap-2">
        <span class="text-violet-400 animate-pulse">?? ��� ���������� ������� � ������ �������� ������</span>
        <span class="text-[9px] text-gray-600">������ 15-30 ���</span>
      </div>`;
    }
    ...
    try {
      ...
      if (!resp.ok) {
        if (statusEl) statusEl.innerHTML = `<span class="text-red-400">? ${escapeHtml(data.error || `������ ${resp.status}`)}</span>`;
      ...
      if (badgeEl) {
        ...
        if (data.used_grounding) {
          ...
          badgeEl.textContent = `?? RSS � ${data.rss_count || ''} �������� ����������`;
        } else {
      ...
      if (!_threadsData.length) {
        if (statusEl) statusEl.innerHTML = '<span class="text-amber-400">?? ����� �� �������. �������� ������ ���� ��� ������� �������.</span>';
        if (resultsEl) resultsEl.innerHTML = '';
        return;
      }
      ...
      // Status
      if (statusEl) {
        const src = data.used_grounding ? '�� ������ �������� �������� (Google)' : data.has_rss ? `�� ������ ${data.rss_count} �������� ���������� (RSS)` : '?? ��� ����������� ����������';
        const staleWarn = data.stale_count > 0 ? ` � <span class="text-amber-400">${data.stale_count} ����������</span>` : '';
        const srcColor = (data.used_grounding || data.has_rss) ? 'text-emerald-400' : 'text-amber-400';
        statusEl.innerHTML = `<span class="${srcColor}">? ������� ${_threadsData.length} ������${query ? ' �� �' + escapeHtml(query) + '�' : ''} � ${src}</span>${staleWarn}`;
      }
      ...
      _renderThreadsPosts();
      log('OK', 'THREADS', `��������� ${_threadsData.length} ��������� ������`);
  ...
  if (resultsEl) {
    resultsEl.addEventListener('click', e => {
      ...
      if (copyPost) {
        ...
        navigator.clipboard.writeText(text).then(() => {
          copyPost.textContent = '? �����������'; sfx.copy();
          setTimeout(() => { copyPost.textContent = '?? ����������'; }, 1500);
        });
        return;
      }
      // Copy variant text
      ...
      if (copyHash) {
        ...
        navigator.clipboard.writeText(text).then(() => {
          copyHash.textContent = '? �����������'; sfx.copy();
          setTimeout(() => { copyHash.textContent = '?? ���������� �������'; }, 1500);
        });
        return;
      }
      ...
      if (toggleAnalysis) {
        ...
        toggleAnalysis.textContent = isHidden ? '^ ������ ������' : '� ������ ��� �������';
        sfx.clickSoft();
        return;
      }
      ...
      if (toggleReels) {
        ...
        const isHidden = body.classList.contains('hidden');
        body.classList.toggle('hidden', !isHidden);
        toggleReels.textContent = isHidden ? '^ ������' : '?? ���� ��� Reels';
        sfx.clickSoft();
        return;
      }
...
async function _sendToThreadsQueue(posts, triggerBtn) {
  ...
  try {
    ...
    if (triggerBtn) {
      triggerBtn.textContent = `? ${data.added} � �������`;
...
async function _updateThreadsQueueBadge() {
  ...
  try {
    ...
    badge.textContent = `?? ${data.pending} ���� � Buffer: ${depth}`;
...
const _confidenceMeta = {
  high:   { label: '?? �� ������ �������', cls: 'bg-emerald-500/15 text-emerald-400 border-emerald-500/25', dot: 'bg-emerald-400' },
  medium: { label: '?? �� ������ ������',  cls: 'bg-amber-500/15 text-amber-400 border-amber-500/25',    dot: 'bg-amber-400' },
  low:    { label: '?? �� ���������',      cls: 'bg-red-500/15 text-red-400 border-red-600/25',          dot: 'bg-red-500' },
};

const _varStyleMeta = {
  bold:      { icon: '??', label: '����������' },
  smart:     { icon: '??', label: '��������' },
  emotional: { icon: '??', label: '������ �������' },
  viral:     { icon: '??', label: '��������' },
  personal:  { icon: '??', label: '������ ����' },
};

function _viralityColor(score) {
  if (score >= 80) return { bar: 'bg-emerald-500', text: 'text-emerald-400', label: '����� ??' };
  if (score >= 60) return { bar: 'bg-violet-500',  text: 'text-violet-400',  label: '�������' };
  if (score >= 40) return { bar: 'bg-amber-500',   text: 'text-amber-400',   label: '�������' };
  return                   { bar: 'bg-gray-500',    text: 'text-gray-400',    label: '������' };
}
...
function _renderThreadsPosts() {
  ...
  resultsEl.innerHTML = _threadsData.map((post, idx) => {
    ...
    const sigLikes  = post.signals?.likes_est || '����������';
    const sigComm   = post.signals?.comments_est || '����������';
    const sigRepost = post.signals?.reposts_est || '����������';
    ...
    const viralityHtml = `
      ...
      <div class="flex gap-1.5 mt-1">
        <span class="text-[8px] text-gray-600" title="�������� (0-30)">??${sb.comment_potential || sb.debate || 0}</span>
        <span class="text-[8px] text-gray-600" title="������������ (0-25)">??${sb.relatability || sb.depth || 0}</span>
        <span class="text-[8px] text-gray-600" title="������ (0-25)">??${sb.shareability || 0}</span>
        <span class="text-[8px] text-gray-600" title="������ (0-20)">??${sb.emotion || 0}</span>
      </div>`;
      ...
      return `<div class="threads-var-panel ${v.style !== (firstVar?.style || 'bold') ? 'hidden' : ''}" data-style="${escapeHtml(v.style)}">
        <div class="flex items-start gap-2">
          <div class="flex-1 space-y-1.5">
            ...
            <div class="flex items-center gap-2">
              <span class="${charColor} text-[9px] font-medium">${charCount} ��������</span>
              ${charCount <= 500 ? '<span class="text-[8px] text-emerald-600">? Threads OK</span>' : '<span class="text-[8px] text-red-600">? �������</span>'}
            </div>
          </div>
      ...
      <div class="border-t border-white/[0.04] px-5 py-4 space-y-2" style="background:rgba(232,121,168,0.02)">
        <div class="flex items-center justify-between">
          <button class="threads-toggle-hashtags text-[10px] text-fuchsia-400 font-semibold uppercase tracking-[0.15em] hover:text-fuchsia-300 transition-colors flex items-center gap-1.5">
            <span>???</span> <span>������� (${allH.length})</span> <span class="text-gray-600">�</span>
          </button>
          <button class="threads-copy-hashtags btn-neon-pink text-[9px] px-2 py-0.5" data-text="${escapeHtml(allH.join(' '))}">?? ����������</button>
        </div>
        <div class="threads-hashtags-body hidden space-y-2 pt-1">
          ${post.hashtags?.high_volume?.length ? `<div class="space-y-1"><div class="text-[8px] text-gray-600 uppercase tracking-wider">����������</div><div class="flex flex-wrap gap-1">${post.hashtags.high_volume.map(h => `<span class="tag tag-green">${escapeHtml(h)}</span>`).join('')}</div></div>` : ''}
          ${post.hashtags?.mid_volume?.length ? `<div class="space-y-1"><div class="text-[8px] text-gray-600 uppercase tracking-wider">�������</div><div class="flex flex-wrap gap-1">${post.hashtags.mid_volume.map(h => `<span class="tag tag-purple">${escapeHtml(h)}</span>`).join('')}</div></div>` : ''}
          ${post.hashtags?.niche?.length ? `<div class="space-y-1"><div class="text-[8px] text-gray-600 uppercase tracking-wider">�������</div><div class="flex flex-wrap gap-1">${post.hashtags.niche.map(h => `<span class="tag" style="background:rgba(6,182,212,0.08);border-color:rgba(6,182,212,0.18);color:#22d3ee">${escapeHtml(h)}</span>`).join('')}</div></div>` : ''}
        </div>
      </div>` : '';
    ...
    const reelHtml = (post.reel_ideas || []).map((r, i) => `
      <div class="rounded-lg p-3 border border-cyan-500/15 bg-cyan-500/5 space-y-1.5">
        ...
        ${r.conflict ? `<div class="text-[10px] text-gray-400"><span class="text-amber-400/70 font-medium">��������:</span> ${escapeHtml(r.conflict)}</div>` : ''}
        ${r.direction ? `<div class="text-[10px] text-gray-400"><span class="text-violet-400/70 font-medium">��������:</span> ${escapeHtml(r.direction)}</div>` : ''}
    ...
    <div class="threads-card" data-post-id="${escapeHtml(post.id)}">
      ...
      <div class="p-5 pb-4 space-y-3">
        ...
        <div class="flex items-start gap-3">
          ...
          <div class="flex-1 min-w-0 space-y-1">
            ...
            <div class="flex items-center gap-2 text-[9px] text-gray-600">
              ...
              ${sigLikes !== '����������' ? `<span>?? ${escapeHtml(sigLikes)}</span>` : ''}
              ${sigComm !== '����������' ? `<span>?? ${escapeHtml(sigComm)}</span>` : ''}
              ${sigRepost !== '����������' ? `<span>?? ${escapeHtml(sigRepost)}</span>` : ''}
            </div>
          </div>
        ...
        <div class="flex items-start gap-1.5 rounded-lg p-2 bg-cyan-500/5 border border-cyan-500/15">
          ...
          <div class="space-y-0.5">
            ...
            ${post.news_url ? `<a href="${escapeHtml(post.news_url)}" target="_blank" rel="noopener" class="block text-[9px] text-cyan-600 hover:text-cyan-400 truncate transition-colors">?? ��������</a>` : ''}
          </div>
        </div>` : ''}
        ...
        <div class="flex items-center gap-2 flex-wrap pt-1">
          <button class="threads-copy-post btn-neon text-[10px] px-2.5 py-1" data-text="${escapeHtml(post.text)}">?? ����������</button>
          <button class="threads-queue-post text-[10px] px-2.5 py-1 rounded-lg bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 border border-emerald-500/20 transition-all font-medium" data-post-idx="${idx}">?? � �������</button>
          ${hasUrl ? `<a href="${escapeHtml(post.url)}" target="_blank" rel="noopener noreferrer" class="btn-neon text-[10px] px-2.5 py-1" style="text-decoration:none">?? �������</a>` : ''}
          <button class="threads-toggle-analysis text-[10px] px-2.5 py-1 rounded-lg bg-amber-500/10 text-amber-400 hover:bg-amber-500/20 border border-amber-500/20 transition-all font-medium">� ������</button>
        </div>
      </div>
      ...
      <div class="threads-analysis-body hidden border-t border-white/[0.04] px-5 py-4 space-y-3" style="background:rgba(245,158,11,0.02)">
        ...
        <div class="rounded-xl p-3 bg-gradient-to-r from-emerald-500/8 to-cyan-500/5 border border-emerald-500/15">
          <div class="text-[9px] text-emerald-500 font-semibold uppercase tracking-[0.15em] mb-1">�������� ������</div>
        ...
        <div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
          ${post.analysis?.hook ? `<div class="rounded-lg p-2.5 bg-black/20 border border-amber-500/10 space-y-1"><div class="text-[9px] text-amber-400/80 font-semibold uppercase tracking-[0.1em]">���</div><div class="text-[11px] text-gray-300">${escapeHtml(post.analysis.hook)}</div></div>` : ''}
          ${post.analysis?.conflict ? `<div class="rounded-lg p-2.5 bg-black/20 border border-red-500/10 space-y-1"><div class="text-[9px] text-red-400/80 font-semibold uppercase tracking-[0.1em]">��������</div><div class="text-[11px] text-gray-300">${escapeHtml(post.analysis.conflict)}</div></div>` : ''}
          ${post.analysis?.audience_pain ? `<div class="rounded-lg p-2.5 bg-black/20 border border-violet-500/10 space-y-1"><div class="text-[9px] text-violet-400/80 font-semibold uppercase tracking-[0.1em]">���� ���������</div><div class="text-[11px] text-gray-300">${escapeHtml(post.analysis.audience_pain)}</div></div>` : ''}
          ${post.analysis?.cta_potential ? `<div class="rounded-lg p-2.5 bg-black/20 border border-cyan-500/10 space-y-1"><div class="text-[9px] text-cyan-400/80 font-semibold uppercase tracking-[0.1em]">CTA-���������</div><div class="text-[11px] text-gray-300">${escapeHtml(post.analysis.cta_potential)}</div></div>` : ''}
        </div>
        ...
        <div class="rounded-xl p-3 bg-gradient-to-r from-violet-500/5 to-fuchsia-500/5 border border-violet-500/15 space-y-2">
          <div class="text-[9px] text-violet-400 font-semibold uppercase tracking-[0.15em]">?? ������������� �����������</div>
      ...
      <div class="border-t border-white/[0.04] px-5 py-4 space-y-3" style="background:rgba(124,92,252,0.02)">
        <div class="text-[10px] text-violet-400 font-semibold uppercase tracking-[0.15em]">������� ����� ��� ����������</div>
      ...
      <div class="border-t border-white/[0.04] px-5 py-4 space-y-2" style="background:rgba(6,182,212,0.02)">
        <button class="threads-toggle-reels text-[10px] text-cyan-400 font-semibold uppercase tracking-[0.15em] hover:text-cyan-300 transition-colors w-full text-left flex items-center gap-1.5">
          <span>??</span> <span>���� ��� Reels (${post.reel_ideas.length})</span> <span class="text-gray-600 ml-auto">�</span>
        </button>
