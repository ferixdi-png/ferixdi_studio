...
function initMobileMenu() {
  ...
  // Show/hide based on screen size (respect force-mobile)
...
function updateCharDisplay() {
  ...
  if (charAName) {
    if (state.selectedA) {
      ...
      charAName.innerHTML = '<span class="text-gray-400">Нажми на персонажа v</span>';
    }
  }
  // B slot
  ...
  if (charBName) {
    if (state.selectedB) {
      ...
      charBName.innerHTML = '<span class="text-gray-400">Нажми на второго или пропусти v</span>';
    }
  }
...
function getCharacterRecommendations(topicText) {
  ...
  // Цены и инфляция
    ...
    recommendations.push(
      ...
      { id: 'ded_matvey', reason: 'ٸголь — элегантно рассуждает о деньгах' },
...
function initLogPanel() {
  document.getElementById('log-toggle')?.addEventListener('click', () => {
    ...
    if (icon) icon.textContent = collapsed ? 'Ў' : '^';
  });
}
...
function _renderTrendStats() {
  ...
  el.innerHTML = `
    ...
    <div class="trend-stat"><span>🔥</span> <span class="trend-stat-value">${avgViral}</span>/10</div>
...
function initJokesLibrary() {
  log('OK', 'JOKES', 'Секция «Источники шуток» — статические ссылки, JS не требуется');
}
  ...
  return `
    <div class="ep-card bg-black/20 rounded-lg border border-gray-800/60 overflow-hidden">
      ...
      <div class="ep-body hidden p-3 space-y-2">
        ...
        </div>` : '<div class="text-[10px] text-gray-600 italic">ℹ️ Промпт не сохранён (старый эпизод)</div>'}
    ...
    const renderStopList = (filter) => {
      ...
      if (toggleEl) {
        if (!f && all.length > STOP_PREVIEW && !stopExpanded) {
          ...
          toggleEl.innerHTML = `<button class="text-[10px] text-red-400/70 hover:text-red-400 transition-colors">Показать все ${all.length} ошибок v</button>`;
  ...
  if (resultsEl) {
    resultsEl.addEventListener('click', e => {
      ...
      if (toggleAnalysis) {
        ...
        toggleAnalysis.textContent = isHidden ? '^ Скрыть анализ' : 'Ў Почему это вирусно';
        sfx.clickSoft();
        return;
      }
